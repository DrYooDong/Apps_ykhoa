...
Để thực hiện chiến lược này, các chuyên gia đã thống nhất các định nghĩa tiêu chuẩn về các nồng độ ức chế, các kỹ thuật giám sát thuốc (TDM) và mô hình dự phóng liều lượng cá thể hóa (MIPD).

[Table 2: Important terms and definitions] - trích từ [Consensus Guidance for Beta‐Lactam Antibiotic Dose Individualization in Acutely Ill - ACCP 2026.pdf]

|Key term|Definition|
|---|---|
|Minimal inhibitory concentration (MIC)|Lowest concentration of a given antimicrobial that inhibits visible growth of an organism when tested in a standardized in vitro setting. The MIC is used to define antibiotic potency during drug development, to assess preclinical PK/PD relationships and when combined with clinical breakpoints, and to select antibiotics for a given patient.|
|Clinical (i.e., susceptibility) breakpoints|MIC cut-off value used to identify an isolate as susceptible, intermediate, susceptible dose-dependent, resistant to a given antibiotic using typical doses; used for antibiotic selection for an individual patient|
|Epidemiological cutoff value (ECV, ECOFF)|The highest MIC of isolates without phenotypic resistance ('wild type')|
|100% $fT_{>MIC}$|Free drug concentration above the MIC of the organism for 100% of the dosing interval|
|100% $fT_{>4xMIC}$|Free drug concentration above 4 times the MIC of the organism for 100% of the dosing interval|
|Therapeutic drug monitoring (TDM)|Measurement of drug concentration in blood or other biological fluids for the purpose of individualizing therapy. Typically occurs on day 1-3 of therapy or after. May or may not be combined with the use of an up-front dosing model (i.e., population pharmacokinetic model, nomogram to aid initial dose selection). One or two specimens are collected to estimate the fT>MIC.|
|Model-informed personalized dosing (MIPD)|Use of a population pharmacokinetic model to guide upfront dosing of beta-lactam therapy. This approach considers more covariates, in complex relationships, than would otherwise be used in traditional simplified dosing schema. Individual observed concentrations obtained at any time, combined with a population pharmacokinetic model, are then used to determine individual pharmacokinetic parameters and probability of pharmacokinetic/pharmacodynamic target attainment. Also known as model-informed precision dosing, but the panel favors the use of model-informed personalized dosing to balance prioritization of both accuracy and precision.|
|Beta-lactam dose individualization|Dosing regimen for one patient based on measured concentrations from that patient|
|Prolonged infusion beta-lactam therapy|Drug delivery over at least 2h which broadly includes extended (non-continuous) and continuous infusion (over 24h) therapy; use of prolonged infusions is a key tactic to achieve the minimum PK/PD target with beta-lactam dose individualization.|
|Beta-lactam concentration discordance|Numerical difference between the observed concentration measured in the patient and the concentration predicted by the model. Rarely will these numbers be in total agreement, i.e., that observed minus predicted $=0$ Discordance is graded and must be evaluated in the context of the individual patient, but an absolute value of > 20% prediction error from the observation represents a threshold of concern based on extrapolation of FDA bioequivalence criteria. The higher the discordance beyond this threshold, the less reliable the MIPD becomes.|
|Estimated free beta-lactam concentration|Estimated free concentration = total measured concentration multiplied by the fraction unbound|

_Chú thích bảng:_ [Table 2: Important terms and definitions] định nghĩa các thuật ngữ trọng tâm được sử dụng trong đánh giá dược động học và dược lực học của kháng sinh.
