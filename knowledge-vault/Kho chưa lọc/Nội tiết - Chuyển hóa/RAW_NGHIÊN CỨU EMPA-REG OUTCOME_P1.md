---
title: "NGHIÊN CỨU EMPA-REG OUTCOME"
part: "P1"
aliases:
  - "NGHIÊN CỨU EMPA-REG OUTCOME"
keywords:
  - "nghiên cứu empa-reg outcome"
  - "tim mạch"
  - "nội tiết - chuyển hóa"
specialty: "Tim mạch"
kho: "Kho chưa lọc"
tags:
  - "he-co-quan/tim-mach"
  - "loai/general"
  - "y-khoa/raw"
updated: "2026-08-21"
---



# BÀI HỌC LÂM SÀNG: NGHIÊN CỨU EMPA-REG OUTCOME

---

## 1. Nội Dung Sinh Lý Học & Kết Quả Lâm Sàng Chi Tiết

Nghiên cứu **EMPA-REG OUTCOME** (Empagliflozin, Cardiovascular Outcomes, and Mortality in Type 2 Diabetes) là một thử nghiệm lâm sàng landmark ngẫu nhiên, mù đôi, kiểm chứng với placebo. Nghiên cứu này đánh giá hiệu quả và độ an toàn của empagliflozin – một chất ức chế đồng vận chuyển sodium–glucose cotransporter 2 (SGLT2) – khi phối hợp vào phác đồ điều trị tiêu chuẩn ở bệnh nhân đái tháo đường tuýp 2 có nguy [[[[Tim]]|cơ tim]] mạch cao.

---

### Cơ Chế Sinh Lý Học của Thuốc Ức Chế SGLT2 (Empagliflozin)

- **Cơ chế tác dụng tại [[Thận]]**: [[Thận]] đóng vai trò quan trọng trong việc hằng định nội môi glucose thông qua quá trình lọc và tái hấp thu. Ở người bình thường, SGLT2 nằm ở ống lượn gần chịu trách nhiệm tái hấp thu phần lớn lượng glucose được lọc qua cầu thận.
- **Hiệu ứng bài niệu glucose**: Empagliflozin là chất ức chế chọn lọc SGLT2. Bằng cách giảm tái hấp thu glucose ở ống lượn gần, thuốc làm tăng đào thải glucose qua nước tiểu, từ đó giúp giảm nồng độ glucose trong máu ở bệnh nhân đái tháo đường tuýp 2.
- **Hiệu ứng huyết động và chuyển hóa ngoài đường huyết**:
    - Giảm cân nặng và mỡ tạng.
    - Hạ huyết áp (cả tâm thu và tâm trương) thông qua hiệu ứng lợi tiểu thẩm thấu và thải natri mà không làm tăng nhịp tim phản xạ.
    - Cải thiện các chỉ số về độ cứng động mạch và kháng lực mạch máu.
    - Giảm nồng độ acid uric huyết thanh và giảm albumin niệu.
    - Tăng nhẹ nồng độ LDL-cholesterol và HDL-cholesterol.

---

### Thiết Kế Nghiên Cứu và Đối Tượng Bệnh Nhân

- **Thiết kế**: Nghiên cứu đa trung tâm được thực hiện tại 590 trung tâm ở 42 quốc gia. Bệnh nhân được phân ngẫu nhiên theo tỷ lệ 1:1:1 vào ba nhóm: empagliflozin 10 mg/ngày, empagliflozin 25 mg/ngày, hoặc placebo uống một lần mỗi ngày.
- **Tiêu chuẩn chọn bệnh chính**:
    - Người trưởng thành ((\ge 18) tuổi), chỉ số khối cơ thể (BMI) (\le 45) kg/m².
    - Độ lọc cầu thận ước tính (eGFR) (\ge 30) mL/phút/1.73 m².
    - Đã xác định có bệnh lý tim mạch do xơ vữa từ trước (established cardiovascular disease).
    - HbA1c từ 7.0% đến 9.0% đối với bệnh nhân chưa dùng thuốc hạ glucose trong ít nhất 12 tuần trước phân nhóm; hoặc HbA1c từ 7.0% đến 10.0% đối với bệnh nhân đang có phác đồ hạ glucose ổn định.
- **Thời gian theo dõi**: Thời gian điều trị trung vị là 2.6 năm và thời gian quan sát trung vị là 3.1 năm. Tổng cộng có 7020 bệnh nhân được điều trị và đưa vào phân tích sửa đổi theo trình tự định ý điều trị (modified intention-to-treat).

---

### Các Kết Cục Tim Mạch và Tử Vong Chính

#### **Mô tả Hình họa: Các biểu đồ tích lũy biến cố lâm sàng**

_Tên hình:_ **Figure 1. Cardiovascular Outcomes and Death from Any Cause**  
_Thuộc tài liệu:_ `NEJMoa1504720.pdf` trong Notebook.

```
  A. Tiêu chí gộp chính (3-point MACE)          B. Tử vong do nguyên nhân Tim mạch
    15% ────────────────────────── Placebo        8% ────────────────────────── Placebo
        ▲              ┌──────────                ▲              ┌──────────
    10% ┼──────────▲───┘ Empagliflozin            4% ┼──────────▲───┘ Empagliflozin
        │      ┌───┘                                  │      ┌───┘
     0% └──────┴───────────────────────            0% └──────┴───────────────────────
        0      12     24     36     48 (Tháng)        0      12     24     36     48 (Tháng)

  C. Tử vong do mọi nguyên nhân                  D. Nhập viện do suy tim
    12% ────────────────────────── Placebo         6% ────────────────────────── Placebo
        ▲              ┌──────────                 ▲              ┌──────────
     6% ┼──────────▲───┘ Empagliflozin             3% ┼──────────▲───┘ Empagliflozin
        │      ┌───┘                                   │      ┌───┘
     0% └──────┴───────────────────────             0% └──────┴───────────────────────
        0      12     24     36     48 (Tháng)         0      12     24     36     48 (Tháng)
```

- **Nội dung mô tả chi tiết dưới biểu đồ Figure 1**:
    1. **Tiêu chí gộp chính (Panel A)**: Tỷ lệ biến cố tim mạch chính (3-point MACE: tử vong do nguyên nhân tim mạch, nhồi máu cơ tim không tử vong, hoặc đột quỵ không tử vong) ở nhóm gộp sử dụng empagliflozin thấp hơn đáng kể so với nhóm placebo (10.5% so với 12.1%; Tỷ số nguy hại [HR] 0.86; khoảng tin cậy [CI] 95.02%: 0.74 đến 0.99; P = 0.04 cho tính ưu trội).
    2. **Tử vong do nguyên nhân Tim mạch (Panel B)**: Nhóm empagliflozin có tỷ lệ tử vong do tim mạch thấp hơn đáng kể (3.7% so với 5.9% ở nhóm placebo; HR 0.62; CI 95%: 0.49 đến 0.77; P < 0.001), tương ứng với mức giảm 38% nguy cơ tương đối. Sự tách biệt giữa hai đường cong tích lũy xuất hiện rất sớm trong thử nghiệm và duy trì ổn định kéo dài suốt quá trình nghiên cứu.
    3. **Tử vong do mọi nguyên nhân (Panel C)**: Giảm đáng kể 32% nguy cơ tương đối ở nhóm điều trị bằng empagliflozin (5.7% so với 8.3% ở nhóm placebo; HR 0.68; CI 95%: 0.57 to 0.82; P < 0.001). Để ngăn ngừa một ca tử vong trong thời gian 3 năm, số lượng bệnh nhân cần điều trị (NNT) ước tính là 39 người.
    4. **Nhập viện do suy tim (Panel D)**: Nguy cơ nhập viện do suy tim ở nhóm dùng empagliflozin giảm 35% tương đối so với nhóm placebo (2.7% so với 4.1%; HR 0.65; CI 95%: 0.50 đến 0.85; P = 0.002).

---

#### **Bảng thông tin: Tóm tắt các kết cục tim mạch và tử vong chính**

_Tên bảng:_ **Table 1. Primary and Secondary Cardiovascular Outcomes**  
_Tài liệu tham khảo:_ `NEJMoa1504720.pdf` trong Notebook.

|Tiêu chí kết cục lâm sàng|Nhóm Placebo (N=2333)|Nhóm Empagliflozin gộp (N=4687)|Tỷ số nguy hại HR (95% CI)|Trị số P|
|:--|:-:|:-:|:-:|:-:|
|**Tiêu chí gộp chính (3-point MACE)**|**282 (12.1%)**|**490 (10.5%)**|**0.86 (0.74–0.99)**|**0.04 (ưu trội)**|
|**Tiêu chí gộp phụ** (MACE + nhập viện [[Đau ngực|đau thắt ngực]] không ổn định)|333 (14.3%)|599 (12.8%)|0.89 (0.78–1.01)|0.08 (ưu trội)|
|**Tử vong do mọi nguyên nhân**|**194 (8.3%)**|**269 (5.7%)**|**0.68 (0.57–0.82)**|**<0.001**|
|**Tử vong do nguyên nhân Tim mạch**|**137 (5.9%)**|**172 (3.7%)**|**0.62 (0.49–0.77)**|**<0.001**|
|Nhồi máu cơ tim (tử vong hoặc không tử vong)*|126 (5.4%)|223 (4.8%)|0.87 (0.70–1.09)|0.23|
|Đột quỵ (tử vong hoặc không tử vong)|69 (3.0%)|164 (3.5%)|1.18 (0.89–1.56)|0.26|
|**Nhập viện do suy tim**|**95 (4.1%)**|**126 (2.7%)**|**0.65 (0.50–0.85)**|**0.002**|
|Nhập viện do [[Đau ngực|đau thắt ngực]] không ổn định|66 (2.8%)|133 (2.8%)|0.99 (0.74–1.34)|0.97|

_*Ghi chú:_ Không bao gồm nhồi máu cơ tim thầm lặng (silent myocardial infarction). Không có sự khác biệt có ý nghĩa thống kê giữa hai nhóm đối với biến cố nhồi máu cơ tim hoặc đột quỵ đơn độc.

---

#### **Sơ đồ & Hướng dẫn: Đánh giá phân nhóm đối với tử vong do Tim mạch**

_Tên hình:_ **Figure 2. Subgroup Analyses for the Primary Outcome and Death from Cardiovascular Causes**  
_Tài liệu tham khảo:_ `NEJMoa1504720.pdf` trong Notebook.

```
                 [ PHÂN TÍCH PHÂN NHÓM TỬ VONG TIM MẠCH ]
                                    │
       ┌────────────────────────────┼────────────────────────────┐
       ▼                            ▼                            ▼
  [ Theo Tuổi ]               [ Theo HbA1c ]               [ Theo eGFR ]
  - < 65 tuổi: HR 0.71        - < 8.5%: HR 0.60            - >= 90: HR 0.78
  - >= 65 tuổi: HR 0.54       - >= 8.5%: HR 0.66           - 60 đến < 90: HR 0.57
                                                           - < 60: HR 0.59
```

- **Các điểm lưu ý lâm sàng từ phân tích phân nhóm Figure 2**:
    1. Hiệu quả giảm tử vong do nguyên nhân tim mạch của empagliflozin là **nhất quán và đồng bộ** trên tất cả các phân nhóm bệnh nhân dựa trên các đặc điểm nền tảng bao gồm tuổi tác, giới tính, chủng tộc, mức HbA1c ban đầu, chỉ số BMI, mức lọc cầu thận eGFR và các thuốc tim mạch điều trị phối hợp đi kèm (như statin, thuốc ức chế men chuyển ACEi hoặc chẹn thụ thể ARB).

---

### Các Chỉ Số Kiểm Soát Đường Huyết và Chuyển Hóa

#### **Mô tả Hình họa: Diễn tiến mức HbA1c trong nghiên cứu**

_Tên hình:_ **Figure 3. Glycated Hemoglobin Levels**  
_Tài liệu tham khảo:_ `NEJMoa1504720.pdf` trong Notebook.

```
   Adjusted Mean HbA1c (%)
    8.5% ────────────────────────────────────────── Placebo
         ▲                   ┌─────────────────────
    8.0% ┼───────────────────┼───────────────────── Empagliflozin 10 mg
         │  ┌────────────────┴───────────────────── Empagliflozin 25 mg
    7.5% ┼──┼──────────────────────────────────────
         │  ▼ (Tuần 12)
    7.0% └─────────────────────────────────────────
         0  12  28  40  52  66  80  94  122 150 178 206 (Tuần)
```

- **Nội dung mô tả chi tiết dưới biểu đồ Figure 3**:
    1. Tại thời điểm **tuần thứ 12** (giai đoạn phác đồ thuốc hạ glucose nền được yêu cầu giữ nguyên tối đa), mức chênh lệch HbA1c hiệu chỉnh giữa nhóm empagliflozin so với placebo là **-0.54%** (đối với liều 10 mg) và **-0.60%** (đối với liều 25 mg).
    2. Tại thời điểm **tuần thứ 94**, mức chênh lệch hiệu chỉnh lần lượt là **-0.42%** và **-0.47%**. Tại thời điểm **tuần thứ 206**, mức chênh lệch là **-0.24%** và **-0.36%**. Mặc dù phác đồ hạ glucose sau tuần 12 được cho phép điều chỉnh tự do theo thực tế lâm sàng, nhóm empagliflozin vẫn duy trì mức HbA1c thấp hơn placebo xuyên suốt thời gian thử nghiệm.

---

### Độ An Toàn và Các Biến Cố Bất Lợi

Tỷ lệ gặp các biến cố bất lợi tổng thể, biến cố bất lợi nghiêm trọng, hoặc biến cố dẫn đến ngừng thuốc là tương đồng giữa nhóm empagliflozin và placebo.

#### **Bảng thông tin: Tóm tắt các biến cố bất lợi chọn lọc**

_Tên bảng:_ **Table 2. Adverse Events**  
_Tài liệu tham khảo:_ `NEJMoa1504720.pdf` trong Notebook.

|Biến cố bất lợi chọn lọc|Nhóm Placebo (N=2333)|Nhóm Empagliflozin gộp (N=4687)|Ý nghĩa thống kê (Trị số P)|
|:--|:-:|:-:|:-:|
|**Bất kỳ biến cố bất lợi nào**|2139 (91.7%)|4230 (90.2%)|P < 0.05|
|Biến cố bất lợi nghiêm trọng|988 (42.3%)|1789 (38.2%)|P < 0.001|
|[[Hạ đường huyết]] xác định (bất kỳ)|650 (27.9%)|1303 (27.8%)|Không khác biệt|
|[[Hạ đường huyết]] nặng cần hỗ trợ|36 (1.5%)|63 (1.3%)|Không khác biệt|
|Biến cố nghi nhiễm trùng tiểu (UTI)|423 (18.1%)|842 (18.0%)|Không khác biệt|
|**Nhiễm trùng sinh dục** (Nam giới)|**25 (1.5%)**|**166 (5.0%)**|**P < 0.001**|
|**Nhiễm trùng sinh dục** (Nữ giới)|**17 (2.6%)**|**135 (10.0%)**|**P < 0.001**|
|Suy thận cấp|155 (6.6%)|246 (5.2%)|P < 0.01 (Ưu thế thuộc Empa)|
|Tổn thương thận cấp|37 (1.6%)|45 (1.0%)|P < 0.05 (Ưu thế thuộc Empa)|
|Nhiễm toan ceton do đái tháo đường (DKA)|1 (<0.1%)|4 (0.1%)|Tỷ lệ rất thấp, không khác biệt|
|Biến cố do giảm thể tích tuần hoàn|115 (4.9%)|239 (5.1%)|Không khác biệt|
|Gãy xương|91 (3.9%)|179 (3.8%)|Không khác biệt|

- **Các điểm lưu ý lâm sàng đặc biệt về độ an toàn**:
    1. **Nhiễm trùng sinh dục**: Là tác dụng phụ phổ biến nhất tăng lên rõ rệt khi dùng empagliflozin (đạt 10.0% ở nữ giới và 5.0% ở nam giới dùng thuốc).
    2. **An toàn trên thận**: Tỷ lệ suy thận cấp và tổn thương thận cấp ở nhóm dùng empagliflozin thấp hơn đáng kể so với placebo, chứng minh vai trò duy trì ổn định chức năng thận lâu dài của nhóm thuốc này.
    3. **Xét nghiệm máu**: Chỉ số dung tích [[Hồng cầu]] (hematocrit) tăng nhẹ ở các nhóm sử dụng empagliflozin (tăng khoảng 4.8% đến 5.0% so với mức nền) do tác dụng bài niệu thẩm thấu.

---

## 2. Trích Dẫn Tài Liệu Tham Khảo Chuẩn AMA

1. Zinman B, Wanner C, Lachin JM, et al; EMPA-REG OUTCOME Investigators. Empagliflozin, cardiovascular outcomes, and mortality in type 2 diabetes. _N Engl J Med_. 2015;373(22):2117–2128. doi:10.1056/NEJMoa1504720.

---

🩺 **Ý tưởng thảo luận tiếp theo**: Bạn có muốn tôi tiến hành phân tích sâu hơn về **cơ chế huyết động và sinh lý thận bảo vệ tim mạch** của nhóm thuốc SGLT2i dựa trên các tài liệu đồng thuận y khoa hiện có trong Notebook không?