...
description: Hướng dẫn thiết kế custom tools, JSON Schema, error handling và mô tả tool chuẩn xác cho LLM/AI Agent. Kích hoạt khi cần tạo script công cụ hoặc MCP tools mới.
---
...
- multi-agent-coordination â†’ multi-agent-orchestration
- agent-memory â†’ agent-memory-systems
- api-design â†’ api-designer
- llm-prompting â†’ prompt-engineering
...
def get_weather(location: str) -> ToolResult:
    ...
    try:
        ...
        return ToolResult(
            success=True,
            content=f"Temperature: {data.temp}Â°F, Conditions: {data.conditions}"
        )
...
def get_weather(location: str, unit: str = "fahrenheit") -> str:
    ...
    '''
    # Implementation
    return json.dumps({"temperature": "72Â°F", "conditions": "Sunny"})
...
const getWeatherTool = betaZodTool({
  ...
  run: async (input) => {
    // Type-safe input!
    return JSON.stringify({temperature: '72Â°F'});
  }
});
...
tool_results = [
    {"type": "tool_result", "tool_use_id": "toolu_01", "content": "72Â°F, Sunny"},
    {"type": "tool_result", "tool_use_id": "toolu_02", "content": "45Â°F, Cloudy"},
