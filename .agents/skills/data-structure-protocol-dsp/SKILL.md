...
description: Giao thức cấu trúc dữ liệu codebase (DSP): Xây dựng bản đồ thực thể (Entity Graph), ranh giới phụ thuộc và lý do liên kết giúp AI định hướng siêu tốc.
---

# Data Structure Protocol (DSP)

LLM coding agents lose context between tasks. On large codebases they spend most of their tokens on "orientation" â€” figuring out where things live, what depends on what, and what is safe to change. DSP solves this by externalizing the project's structural map into a persistent, queryable graph stored in a `.dsp/` directory next to the code.
...
```
.dsp/
â”œâ”€â”€ TOC                        # ordered list of all entity UIDs from root
â”œâ”€â”€ obj-a1b2c3d4/
â”‚   â”œâ”€â”€ description            # source path, kind, purpose (1-3 sentences)
â”‚   â”œâ”€â”€ imports                # UIDs this entity depends on (one per line)
â”‚   â”œâ”€â”€ shared                 # UIDs of public API / exported entities
â”‚   â””â”€â”€ exports/               # reverse index: who imports this and why
â”‚       â”œâ”€â”€ <importer_uid>     # file content = "why" text
â”‚       â””â”€â”€ <shared_uid>/
â”‚           â”œâ”€â”€ description    # what is exported
â”‚           â””â”€â”€ <importer_uid> # why this specific export is imported
â””â”€â”€ func-7f3a9c12/
    â”œâ”€â”€ description
    â”œâ”€â”€ imports
    â””â”€â”€ exports/
...
Every file or artifact that is imported anywhere must be represented in `.dsp` as an Object â€” code, images, styles, configs, JSON, wasm, everything. External dependencies (npm packages, stdlib, etc.) are recorded as `kind: external` but their internals are never analyzed.
...
- **When creating a file/module**: Call `create-object`. For each exported function â€” `create-function` (with `--owner`). Register exports via `create-shared`.
- **When adding an import**: Call `add-import` with a brief `why`. For external deps â€” first `create-object --kind external` if the entity doesn't exist.
...
# Shows every module that imports lodash and WHY â€” lets you systematically replace it
```

## Best Practices

- âœ… **Do:** Update DSP immediately when creating new files, adding imports, or changing public APIs
- âœ… **Do:** Always add a meaningful `why` reason when recording an import â€” this is where most of DSP's value lives
- âœ… **Do:** Use `kind: external` for third-party libraries without analyzing their internals
- âœ… **Do:** Keep descriptions minimal (1-3 sentences about purpose, not implementation)
- âœ… **Do:** Treat `.dsp/` diffs like code diffs â€” review them, keep them accurate
- âŒ **Don't:** Touch `.dsp/` for internal-only changes that don't affect purpose or dependencies
- âŒ **Don't:** Change an entity's UID on rename/move (use `move-entity` instead)
- âŒ **Don't:** Create UIDs for every local variable or helper â€” only file-level Objects and public/shared entities

## Integration

This skill connects naturally to:
- **context-compression** â€” DSP reduces the need for compression by providing targeted retrieval instead of loading everything
- **context-optimization** â€” DSP is a structural optimization: agents pull minimal "context bundles" instead of raw source
- **architecture** â€” DSP captures architectural boundaries (imports/exports) that feed system design decisions
