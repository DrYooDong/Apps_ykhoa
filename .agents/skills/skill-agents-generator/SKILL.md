---
name: skill-agents-generator
description: Meta-Generator sinh tự động cấu hình AGENTS.md và Skills: Khởi tạo bộ rules và tài liệu kỹ thuật chuẩn theo stack công nghệ thực tế của dự án.
---
...
Generates a tailored AGENTS.md + `.agents/rules/*.md` for the target project â€” not a template with placeholders, but a living document that matches the project's real toolchain.
...
```
AGENTS.md
â”œâ”€â”€ Setup commands: bun install, bun dev, bun run test:run, bun doctor
â”œâ”€â”€ Verification Cycle: bunx tsc --noEmit â†’ bun run lint â†’ bun run test:run â†’ bun doctor
â”œâ”€â”€ Conventions: "Bun always. Plain TypeScript types + guards."
â””â”€â”€ Architecture â†’ .agents/rules/architecture.md

.agents/rules/
â”œâ”€â”€ architecture.md       â† ASCII diagram with real directories, exact versions
â”œâ”€â”€ frontend-patterns.md  â† Component rules, state locations, trust boundaries
â”œâ”€â”€ server-actions.md     â† downloadVideo() flow, DownloadResult type, rate limiter
â”œâ”€â”€ testing.md            â† "74 tests in 5 files", vitest commands, mock patterns
â”œâ”€â”€ git-workflow.md       â† Conventional commits, pre-commit checks
â””â”€â”€ sdd-workflow.md       â† Preflight defaults, post-apply verification
```
...
Generate AGENTS.md + `.agents/rules/*.md` for the target project. Never guess â€” read the project's actual files first.
...
- **Detect package manager FIRST.** Check lockfiles: `bun.lock`â†’bun, `pnpm-lock.yaml`â†’pnpm, `package-lock.json`â†’npm, `yarn.lock`â†’yarn. NEVER default to npm. Every command uses the detected PM.
...
1. `git rev-parse --show-toplevel` â†’ project root.
2. **Detect package manager FIRST**: check lockfiles. `bun.lock`â†’bun, `pnpm-lock.yaml`â†’pnpm, `package-lock.json`â†’npm, `yarn.lock`â†’yarn. Never default to npm.
...
1. Read `assets/agents-full.md` â€” this is the AGENTS.md structure with all sections and filling rules.
...
1. Read `assets/agents-minimal.md` â€” 30-line agents.md standard format.
...
| Conditional | `assets/claude.md` | CLAUDE.md â€” only if Claude detected |
