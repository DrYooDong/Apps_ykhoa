...
| `src/content/pathophysiology/giai-phau-sinh-ly.html` | Trang chuyển hướng tương thích (Redirect sang giai-phau-sinh-ly.html) | Cấp 2 |
...
| `src/content/basic-medical/` | **[MỚI]** Phân hệ Basic Medical Sciences (GP, Sinh lý, Cơ chế bệnh sinh, Hóa sinh Y học & Dịch tễ học Y khoa - 6 Khối, 2x2 Matrix Solver, Epicurve Studio) & Master Catalog `index.json` (bao gồm `dth-dengue.html` - Sốt xuất huyết, `dth-sot-ret.html` - Sốt rét, `dth-thuy-dau.html` - Thủy đậu, `dth-xo-gan.html` - Xơ gan, `dth-viem-mang-nao.html` - Viêm màng não, `dth-vgsv-b.html` - Viêm gan B, `dth-vgsv-c.html` - Viêm gan C, `dth-ckd.html` - Bệnh thận mạn, `slb-ccbs-dumping.html` - Hội chứng Dumping DS, `slb-ccbs-rung-nhi.html` - Rung nhĩ AF, `slb-ccbs-tang-ap-cua.html` - Tăng áp lực tĩnh mạch cửa PH, `slb-ccbs-co-truong.html` - Cổ trướng trong xơ gan Ascites, `slb-ccbs-benh-nao-gan.html` - Bệnh não gan HE, `slb-ccbs-ha-duong-huyet.html` - Hạ đường huyết trong ĐTĐ, `slb-ccbs-hhs.html` - Tăng áp lực thẩm thấu do ĐTĐ HHS, `slb-ccbs-dka.html` - Toan Ceton do ĐTĐ & euDKA, `slb-ccbs-rl-magie.html` - Rối loạn Magie máu, `slb-ccbs-rl-canxi.html` - Rối loạn Canxi máu, `slb-ccbs-rl-phosphat.html` - Rối loạn Phosphat máu, `slb-ccbs-rl-natri.html` - Rối loạn Natri máu, `slb-ccbs-rl-kali.html` - Rối loạn Kali máu, `slb-ccbs-hemophilia.html` - Bệnh Hemophilia ở trẻ em, `slb-ccbs-henpq.html` - Hen phế quản ở trẻ em, `slb-ccbs-vtpq.html` - Viêm tiểu phế quản cấp ở trẻ, `WORKFLOW_TAO_TRANG_SINH_LY_BENH.md`) |
...
| File | Vai trò |
|------|---------|
| `src/content/docspace/README.md` | **[MỚI]** Tài liệu hướng dẫn vận hành toàn diện & Kiến trúc DocSpace Clinical Workspace |
...
## 📚 src/content/knowledge-vault/ — Kho Kiến Thức Y Khoa EBM Hub & CRCE

| File | Vai trò |
|------|---------|
| `src/content/knowledge-vault/index.html` | Trang Hub Kho Kiến Thức Y Khoa Web Hub |
| `src/content/knowledge-vault/index.ts` | Điểm khởi tạo Hub & Dynamic Router cho URL Params |
