...
export function renderPhysioReader(slug: string): string {
  ...
  const backLink = isCcbs ? '#/basic-medical/co-che-benh-sinh' : '#/basic-medical/giai-phau-sinh-ly';
    ...
    <div class="physio-reader-container animate-fade-in" style="max-width: 1200px; margin: 0 auto; padding: 1.5rem 1rem;">
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; flex-wrap: wrap; gap: 1rem;">
        <div class="breadcrumb" style="font-size: 0.85rem; color: var(--color-text-muted, #64748b);">
          <a href="#/basic-medical" style="color: var(--color-primary, #0284c7); text-decoration: none;">Cơ Sở Y Khoa</a> &nbsp;/&nbsp;
  ...
  try {
    ...
    container.innerHTML = `
      <div style="text-align: center; padding: 2.5rem; color: #dc2626;">
        ...
        <a href="#/basic-medical" class="btn btn-primary" style="margin-top: 1rem; display: inline-block; padding: 0.5rem 1rem; background: var(--color-primary, #0284c7); color: #fff; border-radius: 6px; text-decoration: none;">Về Hub Cơ Sở Y Khoa</a>
      </div>
    `;
  }
}
