...
export function renderPhysioQuickNav(props?: PhysioQuickNavProps): string {
  ...
  const linksHtml = items
    .map(
      (item, idx) => {
        ...
        return `
        <a href="#${item.id}" class="pillar-tab physio-pillar-tab" style="
          display: inline-flex;
          align-items: center;
          gap: 0.45rem;
  ...
  return `
    <nav class="pillars-nav physio-quicknav" aria-label="Mục lục bài giảng sinh lý học" style="
      position: sticky;
      top: 60px;
      z-index: 90;
      background: var(--color-surface, #ffffff);
    ...
    ">
      <div style="font-size: 0.75rem; font-weight: 800; text-transform: uppercase; color: var(--color-text-muted, #64748b); letter-spacing: 0.05em; margin-bottom: 0.5rem; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 0.5rem;">
        <span style="display: flex; align-items: center; gap: 0.4rem; color: var(--color-primary, #0284c7);">
          ...
          <span>ĐIỀU HƯỚNG NHANH SINH LÝ HỌC • GUYTON & GANONG</span>
        </span>
        <span class="badge" style="background: rgba(2,132,199,0.1); color: var(--color-primary, #0284c7); font-size: 0.7rem; font-weight: 700; padding: 2px 8px; border-radius: 999px;">Guyton 14th & Ganong 26th</span>
      </div>
