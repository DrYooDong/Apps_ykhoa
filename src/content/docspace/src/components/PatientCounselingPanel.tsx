...
export const PatientCounselingPanel: React.FC<PatientCounselingProps> = ({
  ...
  return (
    <div id="sub-patient-education" className="scroll-mt-24 bg-white border border-blue-200 rounded-xl p-4 shadow-2xs flex flex-col gap-3">
      <div className="flex items-center justify-between gap-2 flex-wrap pb-2 border-b border-blue-100">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h4 className="font-display font-bold text-xs sm:text-sm text-slate-900 flex items-center gap-2 flex-wrap">
              <span>Tài Liệu Tư Vấn & Dặn Dò Người Bệnh (Kho TV)</span>
              <span className="px-2 py-0.5 text-[10px] font-mono bg-blue-100 text-blue-800 rounded font-semibold border border-blue-200">
                Kho TV
              </span>
            </h4>
        ...
        <div className="flex items-center gap-2">
          ...
          {matchedTvArticle.id && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center gap-1 px-2.5 py-1 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded shadow-2xs transition-colors"
            >
        ...
        {matchedTvArticle.tags && matchedTvArticle.tags.length > 0 && (
          <div className="flex items-center gap-1.5 flex-wrap">
            {matchedTvArticle.tags.map((tag, idx) => (
              <span
                key={idx}
                className="px-2 py-0.5 bg-white text-blue-800 border border-blue-200 rounded text-[10.5px] font-medium"
              >
                #{tag}
              </span>
            ))}
