/**
 * CliniPortal Physiology - Pathway & Mechanism Viewer (TypeScript)
...
export class PhysioPathwayViewer {
  public static async init(containerId: string, pathwayId: string, jsonPath: string = 'data/physio-pathways.json'): Promise<void> {
    ...
    try {
      ...
      const pathway = (data.pathways || []).find((p: PathwayItem) => p.id === pathwayId);
      if (pathway) this.renderPathway(container, pathway);
    } catch (err) {
      console.error('PhysioPathwayViewer fetch error:', err);
    }
  }

  public static renderPathway(container: HTMLElement, pathway: PathwayItem): void {
    let html = `
...
if (typeof window !== 'undefined') {
  (window as any).PhysioPathwayViewer = PhysioPathwayViewer;
}
