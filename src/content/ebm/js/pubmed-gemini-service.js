/**
 * pubmed-gemini-service.js
 * Quản lý lấy danh sách Guidelines y khoa mới nhất trên PubMed (NCBI E-utilities)
 * và xử lý, chuẩn hóa thông tin (kèm Chuyên khoa) bằng Google Gemini Direct Client-side API.
 * Hỗ trợ tự động dò tìm model (gemini-2.0-flash, gemini-1.5-flash-latest, gemini-1.5-flash...)
 * 
 * CliniPortal - Hệ sinh thái Web Y khoa
 */

(function () {
  'use strict';

  const STORAGE_KEY_API = 'CLINI_GEMINI_KEY';
  const STORAGE_KEY_CACHE = 'yhcc_pubmed_guidelines_cache_v2';
  const CACHE_TTL_MS = 12 * 60 * 60 * 1000; // 12 hours cache

  const CANDIDATE_MODELS = [
    'gemini-3.7-flash',
    'gemini-3.6-flash',
    'gemini-3.5-flash',
    'gemini-2.5-flash-lite',
    'gemini-flash-latest',
    'gemini-2.0-flash',
    'gemini-1.5-flash-latest',
    'gemini-1.5-flash'
  ];

  let detectedWorkingModel = null;

  // State
  let currentGuidelines = [];
  let isGeminiActive = false;

  // DOM Elements
  let containerEl, listEl, syncBtnEl, keyBtnEl, badgeStatusEl;
  let modalEl, modalInputEl, modalSaveBtn, modalCancelBtn, modalClearBtn, modalTestBtn, modalToggleEyeBtn, modalStatusEl;

  document.addEventListener('DOMContentLoaded', () => {
    initDOMElements();
    bindEvents();
    loadAndRenderGuidelines();
  });

  function initDOMElements() {
    containerEl = document.getElementById('pubmed-board-container');
    listEl = document.getElementById('pubmed-guidelines-list');
    syncBtnEl = document.getElementById('btn-pubmed-sync');
    keyBtnEl = document.getElementById('btn-gemini-config');
    badgeStatusEl = document.getElementById('pubmed-status-badge');

    modalEl = document.getElementById('gemini-key-modal');
    modalInputEl = document.getElementById('gemini-api-key-input');
    modalSaveBtn = document.getElementById('btn-gemini-key-save');
    modalCancelBtn = document.getElementById('btn-gemini-key-cancel');
    modalClearBtn = document.getElementById('btn-gemini-key-clear');
    modalTestBtn = document.getElementById('btn-gemini-key-test');
    modalToggleEyeBtn = document.getElementById('btn-toggle-key-visibility');
    modalStatusEl = document.getElementById('gemini-key-status-text');
  }

  function bindEvents() {
    if (syncBtnEl) {
      syncBtnEl.addEventListener('click', () => {
        loadAndRenderGuidelines(true); // force refresh
      });
    }

    if (keyBtnEl) {
      keyBtnEl.addEventListener('click', openGeminiModal);
    }

    if (modalSaveBtn) {
      modalSaveBtn.addEventListener('click', handleSaveKey);
    }

    if (modalCancelBtn) {
      modalCancelBtn.addEventListener('click', closeGeminiModal);
    }

    if (modalClearBtn) {
      modalClearBtn.addEventListener('click', handleClearKey);
    }

    if (modalTestBtn) {
      modalTestBtn.addEventListener('click', handleTestKey);
    }

    if (modalToggleEyeBtn && modalInputEl) {
      modalToggleEyeBtn.addEventListener('click', () => {
        const isPass = modalInputEl.type === 'password';
        modalInputEl.type = isPass ? 'text' : 'password';
        modalToggleEyeBtn.innerHTML = isPass ? '<i class="fa-regular fa-eye-slash"></i>' : '<i class="fa-regular fa-eye"></i>';
      });
    }

    if (modalEl) {
      modalEl.addEventListener('click', (e) => {
        if (e.target === modalEl) closeGeminiModal();
      });
    }
  }

  // --- 1. MODAL CẤU HÌNH & TEST GEMINI KEY ---
  function openGeminiModal() {
    if (!modalEl) return;
    const existingKey = localStorage.getItem(STORAGE_KEY_API) || '';
    if (modalInputEl) {
      modalInputEl.value = existingKey;
      modalInputEl.type = 'password';
      if (modalToggleEyeBtn) modalToggleEyeBtn.innerHTML = '<i class="fa-regular fa-eye"></i>';
    }
    if (modalStatusEl) {
      if (existingKey) {
        modalStatusEl.innerHTML = '<span style="color: #10b981; font-weight: 700;">✓ Đã lưu Key trong trình duyệt. Nhấn "Kiểm tra Key" để kiểm tra kết nối với Google AI.</span>';
      } else {
        modalStatusEl.innerHTML = '<span style="color: var(--hub-text-muted);">Chưa cài đặt Key (Đang chạy ở chế độ PubMed trực tiếp không AI).</span>';
      }
    }
    modalEl.classList.add('active');
  }

  function closeGeminiModal() {
    if (modalEl) modalEl.classList.remove('active');
  }

  // Auto-discover available models from API
  async function discoverAvailableModel(key) {
    if (detectedWorkingModel) return detectedWorkingModel;
    try {
      const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${encodeURIComponent(key)}`, {
        headers: { 'x-goog-api-key': key }
      });
      if (res.ok) {
        const data = await res.json();
        const availableNames = (data.models || []).map(m => m.name.replace('models/', ''));
        for (const candidate of CANDIDATE_MODELS) {
          if (availableNames.includes(candidate)) {
            detectedWorkingModel = candidate;
            return candidate;
          }
        }
        const anyGenerate = (data.models || []).find(m =>
          Array.isArray(m.supportedGenerationMethods) &&
          m.supportedGenerationMethods.includes('generateContent') &&
          !m.name.includes('embedding')
        );
        if (anyGenerate) {
          detectedWorkingModel = anyGenerate.name.replace('models/', '');
          return detectedWorkingModel;
        }
      }
    } catch (e) {
      console.warn('Auto-discovery failed, using default candidate list', e);
    }
    return 'gemini-2.0-flash';
  }

  // Robust multi-model runner
  async function callGeminiGenerateContent(prompt, key) {
    const initialModel = await discoverAvailableModel(key);
    const modelsToTry = [initialModel, ...CANDIDATE_MODELS.filter(m => m !== initialModel)];

    let lastError = null;
    for (const model of modelsToTry) {
      try {
        const endpoint = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${encodeURIComponent(key)}`;
        const payload = {
          contents: [{ role: 'user', parts: [{ text: prompt }] }],
          generationConfig: {
            temperature: 0.1,
            maxOutputTokens: 2048
          }
        };

        const startTime = Date.now();
        const res = await fetch(endpoint, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'x-goog-api-key': key
          },
          body: JSON.stringify(payload)
        });
        const latency = Date.now() - startTime;

        if (res.ok) {
          detectedWorkingModel = model;
          const resJson = await res.json();
          return { resJson, model, latency };
        } else {
          const errText = await res.text();
          let parsedMsg = errText;
          try {
            const errObj = JSON.parse(errText);
            if (errObj.error?.message) parsedMsg = errObj.error.message;
          } catch (e) {}

          lastError = new Error(`[${model}] ${parsedMsg}`);
          
          if (res.status === 400 && (parsedMsg.includes('API_KEY_INVALID') || parsedMsg.includes('API key not valid'))) {
            throw new Error('Key không chính xác (API_KEY_INVALID). Vui lòng kiểm tra lại Key từ Google AI Studio.');
          }
          if (res.status === 404 || parsedMsg.includes('not found') || parsedMsg.includes('not supported')) {
            continue; // Thử tiếp model khác trong danh sách
          }
        }
      } catch (err) {
        lastError = err;
        if (err.message.includes('API_KEY_INVALID')) throw err;
      }
    }

    throw lastError || new Error('Không thể kết nối với model Gemini phù hợp.');
  }

  async function handleTestKey() {
    if (!modalInputEl || !modalStatusEl) return;
    const key = modalInputEl.value.trim();
    if (!key) {
      modalStatusEl.innerHTML = '<span style="color: #ef4444; font-weight: 700;">⚠️ Vui lòng dán API Key vào ô trên trước khi kiểm tra!</span>';
      return;
    }

    modalStatusEl.innerHTML = '<span style="color: var(--hub-blue); font-weight: 600;"><i class="fa-solid fa-spinner fa-spin"></i> Đang tự động kết nối và kiểm tra API Key với Google AI...</span>';
    if (modalTestBtn) modalTestBtn.disabled = true;

    try {
      const result = await callGeminiGenerateContent('Respond with OK only', key);
      modalStatusEl.innerHTML = `<span style="color: #10b981; font-weight: 700;"><i class="fa-solid fa-circle-check"></i> Key hợp lệ 100%! Đã kết nối thành công model <code>${result.model}</code> (${result.latency}ms). Bạn có thể bấm "Lưu & Kích hoạt AI" ngay.</span>`;
    } catch (err) {
      console.error('Test API Key Error:', err);
      modalStatusEl.innerHTML = `<span style="color: #ef4444; font-weight: 700;"><i class="fa-solid fa-triangle-exclamation"></i> Lỗi kết nối: ${err.message}</span>`;
    } finally {
      if (modalTestBtn) modalTestBtn.disabled = false;
    }
  }

  function handleSaveKey() {
    if (!modalInputEl) return;
    const key = modalInputEl.value.trim();
    if (!key) {
      alert('Vui lòng nhập API Key hợp lệ hoặc nhấn Xóa Key nếu không sử dụng.');
      return;
    }
    localStorage.setItem(STORAGE_KEY_API, key);
    closeGeminiModal();
    localStorage.removeItem(STORAGE_KEY_CACHE);
    loadAndRenderGuidelines(true);
  }

  function handleClearKey() {
    if (confirm('Bạn có chắc chắn muốn xóa Gemini API Key khỏi trình duyệt này?')) {
      localStorage.removeItem(STORAGE_KEY_API);
      localStorage.removeItem(STORAGE_KEY_CACHE);
      if (modalInputEl) modalInputEl.value = '';
      closeGeminiModal();
      loadAndRenderGuidelines(true);
    }
  }

  // --- 2. QUERY NCBI PUBMED E-UTILITIES API ---
  async function fetchPubMedRawGuidelines() {
    const currentYear = new Date().getFullYear();
    const prevYear = currentYear - 1;
    const term = encodeURIComponent(`(Practice Guideline[pt] OR Guideline[pt]) AND ("${prevYear}"[dp] : "${currentYear}"[dp])`);
    const searchUrl = `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=${term}&sort=pub_date&retmode=json&retmax=8`;

    const searchRes = await fetch(searchUrl);
    if (!searchRes.ok) throw new Error(`PubMed Search Error: ${searchRes.statusText}`);
    const searchData = await searchRes.json();
    const idList = searchData.esearchresult?.idlist || [];

    if (idList.length === 0) return [];

    const summaryUrl = `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=pubmed&id=${idList.join(',')}&retmode=json`;
    const summaryRes = await fetch(summaryUrl);
    if (!summaryRes.ok) throw new Error(`PubMed Summary Error: ${summaryRes.statusText}`);
    const summaryData = await summaryRes.json();
    const resultObj = summaryData.result || {};

    const rawList = idList.map(pmid => {
      const item = resultObj[pmid] || {};
      return {
        pmid: pmid,
        title: item.title ? item.title.replace(/\[|\]|\.$/g, '') : 'Hướng dẫn thực hành lâm sàng',
        source: item.source || 'PubMed Central',
        pubdate: item.pubdate || '',
        sortpubdate: item.sortpubdate || '',
        authors: (item.authors && item.authors.length > 0) ? item.authors[0].name : '',
        url: `https://pubmed.ncbi.nlm.nih.gov/${pmid}/`
      };
    });

    return rawList;
  }

  // Helper: Đoán Chuyên khoa từ từ khóa khi không có AI
  function inferSpecialtyFromText(text) {
    const lower = (text || '').toLowerCase();
    if (/cardio|heart|hypertension|infarction|arrhythmia|vascular|coronary|aortic/i.test(lower)) return 'Tim Mạch';
    if (/pulmon|respirat|asthma|copd|lung|pneumonia|bronch/i.test(lower)) return 'Hô Hấp';
    if (/gastro|digest|liver|hepat|colon|bowel|gerd|pancrea/i.test(lower)) return 'Tiêu Hóa';
    if (/diabet|endocrin|thyroid|metabol|glycem|hormon/i.test(lower)) return 'Nội Tiết';
    if (/sepsis|critical|intensive|icu|shock|emergency|resuscitat|trauma/i.test(lower)) return 'Cấp Cứu - ICU';
    if (/pediatr|child|infant|neonat|adolescent/i.test(lower)) return 'Nhi Khoa';
    if (/neuro|stroke|brain|epilep|dementia|parkinson/i.test(lower)) return 'Thần Kinh';
    if (/infect|virus|bacteri|antimicrob|hiv|covid|vaccin|septic/i.test(lower)) return 'Truyền Nhiễm';
    if (/oncol|cancer|tumor|carcinoma|chemotherap/i.test(lower)) return 'Ung Bướu';
    if (/renai|kidney|nephro|dialysis/i.test(lower)) return 'Thận - Tiết Niệu';
    if (/rheum|arthrit|lupus|autoimmun/i.test(lower)) return 'Cơ Xương Khớp';
    return 'Đa Khoa';
  }

  // --- 3. FORMAT RAW PUBMED (FALLBACK CHẾ ĐỘ THƯỜNG KHI KHÔNG CÓ GEMINI) ---
  function formatRawPubMedData(rawList) {
    return rawList.map(item => {
      let formattedDate = 'Gần đây';
      if (item.sortpubdate) {
        const parts = item.sortpubdate.split(' ')[0].split('/');
        if (parts.length === 3) {
          formattedDate = `${parts[2].padStart(2, '0')}/${parts[1].padStart(2, '0')}/${parts[0]}`;
        }
      } else if (item.pubdate) {
        formattedDate = item.pubdate;
      }

      const inferredSpec = inferSpecialtyFromText(`${item.title} ${item.source}`);

      return {
        pmid: item.pmid,
        title: item.title,
        specialty: inferredSpec,
        organization: item.source || 'Tổ chức Y khoa Quốc tế',
        releaseDate: formattedDate,
        url: item.url
      };
    });
  }

  // --- 4. GEMINI DIRECT CLIENT-SIDE CALL (OPTION A) ---
  async function processGuidelinesWithGemini(rawList, apiKey) {
    const prompt = `Bạn là Trợ lý AI Y học chứng cứ (EBM AI) của CliniPortal.
Nhiệm vụ: Phân tích danh sách kết quả thô các Hướng dẫn điều trị lâm sàng (Practice Guidelines) mới nhất từ PubMed dưới đây.

Hãy trích xuất và chuẩn hóa 4 thông tin cho mỗi bài báo:
1. title: Tên bài viết/Hướng dẫn điều trị (Dịch tên sang Tiếng Việt chuẩn y khoa, giữ nguyên tên viết tắt của thuốc/bệnh/guideline).
2. specialty: Tên Chuyên khoa tương ứng (ví dụ: Tim Mạch, Hô Hấp, Tiêu Hóa, Nội Tiết, Cấp Cứu - ICU, Thần Kinh, Nhi Khoa, Truyền Nhiễm, Ung Bướu, Thận - Tiết Niệu, Cơ Xương Khớp, hoặc Đa Khoa).
3. organization: Tên Tổ chức/Hiệp hội hoặc Tạp chí y khoa uy tín phát hành (ví dụ: AHA, ESC, ADA, WHO, CHEST, IDSA, KDIGO, JAMA, The Lancet, BMJ, Bộ Y Tế...).
4. releaseDate: Ngày tháng năm phát hành theo đúng định dạng chuẩn DD/MM/YYYY (ví dụ: 14/02/2026). Nếu chỉ có năm/tháng thì để DD là 01 (ví dụ 01/01/2026).

Dữ liệu đầu vào từ PubMed:
${JSON.stringify(rawList, null, 2)}

YÊU CẦU ĐẦU RA:
Chỉ trả về DUY NHẤT 01 mảng JSON thuần túy (JSON Array), KHÔNG kèm bất kỳ lời dẫn giải, mở đầu hay markdown nào ngoài JSON:
[
  {
    "pmid": "mã pmid",
    "title": "Tên Guideline tiếng Việt chuẩn y khoa",
    "specialty": "Tên chuyên khoa",
    "organization": "Tên tổ chức/Hiệp hội/Tạp chí",
    "releaseDate": "DD/MM/YYYY",
    "url": "https://pubmed.ncbi.nlm.nih.gov/pmid/"
  }
]`;

    const { resJson, model } = await callGeminiGenerateContent(prompt, apiKey);
    const rawAiOutput = resJson.candidates?.[0]?.content?.parts?.[0]?.text || '';

    const cleanedJson = rawAiOutput.replace(/^```json/m, '').replace(/^```/m, '').replace(/```$/m, '').trim();
    const parsedData = JSON.parse(cleanedJson);

    return { data: parsedData, model };
  }

  // --- 5. TỔNG HỢP & HIỂN THỊ DỮ LIỆU ---
  async function loadAndRenderGuidelines(forceRefresh = false) {
    if (!listEl) return;

    const apiKey = localStorage.getItem(STORAGE_KEY_API);
    isGeminiActive = Boolean(apiKey);

    if (badgeStatusEl) {
      if (isGeminiActive) {
        badgeStatusEl.className = 'pubmed-board-badge badge-gemini-active';
        badgeStatusEl.innerHTML = '<i class="fa-solid fa-bolt"></i> Gemini AI Active';
      } else {
        badgeStatusEl.className = 'pubmed-board-badge badge-pubmed-direct';
        badgeStatusEl.innerHTML = '<i class="fa-solid fa-globe"></i> PubMed Direct';
      }
    }

    if (!forceRefresh) {
      const cachedStr = localStorage.getItem(STORAGE_KEY_CACHE);
      if (cachedStr) {
        try {
          const cached = JSON.parse(cachedStr);
          if (Date.now() - cached.timestamp < CACHE_TTL_MS && Array.isArray(cached.data) && cached.data.length > 0) {
            currentGuidelines = cached.data;
            renderList(currentGuidelines);
            return;
          }
        } catch (e) {
          console.warn('Invalid cache format, refreshing...', e);
        }
      }
    }

    listEl.innerHTML = `
      <div class="pubmed-loading-skeleton">
        <div class="pubmed-spinner"></div>
        <div>Đang truy vấn PubMed & ${isGeminiActive ? 'chuẩn hóa chuyên khoa với Gemini AI' : 'tải danh sách Guidelines mới nhất'}...</div>
      </div>
    `;

    try {
      const rawList = await fetchPubMedRawGuidelines();
      if (!rawList || rawList.length === 0) {
        listEl.innerHTML = `<div class="pubmed-loading-skeleton">Không tìm thấy Guidelines mới phù hợp từ PubMed trong thời gian gần đây.</div>`;
        return;
      }

      let finalGuidelines = [];

      if (isGeminiActive) {
        try {
          const result = await processGuidelinesWithGemini(rawList, apiKey);
          finalGuidelines = result.data;
          if (badgeStatusEl) {
            badgeStatusEl.className = 'pubmed-board-badge badge-gemini-active';
            badgeStatusEl.innerHTML = `<i class="fa-solid fa-bolt"></i> Gemini AI Active (${result.model})`;
          }
        } catch (geminiError) {
          console.error('Lỗi khi gọi Gemini API, chuyển sang chế độ dự phòng PubMed:', geminiError);
          finalGuidelines = formatRawPubMedData(rawList);
          if (badgeStatusEl) {
            badgeStatusEl.className = 'pubmed-board-badge';
            badgeStatusEl.style.background = 'rgba(239, 68, 68, 0.12)';
            badgeStatusEl.style.color = '#ef4444';
            badgeStatusEl.style.border = '1px solid rgba(239, 68, 68, 0.3)';
            badgeStatusEl.innerHTML = '<i class="fa-solid fa-triangle-exclamation"></i> Lỗi Gemini Key (Dùng PubMed)';
          }
        }
      } else {
        finalGuidelines = formatRawPubMedData(rawList);
      }

      currentGuidelines = finalGuidelines;

      localStorage.setItem(STORAGE_KEY_CACHE, JSON.stringify({
        timestamp: Date.now(),
        data: finalGuidelines
      }));

      renderList(finalGuidelines);

    } catch (err) {
      console.error('Lỗi tải PubMed Guidelines:', err);
      listEl.innerHTML = `
        <div class="pubmed-loading-skeleton" style="color: var(--hub-red, #ef4444);">
          <i class="fa-solid fa-circle-exclamation" style="font-size: 1.2rem; margin-bottom: 0.4rem; display: block;"></i>
          Không thể kết nối với máy chủ PubMed hoặc Gemini AI. Vui lòng kiểm tra kết nối mạng và thử lại.
        </div>
      `;
    }
  }

  // Specialty color/icon helper
  function getSpecialtyBadgeStyle(spec) {
    const s = (spec || '').toLowerCase();
    if (s.includes('tim')) return { icon: '🫀', bg: 'rgba(239, 68, 68, 0.12)', color: '#ef4444' };
    if (s.includes('hô hấp') || s.includes('phổi')) return { icon: '🫁', bg: 'rgba(6, 182, 212, 0.12)', color: '#06b6d4' };
    if (s.includes('nội tiết') || s.includes('đường')) return { icon: '🩺', bg: 'rgba(245, 158, 11, 0.12)', color: '#f59e0b' };
    if (s.includes('tiêu hóa') || s.includes('gan')) return { icon: '🧫', bg: 'rgba(16, 185, 129, 0.12)', color: '#10b981' };
    if (s.includes('cấp cứu') || s.includes('icu')) return { icon: '⚡', bg: 'rgba(124, 58, 237, 0.12)', color: '#7c3aed' };
    if (s.includes('nhi')) return { icon: '👶', bg: 'rgba(236, 72, 153, 0.12)', color: '#ec4899' };
    if (s.includes('thần kinh') || s.includes('não')) return { icon: '🧠', bg: 'rgba(99, 102, 241, 0.12)', color: '#6366f1' };
    if (s.includes('truyền nhiễm') || s.includes('nhiễm')) return { icon: '🛡️', bg: 'rgba(20, 184, 166, 0.12)', color: '#14b8a6' };
    if (s.includes('ung bướu')) return { icon: '🎗️', bg: 'rgba(225, 29, 72, 0.12)', color: '#e11d48' };
    if (s.includes('thận')) return { icon: '💧', bg: 'rgba(3, 105, 161, 0.12)', color: '#0369a1' };
    return { icon: '📋', bg: 'rgba(100, 116, 139, 0.12)', color: '#64748b' };
  }

  // Render HTML cho từng Guideline Item
  function renderList(guidelines) {
    if (!listEl) return;
    if (!guidelines || guidelines.length === 0) {
      listEl.innerHTML = `<div class="pubmed-loading-skeleton">Chưa có dữ liệu Guidelines.</div>`;
      return;
    }

    let html = '';
    guidelines.forEach(item => {
      const pmidUrl = item.url || (item.pmid ? `https://pubmed.ncbi.nlm.nih.gov/${item.pmid}/` : '#');
      const title = item.title || 'Hướng dẫn thực hành lâm sàng';
      const spec = item.specialty || 'Đa Khoa';
      const org = item.organization || 'Tổ chức Y khoa Quốc tế';
      const date = item.releaseDate || '2026';

      const specStyle = getSpecialtyBadgeStyle(spec);

      html += `
        <a href="${pmidUrl}" target="_blank" rel="noopener noreferrer" class="pubmed-guideline-card" title="Xem chi tiết toàn văn trên PubMed (${item.pmid || ''})">
          <div class="guideline-main-col">
            <div class="guideline-title-text">${title}</div>
            <div class="guideline-meta-sub">
              <span class="guideline-spec-pill" style="background: ${specStyle.bg}; color: ${specStyle.color};">
                <span>${specStyle.icon}</span> ${spec}
              </span>
              <span class="guideline-org-pill">
                <i class="fa-solid fa-building-columns"></i> ${org}
              </span>
              <span class="guideline-date-badge">
                <i class="fa-regular fa-calendar"></i> ${date}
              </span>
            </div>
          </div>
          <div>
            <span class="guideline-action-btn">
              PubMed <i class="fa-solid fa-arrow-up-right-from-square"></i>
            </span>
          </div>
        </a>
      `;
    });

    listEl.innerHTML = html;
  }

})();
