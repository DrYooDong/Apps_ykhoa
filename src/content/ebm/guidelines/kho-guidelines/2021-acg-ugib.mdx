  ...
  <div class="page-content">
    ...
    <section class="sec-card" id="sec-3">
      ...
      <div class="sec-body">
        ...
        <FlowchartViewer 
          title="Lưu Đồ Xử Trí Tiếp Cận UGIB Từ Cấp Cứu Đến Sau Nội Soi (ACG 2021)"
          code={`flowchart TD
            START([Tiếp nhận bệnh nhân Xuất huyết tiêu hóa trên (UGIB) | start | badge: Cấp cứu ban đầu | subtitle: Nôn ra máu hoặc đi cầu phân đen])
            GBS{Đánh giá phân tầng nguy cơ qua thang điểm Glasgow-Blatchford (GBS)? | question}

            LOW_RISK[GBS = 0 - 1: NGUY CƠ CỰC THẤP (≤ 1%) 🟢 | success | badge: An toàn ngoại trú | subtitle: Tỷ lệ can thiệp nội soi hoặc tử vong cực thấp - Xuất viện sớm an toàn & theo dõi ngoại trú]
            HIGH_RISK[GBS ≥ 2: CẦN NHẬP VIỆN & HỒI SỨC TÍCH CỰC 🚨 | danger | badge: Nguy cơ cao | subtitle: Đặt 2 đường truyền lớn, theo dõi monitor, duy trì MAP ≥ 65 mmHg]

            RESUS[Hồi sức tiền nội soi chuẩn EBM ACG 2021 💉 | action | subtitle: 1. Truyền máu hạn chế khi Hb < 7 g/dL (Đích 7-9 g/dL) + 2. Erythromycin 250mg IV trước soi 30-90p + 3. Xem xét PPI tiền nội soi]

            ENDO_TIME{Thời điểm can thiệp nội soi tiêu hóa trên? | question}
            ENDO_OPT[NỘI SOI TRONG VÒNG 24 GIỜ (KHI ĐÃ ỔN ĐỊNH HUYẾT ĐỘNG) ⭐ | action | badge: ACG Khuyến cáo Mạnh | subtitle: KHÔNG khuyến cáo soi quá sớm < 6 giờ khi chưa hồi sức ổn định]

            FORREST{Phân loại tổn thương ổ loét theo Forrest? | question}
            FORREST_LOW[Nguy cơ thấp: Đáy sạch (FIII), Vết sắc tố (FIIc) 🟢 | success | subtitle: Không cần can thiệp nội soi cầm máu - Dùng PPI uống chuẩn & xuất viện sớm]
            FORREST_HIGH[Nguy cơ cao: Phun tia (FIa), Rỉ máu (FIb), Mạch lộ (FIIa), Cục máu dính (FIIb) 🚨 | danger | badge: Bắt buộc can thiệp | subtitle: Kẹp Hemoclip hoặc Liệu pháp nhiệt ± Tiêm Epinephrine + PPI liều cao liên tục trong 72 giờ]

            START --> GBS
            GBS -->|GBS 0-1| LOW_RISK
            GBS ==>|GBS ≥ 2| HIGH_RISK
            HIGH_RISK --> RESUS
            RESUS --> ENDO_TIME
            ENDO_TIME -->|Ổn định huyết động| ENDO_OPT
            ENDO_OPT --> FORREST
            FORREST -->|Forrest IIc, III| FORREST_LOW
            FORREST ==>|Forrest Ia, Ib, IIa, IIb| FORREST_HIGH
          `}
        />
