---
title: "IDSA 2017: Hướng Dẫn Thực Hành Lâm Sàng Chẩn Đoán & Quản Lý Tiêu Chảy Nhiễm Trùng"
slug: "2017-idsa-infectious-diarrhea"
code: "GDL-2017-IDSA-DIARRHEA"
organization: "IDSA"
year: "2017"
category: "guidelines"
status: "published"
version: "3.0.0"
updatedAt: "2026-09-02"
cor: "I"
loe: "A"
description: "Tóm tắt toàn diện Hướng dẫn Lâm sàng IDSA 2017 về Tiêu chảy Nhiễm trùng: Tiếp cận dịch tễ và biểu hiện lâm sàng, chỉ định xét nghiệm phân/cấy máu/NAAT, phác đồ bù dịch ORS độ thẩm thấu giảm, nguyên tắc kháng sinh kinh nghiệm và trúng đích, cảnh báo chống chỉ định tuyệt đối kháng sinh trong STEC (ngừa HUS), chống chỉ định Loperamide ở trẻ em/tiêu chảy máu và khung đánh giá chứng cứ GRADE."
tags:
  - "IDSA"
  - "2017"
  - "Tiêu chảy nhiễm trùng"
  - "Infectious Diarrhea"
  - "STEC"
  - "Shiga Toxin"
  - "HUS"
  - "ORS"
  - "Loperamide"
  - "Khuyến cáo lâm sàng"
  - "Evidence-Based Medicine"
keyRecommendations:
  - "Liệu pháp bù dịch: Dung dịch bù dịch đường uống độ thẩm thấu giảm (Reduced Osmolarity ORS) là lựa chọn đầu tay cho mất nước nhẹ - trung bình ở mọi lứa tuổi; duy trì liên tục bú mẹ và ăn sớm."
  - "Cảnh báo an toàn STEC / HUS: Chống chỉ định tuyệt đối điều trị kháng sinh ở bệnh nhân nghi ngờ hoặc xác định nhiễm STEC O157 / STEC sinh Shiga toxin 2 để ngăn ngừa giải phóng ồ ạt độc tố gây hội chứng tán huyết tăng ure máu (HUS)."
  - "Hạn chế kháng sinh kinh nghiệm: Không dùng kháng sinh kinh nghiệm thường quy cho tiêu chảy máu ở người miễn dịch bình thường; chỉ định chọn lọc cho trẻ <3 tháng, hội chứng lỵ trực trùng nặng nghi do Shigella hoặc du lịch kèm sốt ≥ 38.5°C."
  - "An toàn thuốc hỗ trợ: Chống chỉ định tuyệt đối thuốc giảm nhu động ruột (Loperamide) cho trẻ em <18 tuổi và mọi bệnh nhân tiêu chảy phân máu hoặc có sốt cao (nguy cơ phình đại tràng nhiễm độc)."
sections:
  - id: "sec-phan-loai-dich-te"
    number: 1
    title: "Phân Loại Thời Gian & Khai Thác Yếu Tố Dịch Tễ Phơi Nhiễm"
    icon: "fa-solid fa-clock"
  - id: "sec-lam-sang-luu-do"
    number: 2
    title: "Biểu Hiện Lâm Sàng & Lưu Đồ Đánh Giá Toàn Diện (Figure 1)"
    icon: "fa-solid fa-stethoscope"
  - id: "sec-chi-dinh-xet-nghiem"
    number: 3
    title: "Chỉ Định Xét Nghiệm Phân, Cấy Máu & Bảng Đa Mồi NAAT"
    icon: "fa-solid fa-microscope"
  - id: "sec-bu-dich-dinh-duong"
    number: 4
    title: "Liệu Pháp Bù Dịch ORS & Chế Độ Dinh Dưỡng Theo Lứa Tuổi"
    icon: "fa-solid fa-glass-water"
  - id: "sec-khang-sinh-stec"
    number: 5
    title: "Kháng Sinh Kinh Nghiệm, Trúng Đích & Cảnh Báo Tuyệt Đối STEC/HUS"
    icon: "fa-solid fa-capsules"
  - id: "sec-thuoc-ho-tro-du-phong-grade"
    number: 6
    title: "Liệu Pháp Hỗ Trợ, Dự Phòng Vắc-xin & Phương Pháp GRADE (Figure 2)"
    icon: "fa-solid fa-shield-virus"
  - id: "sec-trich-dan-y-van"
    number: 7
    title: "Tài Liệu Tham Khảo Chuẩn AMA & Bảng Viết Tắt Y Khoa"
    icon: "fa-solid fa-book-medical"
---

<div class="stats-strip">
  <div class="stats-grid">
    <div class="stat-card">
      <div class="stat-val green">Reduced ORS</div>
      <div class="stat-lbl">Lựa chọn đầu tay bù dịch đường uống độ thẩm thấu giảm ở mọi lứa tuổi</div>
    </div>
    <div class="stat-card">
      <div class="stat-val red">CẤM Kháng Sinh STEC</div>
      <div class="stat-lbl">Tránh kích hoạt bacteriophage giải phóng Shiga toxin gây hội chứng HUS</div>
    </div>
    <div class="stat-card">
      <div class="stat-val amber">CẤM Loperamide &lt; 18T</div>
      <div class="stat-lbl">Chống chỉ định ở trẻ em, tiêu chảy phân máu hoặc có sốt cao</div>
    </div>
    <div class="stat-card">
      <div class="stat-val blue">Khung Chuẩn GRADE</div>
      <div class="stat-lbl">Phân cấp chất lượng bằng chứng và xác định mức độ khuyến cáo rõ ràng</div>
    </div>
  </div>
</div>

<div class="pillars">
  <div class="pillars-inner">
    <div class="pillar p1">
      <div class="pillar-icon">💧</div>
      <div>
        <div class="pillar-title">Bù Dịch &amp; Dinh Dưỡng Sớm (Rehydration First)</div>
        <div class="pillar-desc">ORS độ thẩm thấu giảm là nền tảng cốt lõi cứu mạng; tiếp tục duy trì bú mẹ liên tục và tái lập chế độ ăn bình thường ngay sau khi bù đủ dịch, tránh kiêng khem quá mức.</div>
      </div>
    </div>
    <div class="pillar p2">
      <div class="pillar-icon">🔬</div>
      <div>
        <div class="pillar-title">Chẩn Đoán Vi Sinh Trúng Đích (Targeted Diagnostics)</div>
        <div class="pillar-desc">Chỉ định cấy phân / NAAT cho tiêu chảy phân máu, sốt cao, nhiễm khuẩn huyết; bắt buộc sàng lọc STEC phân biệt O157:H7 và độc tố Shiga 1/2; nuôi cấy kiểm chứng để làm kháng sinh đồ.</div>
      </div>
    </div>
    <div class="pillar p3">
      <div class="pillar-icon">🛡️</div>
      <div>
        <div class="pillar-title">Cá Thể Hóa &amp; An Toàn Tuyệt Đối (Prudent Management)</div>
        <div class="pillar-desc">Hạn chế lạm dụng kháng sinh kinh nghiệm; cấm kháng sinh khi nghi ngờ STEC để tránh HUS; cấm Loperamide ở trẻ nhỏ và tiêu chảy xâm lấn nhằm ngừa ứ đọng độc tố và phình đại tràng.</div>
      </div>
    </div>
  </div>
</div>

<div class="quickmenu" id="quickmenu">
  <div class="quickmenu-inner">
    <a href="#sec-phan-loai-dich-te" class="quickmenu-item active">1. Phân Loại &amp; Dịch Tễ</a>
    <a href="#sec-lam-sang-luu-do" class="quickmenu-item">2. Lâm Sàng &amp; Lưu Đồ (Fig 1)</a>
    <a href="#sec-chi-dinh-xet-nghiem" class="quickmenu-item">3. Chỉ Định Xét Nghiệm</a>
    <a href="#sec-bu-dich-dinh-duong" class="quickmenu-item">4. Liệu Pháp Bù Dịch</a>
    <a href="#sec-khang-sinh-stec" class="quickmenu-item">5. Kháng Sinh &amp; Cảnh Báo STEC</a>
    <a href="#sec-thuoc-ho-tro-du-phong-grade" class="quickmenu-item">6. Thuốc Hỗ Trợ &amp; GRADE (Fig 2)</a>
    <a href="#sec-trich-dan-y-van" class="quickmenu-item">7. Y Văn &amp; Tra Cứu</a>
  </div>
</div>

<div class="page-content">

  {/* SECTION 1 */}
  <div class="sec-card" id="sec-phan-loai-dich-te">
    <div class="sec-hdr">
      <div class="sec-hdr-left">
        <span class="sec-hdr-icon">⏱️</span>
        <h2 class="sec-title">1. Phân Loại Thời Gian Tiêu Chảy &amp; Khai Thác Yếu Tố Dịch Tễ Phơi Nhiễm</h2>
      </div>
      <span class="sec-badge">Epidemiology &amp; Exposure</span>
    </div>
    <div class="sec-body">

      <div class="infobox info">
        <div class="infobox-icon">💡</div>
        <div>
          <span class="infobox-title">Nguyên Tắc Tiếp Cận Ban Đầu: Đánh Giá Mất Nước &amp; Phân Nhóm Thời Gian</span>
          Mọi bệnh nhân ở tất cả các lứa tuổi bị tiêu chảy cấp đều phải được <strong>đánh giá tình trạng mất nước (dehydration)</strong> đầu tiên, vì mất nước nặng là yếu tố hàng đầu gây tử vong ở trẻ nhỏ và người cao tuổi. Quá trình đánh giá bắt buộc phải phân loại dựa trên mốc thời gian:
          <ul style="margin-top: 0.5rem; margin-left: 1.25rem;">
            <li><strong>Tiêu chảy cấp tính (Acute Diarrhea):</strong> Triệu chứng kéo dài <strong>dưới 14 ngày</strong> (từ 0 đến 13 ngày).</li>
            <li><strong>Tiêu chảy kéo dài (Persistent Diarrhea):</strong> Triệu chứng kéo dài từ <strong>14 đến 29 ngày</strong>.</li>
            <li><strong>Tiêu chảy mạn tính (Chronic Diarrhea):</strong> Triệu chứng kéo dài từ <strong>30 ngày trở lên</strong>.</li>
          </ul>
        </div>
      </div>

      <div class="sec-subtitle">Bảng 1: Các Phương Thức Lây Truyền Của Vi Sinh Vật Đường Ruột &amp; Hướng Dẫn Tham Chiếu (Table 1 IDSA 2017)</div>
      <div class="table-wrapper">
        <table class="data-table">
          <thead>
            <tr>
              <th style="width: 30%;">Phương Thức Lây Truyền / Bối Cảnh</th>
              <th style="width: 45%;">Tài Liệu Hướng Dẫn Tham Chiếu</th>
              <th style="width: 25%;">Cơ Quan Ban Hành</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td><strong>Du lịch quốc tế (International travel)</strong></td>
              <td>• Expert Review of the Evidence Base for Prevention of Travelers' Diarrhea<br />• Medical Considerations Before Travel / The Yellow Book</td>
              <td>DuPont et al / Freedman et al / CDC</td>
            </tr>
            <tr>
              <td><strong>Ký chủ suy giảm miễn dịch (Immunocompromised hosts)</strong></td>
              <td>• Guidelines for the Prevention and Treatment of Opportunistic Infections in HIV-Infected Adults, Adolescents &amp; Children</td>
              <td>CDC / NIH / HIVMA / IDSA</td>
            </tr>
            <tr>
              <td><strong>Lây truyền qua thực phẩm &amp; nguồn nước (Food &amp; Water)</strong></td>
              <td>• Surveillance for Foodborne Disease Outbreaks<br />• Food Safety &amp; Healthy Water Guidelines</td>
              <td>CDC</td>
            </tr>
            <tr>
              <td><strong>Tiêu chảy do kháng sinh (<em>C. difficile</em>)</strong></td>
              <td>• Clinical Practice Guidelines for <em>Clostridium difficile</em> Infection in Adults and Children (Update)</td>
              <td>IDSA / SHEA</td>
            </tr>
            <tr>
              <td><strong>Cơ sở chăm sóc y tế (Healthcare-associated)</strong></td>
              <td>• Healthcare-Associated Infections Guidelines</td>
              <td>CDC</td>
            </tr>
            <tr>
              <td><strong>Nhà trẻ &amp; Cơ sở chăm sóc trẻ em (Child care)</strong></td>
              <td>• Caring for Our Children: Health &amp; Safety Standards<br />• Managing Infectious Diseases in Child Care &amp; Schools</td>
              <td>AAP / APHA / NRC</td>
            </tr>
            <tr>
              <td><strong>Cơ sở chăm sóc dài hạn (Long-term care)</strong></td>
              <td>• Infection Prevention and Control in Long-term Care Facilities</td>
              <td>CDC / SHEA / APIC</td>
            </tr>
            <tr>
              <td><strong>Bệnh lây truyền từ động vật (Zoonoses)</strong></td>
              <td>• Animals in Public Settings / One Health Initiative</td>
              <td>CDC / Pickering / Rubin</td>
            </tr>
          </tbody>
        </table>
      </div>

      <div class="sec-subtitle" style="margin-top: 1.5rem;">Bảng 2: Mối Liên Quan Sinh Lý Giữa Yếu Tố Phơi Nhiễm / Cơ Địa &amp; Tác Nhân Gây Bệnh (Table 2 IDSA 2017)</div>
      <div class="table-wrapper">
        <table class="data-table">
          <thead>
            <tr>
              <th style="width: 32%;">Hoàn Cảnh Phơi Nhiễm / Tình Trạng Cơ Địa</th>
              <th style="width: 68%;">Các Tác Nhân Gây Bệnh Đường Ruột Liên Quan Phổ Biến</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td><strong>Khách sạn, tàu du lịch, nhà hàng, tiệc tập thể</strong></td>
              <td><em>Norovirus</em>, <em>Salmonella</em> không thương hàn, <em>Clostridium perfringens</em>, <em>Bacillus cereus</em>, <em>Staphylococcus aureus</em>, <em>Campylobacter</em> spp, ETEC, STEC, <em>Listeria</em>, <em>Shigella</em>, <em>Cyclospora cayetanensis</em>, <em>Cryptosporidium</em> spp.</td>
            </tr>
            <tr>
              <td><strong>Sữa tươi chưa tiệt trùng &amp; sản phẩm từ sữa</strong></td>
              <td><em>Salmonella</em>, <em>Campylobacter</em>, <em>Yersinia enterocolitica</em>, độc tố <em>S. aureus</em>, <em>Cryptosporidium</em>, STEC; ít gặp hơn: <em>Listeria</em>, <em>Brucella</em> (phô mai dê), <em>M. bovis</em>, <em>Coxiella burnetii</em>.</td>
            </tr>
            <tr>
              <td><strong>Thịt hoặc gia cầm chưa nấu chín kỹ</strong></td>
              <td>STEC (thịt bò tái), <em>C. perfringens</em> (thịt bò/gia cầm), <em>Salmonella</em> / <em>Campylobacter</em> (gia cầm), <em>Yersinia</em> (thịt lợn, lòng lợn), <em>Trichinella</em> (thịt thú rừng/thịt lợn).</td>
            </tr>
            <tr>
              <td><strong>Trái cây, rau mầm, rau sống, nước ép tươi</strong></td>
              <td>STEC, <em>Salmonella</em> không thương hàn, <em>Cyclospora</em>, <em>Cryptosporidium</em>, <em>Norovirus</em>, <em>Hepatitis A</em>, <em>Listeria monocytogenes</em>.</td>
            </tr>
            <tr>
              <td><strong>Trứng sống hoặc lòng đào</strong></td>
              <td><em>Salmonella enterica</em>, <em>Shigella</em> (trong salad trứng).</td>
            </tr>
            <tr>
              <td><strong>Hải sản có vỏ sống (hàu, sò, vẹm)</strong></td>
              <td>Các loài <em>Vibrio</em> (<em>V. vulnificus, V. parahaemolyticus</em>), <em>Norovirus</em>, <em>Hepatitis A</em>, <em>Plesiomonas</em>.</td>
            </tr>
            <tr>
              <td><strong>Bơi / Uống nước ngọt tự nhiên (sông, hồ)</strong></td>
              <td><em>Campylobacter</em>, <em>Cryptosporidium</em>, <em>Giardia lamblia</em>, <em>Shigella</em>, <em>Salmonella</em>, STEC, <em>Plesiomonas shigelloides</em>.</td>
            </tr>
            <tr>
              <td><strong>Hồ bơi công cộng xử lý Clo</strong></td>
              <td><em>Cryptosporidium</em> (u nang kháng clo rất cao khi nồng độ khử trùng không đạt chuẩn).</td>
            </tr>
            <tr>
              <td><strong>Bệnh viện, cơ sở điều dưỡng, nhà tù</strong></td>
              <td><em>Norovirus</em>, <em>Clostridioides difficile</em>, <em>Shigella</em>, <em>Cryptosporidium</em>, <em>Giardia</em>, STEC, <em>Rotavirus</em>.</td>
            </tr>
            <tr>
              <td><strong>Nhà trẻ / Mẫu giáo</strong></td>
              <td><em>Rotavirus</em>, <em>Cryptosporidium</em>, <em>Giardia</em>, <em>Shigella</em>, STEC.</td>
            </tr>
            <tr>
              <td><strong>Sử dụng kháng sinh gần đây</strong></td>
              <td><em>C. difficile</em>, <em>Salmonella</em> kháng đa thuốc.</td>
            </tr>
            <tr>
              <td><strong>Du lịch đến quốc gia nghèo tài nguyên</strong></td>
              <td>EAEC, ETEC, EIEC, <em>Shigella</em>, <em>Salmonella</em> Typhi / non-typhoidal, <em>Campylobacter</em>, <em>Vibrio cholerae</em>, <em>Entamoeba histolytica</em>, <em>Giardia</em>, <em>Cyclospora</em>, <em>Cryptosporidium</em>.</td>
            </tr>
            <tr>
              <td><strong>Tiếp xúc động vật nuôi bị tiêu chảy</strong></td>
              <td><em>Campylobacter</em>, <em>Yersinia</em>; Tiếp xúc bò sát/gia cầm non: <em>Salmonella</em>; Tham quan sở thú tương tác: STEC, <em>Cryptosporidium</em>.</td>
            </tr>
            <tr>
              <td><strong>Yếu tố tuổi tác</strong></td>
              <td>• 6–18 tháng: <em>Rotavirus</em>.<br />• Sơ sinh &lt;3 tháng &amp; Người lớn &gt;50 tuổi có xơ vữa mạch: <em>Salmonella</em> không thương hàn.<br />• 1–7 tuổi: <em>Shigella</em>.<br />• Người trẻ tuổi: <em>Campylobacter</em>.</td>
            </tr>
            <tr>
              <td><strong>Suy giảm miễn dịch / Bệnh lý nền</strong></td>
              <td>• Ứ sắt (Hemochromatosis): <em>Y. enterocolitica</em>, <em>Salmonella</em>.<br />• AIDS / Ức chế miễn dịch: <em>Cryptosporidium</em>, <em>Microsporidia</em>, <em>Cystoisospora</em>, <em>MAC</em>, <em>CMV</em>.<br />• Quan hệ tình dục qua đường hậu môn: <em>Shigella</em>, <em>Salmonella</em>, <em>Campylobacter</em>, <em>E. histolytica</em>, <em>Giardia</em>.</td>
            </tr>
          </tbody>
        </table>
      </div>

    </div>
  </div>

  {/* SECTION 2 */}
  <div class="sec-card" id="sec-lam-sang-luu-do">
    <div class="sec-hdr">
      <div class="sec-hdr-left">
        <span class="sec-hdr-icon">🩺</span>
        <h2 class="sec-title">2. Biểu Hiện Lâm Sàng Định Hướng &amp; Lưu Đồ Đánh Giá Toàn Diện (Figure 1)</h2>
      </div>
      <span class="sec-badge">Clinical Presentations &amp; Algorithm</span>
    </div>
    <div class="sec-body">

      <div class="sec-subtitle">Bảng 3: Các Biểu Hiện Lâm Sàng Gợi Ý Căn Nguyên Gây Bệnh Tiêu Chảy Nhiễm Trùng (Table 3 IDSA 2017)</div>
      <div class="table-wrapper">
        <table class="data-table">
          <thead>
            <tr>
              <th style="width: 32%;">Hội Chứng Lâm Sàng Điển Hình</th>
              <th style="width: 68%;">Căn Nguyên Gợi Ý Phổ Biến &amp; Ý Nghĩa Lâm Sàng</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td><span class="rx-tag danger">Phân máu đại thể (Bloody stools)</span></td>
              <td><strong>STEC</strong>, <em>Shigella</em>, <em>Salmonella</em>, <em>Campylobacter</em>, <em>Entamoeba histolytica</em>, <em>Vibrio</em> không gây tả, <em>Yersinia</em>, <em>Plesiomonas</em>.</td>
            </tr>
            <tr>
              <td><span class="rx-tag warning">Đau bụng dữ dội + Phân máu + KHÔNG sốt / sốt nhẹ</span></td>
              <td><strong>Shiga toxin-producing <em>E. coli</em> (STEC)</strong> điển hình (bệnh nhân thường không sốt lúc khởi phát), <em>Salmonella</em>, <em>Shigella</em>, <em>Campylobacter</em>, <em>Y. enterocolitica</em>.</td>
            </tr>
            <tr>
              <td><span class="rx-tag info">Đau hố chậu phải kéo dài + Sốt</span></td>
              <td><strong><em>Yersinia enterocolitica</em></strong> và <em>Y. pseudotuberculosis</em> (gây viêm hạch mạc treo kích thích đau giống hệt viêm ruột thừa cấp).</td>
            </tr>
            <tr>
              <td><span class="rx-tag">Nôn mửa dữ dội, khởi phát nhanh &le; 24 giờ</span></td>
              <td>Ngộ độc độc tố ruột của <strong><em>Staphylococcus aureus</em></strong> hoặc <strong><em>Bacillus cereus</em></strong> (thể nôn, ủ bệnh ngắn 1–6 giờ sau ăn).</td>
            </tr>
            <tr>
              <td><span class="rx-tag">Tiêu chảy kèm đau quặn bụng 1–2 ngày</span></td>
              <td>Ngộ độc độc tố của <strong><em>Clostridium perfringens</em></strong> hoặc <strong><em>Bacillus cereus</em></strong> (thể tiêu chảy, ủ bệnh 8–16 giờ).</td>
            </tr>
            <tr>
              <td><span class="rx-tag preferred">Nôn mửa + Tiêu chảy không máu &le; 2–3 ngày</span></td>
              <td><strong><em>Norovirus</em></strong> (nguyên nhân hàng đầu gây bùng phát dịch nôn mửa tiêu chảy mùa đông ở mọi lứa tuổi, ~40% có sốt nhẹ &lt;24h).</td>
            </tr>
            <tr>
              <td><span class="rx-tag">Tiêu chảy kéo dài / mạn tính (&ge; 14 ngày)</span></td>
              <td>Ký sinh trùng đường ruột: <em>Cryptosporidium</em>, <em>Giardia lamblia</em>, <em>Cyclospora cayetanensis</em>, <em>Cystoisospora belli</em>, <em>Entamoeba histolytica</em>.</td>
            </tr>
            <tr>
              <td><span class="rx-tag">Tiêu chảy nước mạn tính kéo dài hàng năm</span></td>
              <td>Tiêu chảy Brainerd (Brainerd diarrhea); Hội chứng ruột kích thích hậu nhiễm trùng (Postinfectious IBS).</td>
            </tr>
          </tbody>
        </table>
      </div>

      <div class="sec-subtitle" style="margin-top: 1.5rem;">Figure 1: Lưu Đồ Tiếp Cận Đánh Giá Người Bệnh Tiêu Chảy Nhiễm Trùng (IDSA 2017)</div>
      <div class="fig-card">
        <img src="./images/2017-idsa-infectious-diarrhea-fig1.png" alt="IDSA 2017 Diarrhea Evaluation Algorithm" class="fig-img" loading="lazy" />
        <div class="fig-caption">
          <div class="fig-title">Figure 1. Considerations when evaluating people with infectious diarrhea (IDSA 2017 Guidelines)</div>
          Lưu đồ tiếp cận đa chiều kết hợp giữa: <strong>(1) Phân loại thời gian</strong> (Cấp &lt;14 ngày, Kéo dài 14–29 ngày, Mạn tính &ge;30 ngày) song hành cùng <strong>(2) 9 khía cạnh dịch tễ phơi nhiễm</strong> (Nhà trẻ, Điều dưỡng, Nhập viện, Nước/Thực phẩm, Du lịch, Kháng sinh, Ký chủ suy giảm miễn dịch, Quan hệ tình dục, Tiếp xúc động vật) và <strong>(3) Biểu hiện lâm sàng</strong> để đưa ra quyết định chỉ định cận lâm sàng chính xác.
        </div>
      </div>

      <div class="flowchart-editorial-card">
  <div class="flowchart-card-header">
    <div class="flowchart-card-title">
      <svg class="flowchart-title-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        <polygon points="12 2 2 7 12 12 22 7 12 2"></polygon>
        <polyline points="2 17 12 22 22 17"></polyline>
        <polyline points="2 12 12 17 22 12"></polyline>
      </svg>
      <span>Lưu Đồ Tiếp Cận Lâm Sàng &amp; Phân Tầng Xét Nghiệm Tiêu Chảy Nhiễm Trùng (IDSA 2017)</span>
    </div>
    <div class="flowchart-card-subtitle">Tiếp cận đa chiều dựa trên thời gian, dấu hiệu phân máu và tầm soát Shiga toxin tránh HUS</div>
  </div>
  <div class="flowchart-card-body">
    <div class="flowchart-svg-wrapper">
<svg class="med-svg" viewBox="0 0 940 520" width="100%" height="100%" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <marker id="arr-diar" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
      <path d="M 0 1.5 L 8 5 L 0 8.5 z" fill="var(--color-primary, #0284c7)"/>
    </marker>
  </defs>

  <!-- Root -->
  <g transform="translate(330, 20)">
    <rect width="280" height="52" rx="10" fill="var(--color-surface)" stroke="#0284c7" stroke-width="2"/>
    <rect x="0" y="0" width="280" height="52" rx="10" fill="#0284c7" fill-opacity="0.08"/>
    <text x="140" y="24" text-anchor="middle" font-size="10" font-weight="700" fill="#0284c7" letter-spacing="1">IDSA 2017 GUIDELINES</text>
    <text x="140" y="42" text-anchor="middle" font-size="13" font-weight="700" fill="var(--color-text)">Bệnh Nhân Tiêu Chảy Nhiễm Trùng</text>
  </g>

  <path d="M 470 72 L 470 105" fill="none" stroke="#0284c7" stroke-width="2" marker-end="url(#arr-diar)"/>

  <!-- Duration Decision -->
  <g transform="translate(320, 105)">
    <rect width="300" height="50" rx="8" fill="var(--color-surface)" stroke="#d97706" stroke-width="1.8"/>
    <rect x="0" y="0" width="300" height="50" rx="8" fill="#d97706" fill-opacity="0.06"/>
    <text x="150" y="22" text-anchor="middle" font-size="10" font-weight="700" fill="#d97706">PHÂN LOẠI THỜI GIAN DIỄN TIẾN</text>
    <text x="150" y="39" text-anchor="middle" font-size="11" font-weight="600" fill="var(--color-text)">Cấp tính (&lt;14 ngày) | Kéo dài (14-29 ngày) | Mạn tính (&gt;=30 ngày)</text>
  </g>

  <path d="M 320 130 L 220 130 L 220 185" fill="none" stroke="#0284c7" stroke-width="1.8" marker-end="url(#arr-diar)"/>
  <path d="M 620 130 L 720 130 L 720 185" fill="none" stroke="#f59e0b" stroke-width="1.8" marker-end="url(#arr-diar)"/>

  <!-- Left: Acute -->
  <g transform="translate(90, 185)">
    <rect width="260" height="52" rx="8" fill="var(--color-surface)" stroke="#0ea5e9" stroke-width="1.5"/>
    <text x="130" y="22" text-anchor="middle" font-size="11" font-weight="700" fill="#0ea5e9">TIÊU CHẢY CẤP TÍNH (&lt; 14 NGÀY)</text>
    <text x="130" y="39" text-anchor="middle" font-size="10" fill="var(--color-text-muted)">Đánh giá phân máu, sốt cao, đau dữ dội?</text>
  </g>

  <!-- Right: Persistent / Chronic -->
  <g transform="translate(590, 185)">
    <rect width="260" height="52" rx="8" fill="var(--color-surface)" stroke="#f59e0b" stroke-width="1.5"/>
    <rect x="0" y="0" width="260" height="52" rx="8" fill="#f59e0b" fill-opacity="0.07"/>
    <text x="130" y="22" text-anchor="middle" font-size="11" font-weight="700" fill="#f59e0b">KÉO DÀI / MẠN TÍNH (&gt;= 14 NGÀY)</text>
    <text x="130" y="39" text-anchor="middle" font-size="10" fill="var(--color-text-muted)">Tầm soát ký sinh trùng, vi khuẩn dai dẳng, IBD</text>
  </g>

  <!-- Split Acute into Watery vs Bloody -->
  <path d="M 150 237 L 150 275" fill="none" stroke="#10b981" stroke-width="1.8" marker-end="url(#arr-diar)"/>
  <path d="M 290 237 L 290 275" fill="none" stroke="#f43f5e" stroke-width="1.8" marker-end="url(#arr-diar)"/>

  <!-- Watery Branch -->
  <g transform="translate(40, 275)">
    <rect width="210" height="74" rx="8" fill="var(--color-surface)" stroke="#10b981" stroke-width="2"/>
    <rect x="0" y="0" width="210" height="74" rx="8" fill="#10b981" fill-opacity="0.08"/>
    <text x="105" y="20" text-anchor="middle" font-size="10" font-weight="700" fill="#10b981">PHÂN NƯỚC TỰ GIỚI HẠN</text>
    <text x="105" y="38" text-anchor="middle" font-size="11" font-weight="700" fill="var(--color-text)">Bù Dịch ORS + Dinh Dưỡng</text>
    <text x="105" y="54" text-anchor="middle" font-size="10" fill="var(--color-text-muted)">KHÔNG XÉT NGHIỆM PHÂN</text>
    <text x="105" y="66" text-anchor="middle" font-size="9" fill="var(--color-text-muted)">thường quy ở người khỏe mạnh</text>
  </g>

  <!-- Connect Bloody and Chronic to Stool Testing -->
  <path d="M 290 330 L 450 330 L 450 350" fill="none" stroke="#f43f5e" stroke-width="1.8" marker-end="url(#arr-diar)"/>
  <path d="M 720 237 L 720 330 L 580 330 L 580 350" fill="none" stroke="#f59e0b" stroke-width="1.8" marker-end="url(#arr-diar)"/>

  <!-- Stool Testing Node -->
  <g transform="translate(380, 350)">
    <rect width="270" height="52" rx="8" fill="var(--color-surface)" stroke="#f43f5e" stroke-width="2"/>
    <rect x="0" y="0" width="270" height="52" rx="8" fill="#f43f5e" fill-opacity="0.08"/>
    <text x="135" y="20" text-anchor="middle" font-size="10" font-weight="700" fill="#f43f5e">CHỈ ĐỊNH XÉT NGHIỆM PHÂN CHUYÊN SÂU</text>
    <text x="135" y="36" text-anchor="middle" font-size="11" font-weight="600" fill="var(--color-text)">Cấy phân, Shiga toxin EIA, NAAT đa mồi</text>
    <text x="135" y="48" text-anchor="middle" font-size="9" fill="var(--color-text-muted)">Độc tố C. difficile nếu có dùng kháng sinh</text>
  </g>

  <path d="M 440 402 L 440 435" fill="none" stroke="#f43f5e" stroke-width="2" marker-end="url(#arr-diar)"/>
  <path d="M 590 402 L 590 435" fill="none" stroke="#0ea5e9" stroke-width="1.5" marker-end="url(#arr-diar)"/>

  <!-- Shiga Toxin (+) Warning -->
  <g transform="translate(320, 435)">
    <rect width="240" height="64" rx="8" fill="var(--color-surface)" stroke="#f43f5e" stroke-width="2"/>
    <rect x="0" y="0" width="240" height="64" rx="8" fill="#f43f5e" fill-opacity="0.09"/>
    <text x="120" y="19" text-anchor="middle" font-size="10" font-weight="700" fill="#f43f5e">SHIGA TOXIN (+) / NGHI STEC 🚫</text>
    <text x="120" y="36" text-anchor="middle" font-size="11" font-weight="700" fill="var(--color-text)">CHỐNG CHỈ ĐỊNH KHÁNG SINH</text>
    <text x="120" y="52" text-anchor="middle" font-size="9" fill="var(--color-text-muted)">Tránh khởi phát HUS - Bù dịch TM tích cực</text>
  </g>

  <!-- Targeted Antibiotics -->
  <g transform="translate(580, 435)">
    <rect width="240" height="64" rx="8" fill="var(--color-surface)" stroke="#0ea5e9" stroke-width="1.5"/>
    <rect x="0" y="0" width="240" height="64" rx="8" fill="#0ea5e9" fill-opacity="0.07"/>
    <text x="120" y="19" text-anchor="middle" font-size="10" font-weight="700" fill="#0ea5e9">TÁC NHÂN KHÁC (SHIGELLA, SALMONELLA)</text>
    <text x="120" y="36" text-anchor="middle" font-size="11" font-weight="600" fill="var(--color-text)">Kháng Sinh Trúng Đích</text>
    <text x="120" y="52" text-anchor="middle" font-size="9" fill="var(--color-text-muted)">Azithromycin / Ciprofloxacin theo KS đồ</text>
  </g>
</svg>
    </div>
  </div>
  <div class="flowchart-card-footer">
    <div class="flowchart-legend-row">
      <span class="legend-item"><span class="legend-dot" style="background: #10b981;"></span>Tiêu chảy phân nước tự giới hạn: Bù ORS, không xét nghiệm phân thường quy</span>
      <span class="legend-item"><span class="legend-dot" style="background: #f59e0b;"></span>Tiêu chảy kéo dài >= 14 ngày hoặc sốt/phân máu: Chỉ định xét nghiệm phân chuyên sâu</span>
      <span class="legend-item"><span class="legend-dot" style="background: #f43f5e;"></span>Shiga toxin (+) / STEC: CHỐNG CHỈ ĐỊNH KHÁNG SINH TUYỆT ĐỐI để phòng ngừa HUS</span>
    </div>
  </div>
</div>

    </div>
  </div>

  {/* SECTION 3 */}
  <div class="sec-card" id="sec-chi-dinh-xet-nghiem">
    <div class="sec-hdr">
      <div class="sec-hdr-left">
        <span class="sec-hdr-icon">🔬</span>
        <h2 class="sec-title">3. Chỉ Định Xét Nghiệm Phân, Cấy Máu, Panel Đa Mồi NAAT &amp; Thăm Dò Khác</h2>
      </div>
      <span class="sec-badge">Laboratory &amp; Diagnostics</span>
    </div>
    <div class="sec-body">

      <div class="grid-2col">
        <div class="clin-card">
          <div class="clin-card-accent accent-blue"></div>
          <div class="clin-card-title"><span>Chỉ Định Xét Nghiệm Phân (Stool Testing)</span> <span>💩</span></div>
          <div class="clin-card-body">
            <ul class="clin-card-list">
              <li><strong>Không xét nghiệm thường quy:</strong> Không chỉ định cấy phân cho tiêu chảy cấp nước tự giới hạn ở người khỏe mạnh.</li>
              <li><strong>Chỉ định bắt buộc khi:</strong> Tiêu chảy kèm <strong>sốt</strong>, phân có <strong>máu hoặc chất nhầy</strong>, đau bụng dữ dội, dấu hiệu <strong>nhiễm khuẩn huyết (sepsis)</strong>, hoặc tiêu chảy kéo dài &ge; 14 ngày.</li>
              <li><strong>Panel tác nhân cơ bản:</strong> <em>Salmonella, Shigella, Campylobacter, Yersinia, C. difficile</em> và STEC.</li>
              <li><strong>Quy chuẩn STEC:</strong> Bắt buộc dùng phương pháp phát hiện Shiga toxin (hoặc gen độc tố), phân biệt chủng <strong>O157:H7</strong> (trên thạch Sorbitol-MacConkey) và định type <strong>Shiga toxin 1 vs Shiga toxin 2</strong> (độc lực cao hơn).</li>
              <li><strong>Mẫu bệnh phẩm:</strong> Ưu tiên mẫu phân lỏng thực sự; chỉ dùng tăm bông trực tràng khi không lấy được phân và chỉ cho xét nghiệm vi khuẩn (cấm dùng tăm bông cho virus, ký sinh trùng và độc tố <em>C. difficile</em>).</li>
            </ul>
          </div>
        </div>

        <div class="clin-card">
          <div class="clin-card-accent accent-green"></div>
          <div class="clin-card-title"><span>Chỉ Định Cấy Máu (Blood Cultures)</span> <span>🩸</span></div>
          <div class="clin-card-body">
            <ul class="clin-card-list">
              <li><strong>1. Trẻ nhỏ &lt; 3 tháng tuổi</strong> bị tiêu chảy cấp.</li>
              <li><strong>2. Dấu hiệu nhiễm khuẩn huyết (Septicemia)</strong> hoặc nghi ngờ sốt thương hàn / phó thương hàn (Enteric fever).</li>
              <li><strong>3. Biểu hiện nhiễm trùng hệ thống</strong> ngoài đường tiêu hóa.</li>
              <li><strong>4. Bệnh nhân suy giảm miễn dịch</strong> (AIDS, ung thư, ghép tạng, đang dùng ức chế miễn dịch).</li>
              <li><strong>5. Bệnh lý nền nguy cơ cao</strong> (như bệnh thiếu máu tán huyết bẩm sinh).</li>
              <li><strong>6. Sốt không rõ nguyên nhân</strong> sau khi du lịch đến vùng dịch tễ thương hàn.</li>
            </ul>
          </div>
        </div>
      </div>

      <div class="updates-grid" style="margin-top: 1.5rem;">
        <div class="update-card">
          <div class="update-card-accent" style="background: #7c3aed;"></div>
          <div class="update-card-header">
            <div class="update-card-icon" style="background: #faf5ff; color: #7c3aed;">🧬</div>
            <div class="update-card-title">Panel Đa Mồi Khuếch Đại Axit Nucleic (NAAT)</div>
          </div>
          <div class="update-card-body">
            <ul style="margin-left: 1rem; line-height: 1.7;">
              <li><strong>Độ nhạy vượt trội:</strong> Phát hiện đồng thời hàng chục tác nhân virus, vi khuẩn, ký sinh trùng trong vài giờ.</li>
              <li><strong>Thận trọng phiên giải:</strong> NAAT phát hiện DNA/RNA chứ <strong>không chứng minh vi khuẩn còn sống hay đang hoạt động</strong>. Cần đối chiếu sát bệnh cảnh lâm sàng.</li>
              <li><strong>Bắt buộc nuôi cấy kiểm chứng:</strong> Các mẫu NAAT dương tính với vi khuẩn vẫn phải nuôi cấy lại để phục vụ <strong>kháng sinh đồ</strong> và giám sát dịch tễ học.</li>
            </ul>
          </div>
          <div class="update-verdict verdict-purple">Công Nghệ Phân Tử Hiện Đại</div>
        </div>

        <div class="update-card">
          <div class="update-card-accent" style="background: #ea580c;"></div>
          <div class="update-card-header">
            <div class="update-card-icon" style="background: #fff7ed; color: #ea580c;">⚠️</div>
            <div class="update-card-title">Xét Nghiệm Không Vi Sinh &amp; Cảnh Báo</div>
          </div>
          <div class="update-card-body">
            <ul style="margin-left: 1rem; line-height: 1.7;">
              <li><strong>Bạch cầu phân &amp; Lactoferrin:</strong> <strong>KHÔNG khuyến cáo</strong> sử dụng thường quy vì không giúp phân biệt căn nguyên chính xác.</li>
              <li><strong>Công thức máu ngoại vi:</strong> Không dùng để xác định nguyên nhân tiêu chảy; nhưng <strong>bắt buộc theo dõi sát Hb, Tiểu cầu, Creatinine máu và mảnh vỡ hồng cầu (schistocytes)</strong> ở bệnh nhân nhiễm STEC để phát hiện sớm biến chứng HUS.</li>
              <li><strong>Nội soi đại trực tràng / Hút dịch tá tràng:</strong> Chỉ định chọn lọc khi nghi ngờ viêm đại tràng nặng, AIDS, hoặc cấy phân âm tính nhiều lần.</li>
            </ul>
          </div>
          <div class="update-verdict verdict-amber">Tránh Lạm Dụng Cận Lâm Sàng</div>
        </div>
      </div>

    </div>
  </div>

  {/* SECTION 4 */}
  <div class="sec-card" id="sec-bu-dich-dinh-duong">
    <div class="sec-hdr">
      <div class="sec-hdr-left">
        <span class="sec-hdr-icon">💧</span>
        <h2 class="sec-title">4. Liệu Pháp Bù Dịch (Rehydration Therapy) &amp; Chế Độ Dinh Dưỡng Theo Lứa Tuổi</h2>
      </div>
      <span class="sec-badge">Fluid Therapy &amp; Nutrition</span>
    </div>
    <div class="sec-body">

      <div class="table-wrapper">
        <table class="data-table">
          <thead>
            <tr>
              <th style="width: 25%;">Phương Thức Bù Dịch</th>
              <th style="width: 35%;">Chỉ Định Lâm Sàng</th>
              <th style="width: 40%;">Nguyên Tắc Sinh Lý Học &amp; Hướng Dẫn Thực Hành</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td><span class="rx-tag preferred">Reduced Osmolarity ORS (Đường uống)</span></td>
              <td><strong>Lựa chọn hàng đầu</strong> cho mọi bệnh nhân mất nước từ nhẹ đến trung bình do bất kỳ căn nguyên nào.</td>
              <td>ORS độ thẩm thấu giảm (245 mOsm/L) tối ưu hóa đồng vận chuyển Na-Glucose qua niêm mạc ruột non mà không gây tiêu chảy thẩm thấu ngược như dung dịch ưu trương.</td>
            </tr>
            <tr>
              <td><span class="rx-tag info">ORS qua Ống thông dạ dày (Nasogastric Tube)</span></td>
              <td>Bệnh nhân mất nước trung bình nhưng <strong>nôn liên tục</strong> không uống được, hoặc trẻ quá yếu/từ chối uống nhưng tri giác còn tỉnh.</td>
              <td>Truyền nhỏ giọt ORS tốc độ chậm qua sonde dạ dày giúp hấp thu hiệu quả, hạn chế phải can thiệp đường truyền tĩnh mạch xâm lấn.</td>
            </tr>
            <tr>
              <td><span class="rx-tag danger">Truyền Tĩnh Mạch Đẳng Trương (IV Fluids)</span></td>
              <td><strong>Mất nước nặng, sốc giảm thể tích</strong>, thay đổi ý thức, tắc ruột (ileus), hoặc thất bại hoàn toàn với bù dịch đường uống.</td>
              <td>Sử dụng dịch đẳng trương <strong>Lactated Ringer's</strong> hoặc <strong>NaCl 0.9%</strong>. Ở BN có toan ceton do nhịn đói (ketonemia), truyền dịch ban đầu giúp cải thiện toan hóa để tiếp tục dung nạp ORS. Ngừng IV chuyển sang ORS ngay khi sinh hiệu ổn định và bệnh nhân tỉnh táo.</td>
            </tr>
          </tbody>
        </table>
      </div>

      <div class="infobox teal" style="margin-top: 1.5rem;">
        <div class="infobox-icon">🍼</div>
        <div>
          <span class="infobox-title">Chế Độ Dinh Dưỡng Trong Đợt Tiêu Chảy Cấp</span>
          <ul style="margin-top: 0.25rem; margin-left: 1.25rem;">
            <li><strong>Trẻ bú mẹ:</strong> Bắt buộc <strong>tiếp tục cho bú mẹ hoàn toàn và liên tục</strong> trong suốt đợt tiêu chảy. Không được ngừng bú mẹ để thay bằng dung dịch khác.</li>
            <li><strong>Trẻ ăn dặm &amp; Người lớn:</strong> Quay trở lại chế độ ăn thông thường phù hợp lứa tuổi ngay trong hoặc sau khi bù đủ dịch. <strong>Tuyệt đối tránh nhịn ăn hoặc kiêng khem nghiêm ngặt</strong> vì dinh dưỡng sớm thúc đẩy tế bào biểu mô ruột tái tạo và phục hồi chức năng hấp thu nhanh hơn.</li>
          </ul>
        </div>
      </div>

    </div>
  </div>

  {/* SECTION 5 */}
  <div class="sec-card" id="sec-khang-sinh-stec">
    <div class="sec-hdr">
      <div class="sec-hdr-left">
        <span class="sec-hdr-icon">💊</span>
        <h2 class="sec-title">5. Kháng Sinh Kinh Nghiệm, Điều Trị Trúng Đích &amp; Cảnh Báo Tuyệt Đối STEC/HUS</h2>
      </div>
      <span class="sec-badge">Antimicrobial Stewardship &amp; STEC Warning</span>
    </div>
    <div class="sec-body">

      <div class="infobox danger">
        <div class="infobox-icon">🚫</div>
        <div>
          <span class="infobox-title">CẢNH BÁO ĐỎ: CHỐNG CHỈ ĐỊNH TUYỆT ĐỐI KHÁNG SINH TRONG NHIỄM STEC</span>
          <strong>Tránh hoàn toàn việc sử dụng kháng sinh</strong> cho bệnh nhân nghi ngờ hoặc xác định nhiễm <strong>STEC O157</strong> hoặc các chủng STEC sinh độc tố Shiga 2.
          <br /><br />
          <strong>Cơ chế bệnh sinh kích hoạt Hội chứng Tán huyết Tăng Ure máu (HUS):</strong>
          <br />Gen mã hóa độc tố Shiga nằm trên bộ gen của <em>bacteriophage</em> tích hợp trong vi khuẩn STEC. Khi tiếp xúc với kháng sinh (đặc biệt là Fluoroquinolones, Beta-lactams gây tổn thương DNA hoặc ức chế vách tế bào), phản ứng SOS của vi khuẩn được kích hoạt &rarr; bacteriophage chuyển sang chu kỳ sinh tan (lytic cycle) &rarr; nhân bản ồ ạt và giải phóng một lượng khổng lồ <strong>độc tố Shiga (Stx1, Stx2)</strong> vào lòng ruột và máu &rarr; Shiga toxin gắn vào thụ thể Gb3 trên tế bào nội mô mao mạch cầu thận &rarr; hoại tử nội mô, kết tập tiểu cầu vi mạch, thiếu máu tán huyết vi mạch, giảm tiểu cầu tiêu thụ và <strong>suy thận cấp đe dọa tính mạng (HUS)</strong>.
        </div>
      </div>

      <div class="sec-subtitle" style="margin-top: 1.5rem;">Nguyên Tắc Điều Trị Kháng Sinh Theo Kinh Nghiệm (Empiric Therapy)</div>
      <div class="table-wrapper">
        <table class="regimen-table">
          <thead>
            <tr>
              <th style="width: 25%;">Tình Huống Lâm Sàng</th>
              <th style="width: 40%;">Khuyến Cáo Kháng Sinh Kinh Nghiệm IDSA</th>
              <th style="width: 35%;">Phác Đồ &amp; Lưu Ý</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td class="source-cell">
                <span class="rx-tag danger">Tiêu chảy phân máu thường quy</span>
              </td>
              <td><strong>KHÔNG khuyến cáo dùng kháng sinh kinh nghiệm</strong> ở trẻ em và người lớn có miễn dịch bình thường trong khi chờ kết quả xét nghiệm vi sinh.</td>
              <td>Đa số tự giới hạn; tránh kích hoạt biến chứng HUS nếu căn nguyên là STEC.</td>
            </tr>
            <tr>
              <td class="source-cell">
                <span class="rx-tag preferred">Ngoại Lệ 1: Trẻ sơ sinh &lt; 3 tháng tuổi</span>
              </td>
              <td>Chỉ định kháng sinh kinh nghiệm sớm khi nghi ngờ căn nguyên vi khuẩn đường ruột xâm lấn.</td>
              <td><strong>Cephalosporin thế hệ 3 (Ceftriaxone IV)</strong> hoặc <strong>Azithromycin</strong>.</td>
            </tr>
            <tr>
              <td class="source-cell">
                <span class="rx-tag preferred">Ngoại Lệ 2: Hội chứng Lỵ trực trùng nặng</span>
              </td>
              <td>Sốt cao, đau quặn bụng, mót rặn, tiêu phân máu nhầy lượng ít nhiều lần nghi do <em>Shigella</em>.</td>
              <td>• Người lớn: <strong>Fluoroquinolone (Ciprofloxacin)</strong> hoặc <strong>Azithromycin</strong>.<br />• Trẻ em: <strong>Azithromycin</strong> hoặc <strong>Ceftriaxone</strong>.</td>
            </tr>
            <tr>
              <td class="source-cell">
                <span class="rx-tag preferred">Ngoại Lệ 3: Du lịch quốc tế kèm sốt &ge; 38.5°C</span>
              </td>
              <td>Bệnh nhân du lịch trở về bị sốt cao kèm dấu hiệu nhiễm trùng huyết hoặc nhiễm trùng nặng.</td>
              <td><strong>Azithromycin</strong> (lựa chọn ưu tiên cho vùng Đông Nam Á có tỷ lệ kháng Quinolone cao của <em>Campylobacter</em>).</td>
            </tr>
            <tr>
              <td class="source-cell">
                <span class="rx-tag warning">Sốt Thương Hàn (Enteric Fever / Sepsis)</span>
              </td>
              <td>Sốt cao kéo dài, lách to, độc thần kinh, nghi ngờ nhiễm khuẩn huyết do <em>S. Typhi / Paratyphi</em>.</td>
              <td>Cấy máu + Cấy phân ngay &rarr; Khởi trị ngay <strong>Kháng sinh phổ rộng đường tĩnh mạch</strong> (Ceftriaxone / Azithromycin / Carbapenem tùy dịch tễ) &rarr; Thu hẹp theo kháng sinh đồ.</td>
            </tr>
          </tbody>
        </table>
      </div>

      <div class="sec-subtitle" style="margin-top: 1.5rem;">Quản Lý Người Lành Mang Trùng Không Triệu Chứng (Asymptomatic Carriers)</div>
      <div class="grid-2col">
        <div class="clin-card">
          <div class="clin-card-accent accent-green"></div>
          <div class="clin-card-title"><span>Nhóm Nguy Cơ Thấp</span> <span>✅</span></div>
          <div class="clin-card-body">
            <p>Người không triệu chứng, vệ sinh cá nhân tốt, không tiếp xúc nhóm đối tượng nhạy cảm: <strong>KHÔNG CẦN điều trị kháng sinh</strong> (ngoại trừ người mang trùng <em>Salmonella enterica serovar Typhi</em> cần điều trị để tiệt căn mầm bệnh trong cộng đồng).</p>
          </div>
        </div>

        <div class="clin-card">
          <div class="clin-card-accent accent-red"></div>
          <div class="clin-card-title"><span>Nhóm Nguy Cơ Cao</span> <span>⚠️</span></div>
          <div class="clin-card-body">
            <p>Nhân viên y tế, nhân viên nhà trẻ/mẫu giáo, người chăm sóc viện dưỡng lão, nhân viên chế biến thức ăn: <strong>Bắt buộc quản lý và cách ly / điều trị nghiêm ngặt</strong> theo quy định của cơ quan Y tế Dự phòng địa phương cho đến khi cấy phân âm tính.</p>
          </div>
        </div>
      </div>

    </div>
  </div>

  {/* SECTION 6 */}
  <div class="sec-card" id="sec-thuoc-ho-tro-du-phong-grade">
    <div class="sec-hdr">
      <div class="sec-hdr-left">
        <span class="sec-hdr-icon">🛡️</span>
        <h2 class="sec-title">6. Liệu Pháp Hỗ Trợ, Dự Phòng Vắc-Xin &amp; Phương Pháp Đánh Giá Chứng Cứ GRADE (Figure 2)</h2>
      </div>
      <span class="sec-badge">Ancillary Drugs, Prevention &amp; GRADE</span>
    </div>
    <div class="sec-body">

      <div class="sec-subtitle">Các Thuốc Điều Trị Hỗ Trợ Triệu Chứng (Ancillary Management)</div>
      <div class="table-wrapper">
        <table class="data-table">
          <thead>
            <tr>
              <th style="width: 25%;">Thuốc / Liệu Pháp</th>
              <th style="width: 35%;">Chỉ Định &amp; Lưu Ý Lâm Sàng</th>
              <th style="width: 40%;">Chống Chỉ Định &amp; Cảnh Báo An Toàn</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td><span class="rx-tag danger">Thuốc giảm nhu động ruột (Loperamide)</span></td>
              <td>Chỉ cân nhắc ở <strong>người lớn có hệ miễn dịch bình thường</strong> bị tiêu chảy nước cấp tính mức độ nhẹ đến trung bình đã được bù đủ nước.</td>
              <td><strong>CHỐNG CHỈ ĐỊNH TUYỆT ĐỐI Ở TRẺ EM &lt; 18 TUỔI</strong> (nguy cơ liệt ruột, ức chế hô hấp thần kinh) và <strong>mọi lứa tuổi bị tiêu chảy phân máu hoặc có sốt cao</strong> (ngăn cản đào thải độc tố, tăng nguy cơ <strong>phình đại tràng nhiễm độc - toxic megacolon</strong>).</td>
            </tr>
            <tr>
              <td><span class="rx-tag info">Thuốc chống nôn (Ondansetron)</span></td>
              <td>Cân nhắc cho <strong>trẻ &gt; 4 tuổi và vị thành niên</strong> bị nôn ói nhiều để giúp dung nạp ORS đường uống, giảm tỷ lệ phải truyền dịch tĩnh mạch.</td>
              <td>Không dùng thường quy cho trẻ nhỏ &lt; 4 tuổi nếu không có chỉ định chuyên khoa.</td>
            </tr>
            <tr>
              <td><span class="rx-tag preferred">Bổ sung Kẽm (Zinc)</span></td>
              <td>Rút ngắn thời gian tiêu chảy ở trẻ em từ <strong>6 tháng đến 5 tuổi</strong> tại các vùng có nguy cơ thiếu kẽm cao hoặc trẻ có suy dinh dưỡng.</td>
              <td>Không khuyến cáo thường quy cho trẻ em có dinh dưỡng đầy đủ ở các nước phát triển.</td>
            </tr>
            <tr>
              <td><span class="rx-tag teal">Chế phẩm Probiotics</span></td>
              <td>Có thể cân nhắc để giảm mức độ nặng và rút ngắn thời gian tiêu chảy ở người lớn và trẻ em có miễn dịch bình thường.</td>
              <td>Lựa chọn chủng vi sinh vật và liều lượng dựa trên bằng chứng lâm sàng RCTs đã được công bố.</td>
            </tr>
          </tbody>
        </table>
      </div>

      <div class="sec-subtitle" style="margin-top: 1.5rem;">Biện Pháp Dự Phòng Y Tế Công Cộng &amp; Vắc-Xin</div>
      <div class="updates-grid">
        <div class="update-card">
          <div class="update-card-accent" style="background: #10b981;"></div>
          <div class="update-card-header">
            <div class="update-card-icon" style="background: #ecfdf5; color: #10b981;">🧼</div>
            <div class="update-card-title">Vệ Sinh Tay &amp; An Toàn Thực Phẩm</div>
          </div>
          <div class="update-card-body">
            <ul style="margin-left: 1rem; line-height: 1.7;">
              <li><strong>Rửa tay bằng xà phòng và nước:</strong> Biện pháp phòng bệnh số 1 sau khi đi vệ sinh, thay tã, tiếp xúc thú nuôi và trước khi ăn.</li>
              <li><strong>Đặc thù <em>C. difficile</em>:</strong> Bắt buộc rửa tay bằng xà phòng và nước vì <strong>cồn không diệt được bào tử <em>C. diff</em></strong>.</li>
              <li><strong>An toàn thực phẩm:</strong> Ăn chín, uống sôi, tách biệt dụng cụ thực phẩm sống/chín.</li>
              <li><strong>Cách ly nguồn lây:</strong> Người đang tiêu chảy cấm bơi lội công cộng và cấm quan hệ tình dục.</li>
            </ul>
          </div>
          <div class="update-verdict verdict-green">Kiểm Soát Nhiễm Khuẩn</div>
        </div>

        <div class="update-card">
          <div class="update-card-accent" style="background: #0284c7;"></div>
          <div class="update-card-header">
            <div class="update-card-icon" style="background: #eff6ff; color: #0284c7;">💉</div>
            <div class="update-card-title">Chương Trình Vắc-Xin Phòng Bệnh</div>
          </div>
          <div class="update-card-body">
            <ul style="margin-left: 1rem; line-height: 1.7;">
              <li><strong>Vắc-xin Rotavirus:</strong> Tiêm phòng đầy đủ cho toàn bộ trẻ sơ sinh không có chống chỉ định.</li>
              <li><strong>Vắc-xin Thương hàn (Typhoid):</strong> Dạng uống hoặc tiêm cho người du lịch đến vùng dịch, nhân viên lab và người sống chung với người mang trùng <em>S. Typhi</em>.</li>
              <li><strong>Vắc-xin Tả (Cholera):</strong> Vắc-xin sống giảm độc lực dạng uống (1 liều duy nhất) cho người 18–64 tuổi du lịch vùng dịch tả.</li>
            </ul>
          </div>
          <div class="update-verdict verdict-blue">Miễn Dịch Chủ Động</div>
        </div>
      </div>

      <div class="sec-subtitle" style="margin-top: 1.5rem;">Figure 2: Phương Pháp Đánh Giá Bằng Chứng &amp; Xây Dựng Khuyến Cáo GRADE (IDSA 2017)</div>
      <div class="fig-card">
        <img src="./images/2017-idsa-infectious-diarrhea-fig2.png" alt="IDSA GRADE Approach Diagram" class="fig-img" loading="lazy" />
        <div class="fig-caption">
          <div class="fig-title">Figure 2. Grading of Recommendations Assessment, Development and Evaluation (GRADE) approach (IDSA 2017 Guidelines)</div>
          Quy trình 4 bước khoa học của phương pháp GRADE: <strong>(1) Thiết lập độ tin cậy ban đầu</strong> (RCT &rarr; Cao, Quan sát &rarr; Thấp) &rarr; <strong>(2) Cân nhắc hạ bậc</strong> (nguy cơ sai lệch, không nhất quán, gián tiếp, không chính xác, sai lệch xuất bản) hoặc <strong>tăng bậc</strong> (hiệu ứng lớn, liều-đáp ứng) &rarr; <strong>(3) Phân hạng chất lượng chứng cứ cuối cùng</strong> (High, Moderate, Low, Very Low) &rarr; <strong>(4) Quyết định mức độ khuyến cáo</strong> (Strong vs Weak) dựa trên cán cân lợi ích/tác hại, giá trị bệnh nhân và chi phí y tế.
        </div>
      </div>

    </div>
  </div>

  {/* SECTION 7 */}
  <div class="sec-card" id="sec-trich-dan-y-van">
    <div class="sec-hdr">
      <div class="sec-hdr-left">
        <span class="sec-hdr-icon">📚</span>
        <h2 class="sec-title">7. Tài Liệu Tham Khảo Chuẩn AMA &amp; Bảng Viết Tắt Y Khoa</h2>
      </div>
      <span class="sec-badge">References &amp; Glossary</span>
    </div>
    <div class="sec-body">

      <div class="citation-box">
        <strong>Tài liệu gốc (AMA Citation):</strong>
        <br />Shane AL, Mody RK, Crump JA, Tarr PI, Steiner TS, Kotloff K, Langley JM, Wanke C, Warren CA, Cheng AC, Cantey J, Pickering LK. <em>2017 Infectious Diseases Society of America Clinical Practice Guidelines for the Diagnosis and Management of Infectious Diarrhea</em>. <strong>Clinical Infectious Diseases</strong>. 2017;65(12):1963-1973. doi:10.1093/cid/cix959.
      </div>

      <div class="sec-subtitle" style="margin-top: 1.5rem;">Bảng Viết Tắt Y Khoa Chuẩn (Glossary)</div>
      <div class="table-wrapper">
        <table class="data-table">
          <thead>
            <tr>
              <th style="width: 20%;">Viết Tắt</th>
              <th style="width: 40%;">Thuật Ngữ Tiếng Anh</th>
              <th style="width: 40%;">Ý Nghĩa Lâm Sàng</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td><strong>AAD</strong></td>
              <td>Antibiotic-Associated Diarrhea</td>
              <td>Tiêu chảy do kháng sinh</td>
            </tr>
            <tr>
              <td><strong>CDI</strong></td>
              <td><em>Clostridioides difficile</em> Infection</td>
              <td>Nhiễm trùng đại tràng do <em>C. difficile</em></td>
            </tr>
            <tr>
              <td><strong>EAEC / ETEC / EIEC</strong></td>
              <td>Enteroaggregative / Enterotoxigenic / Enteroinvasive <em>E. coli</em></td>
              <td>Các nhóm <em>E. coli</em> gây bệnh đường ruột</td>
            </tr>
            <tr>
              <td><strong>GRADE</strong></td>
              <td>Grading of Recommendations Assessment, Development and Evaluation</td>
              <td>Hệ thống phân cấp đánh giá chất lượng chứng cứ và khuyến cáo</td>
            </tr>
            <tr>
              <td><strong>HUS</strong></td>
              <td>Hemolytic Uremic Syndrome</td>
              <td>Hội chứng tán huyết tăng ure máu</td>
            </tr>
            <tr>
              <td><strong>IDSA</strong></td>
              <td>Infectious Diseases Society of America</td>
              <td>Hiệp hội Bệnh truyền nhiễm Hoa Kỳ</td>
            </tr>
            <tr>
              <td><strong>NAAT</strong></td>
              <td>Nucleic Acid Amplification Test</td>
              <td>Xét nghiệm khuếch đại axit nucleic (PCR / RT-PCR)</td>
            </tr>
            <tr>
              <td><strong>NNT</strong></td>
              <td>Number Needed to Treat</td>
              <td>Số bệnh nhân cần điều trị để ngừa 1 biến cố</td>
            </tr>
            <tr>
              <td><strong>ORS</strong></td>
              <td>Oral Rehydration Salts / Solution</td>
              <td>Dung dịch bù nước và điện giải đường uống</td>
            </tr>
            <tr>
              <td><strong>STEC</strong></td>
              <td>Shiga Toxin-producing <em>Escherichia coli</em></td>
              <td>Vi khuẩn <em>E. coli</em> sinh độc tố Shiga</td>
            </tr>
            <tr>
              <td><strong>Stx1 / Stx2</strong></td>
              <td>Shiga Toxin 1 / Shiga Toxin 2</td>
              <td>Các type ngoại độc tố Shiga</td>
            </tr>
            <tr>
              <td><strong>WBC</strong></td>
              <td>White Blood Cell</td>
              <td>Bạch cầu máu / phân</td>
            </tr>
          </tbody>
        </table>
      </div>

    </div>
  </div>

</div>

{/* BOTTOM NAVIGATION */}
<div style="margin-top: 2rem; margin-bottom: 3rem;">
  <div class="btn-row" style="display: flex; gap: 1rem; flex-wrap: wrap;">
    <a href="#/ebm/kho-guidelines" class="btn btn-primary" style="text-decoration: none;">
      <i class="fa-solid fa-arrow-left"></i> Quay lại Kho Guidelines
    </a>
    <a href="#sec-phan-loai-dich-te" class="btn" style="text-decoration: none;">
      <i class="fa-solid fa-arrow-up"></i> Lên đầu trang
    </a>
  </div>
</div>