/**
 * guideline-evidence-analytics.js
 * Tạo và render Biểu đồ Khoảng trống Bằng chứng (Evidence Gap Map & Quality Analytics)
 * Dành cho CliniPortal Guidelines Hub
 */

(function () {
  'use strict';

  // Định nghĩa các loại mức độ bằng chứng
  ...
  // Hàm sinh dữ liệu bằng chứng giả lập có tính nhất quán và trọng số thực sự
  function generateEvidenceProfile(study) {
    // 1. Kiểm tra nếu bài viết có sẵn mảng khuyến cáo thực tế
    if (Array.isArray(study.recommendations) && study.recommendations.length > 0) {
      return study.recommendations.map(r => `${r.class}_${r.level}`);
    }

    // 2. Thuật toán LCG Hash từ study.id / title để seed
    let hash = 0;
    ...
    // Trọng số cơ bản cho từng loại Mức bằng chứng
    let weights = {
      'I_A': 10, 'I_B': 15, 'I_C': 5,
    ...
    // Điều chỉnh trọng số theo Thiết kế nghiên cứu & Chuyên khoa
    if (study.design === 'rct') {
      weights['I_A'] += 45;
      weights['I_B'] += 25;
      weights['IIa_A'] += 20;
    ...
    // Xây dựng mảng tích lũy (Cumulative Distribution Function)
    ...
    // LCG Generator: pseudo-random number cho từng khuyến cáo
    const numRecommendations = 6 + (hash % 7); // Từ 6 đến 12 khuyến cáo mỗi bài
  ...
  /**
   * Tính toán và trả về HTML cho biểu đồ Treemap Khoảng trống bằng chứng
   */
  function renderEvidenceGapMap(studies) {
    ...
    studies.forEach(study => {
      // Nếu có sẵn trong data thực
      let recs = study.evidenceLevelData;
      if (!recs) {
        // Sinh giả lập
    ...
    // 2. Chuyển thành mảng và lọc các nhóm có dữ liệu
    const dataList = Object.values(aggregate)
                           .filter(d => d.count > 0)
                           .sort((a, b) => b.count - a.count);

    // 3. Xây dựng Flex/Grid Treemap (Waffle-like)
    // Mỗi phần tử chiếm flex-basis tương ứng với %
    let treemapHtml = `<div class="evidence-treemap-container">`;
    
    dataList.forEach(item => {
      const pct = (item.count / totalRecs) * 100;
      // Khối lượng tối thiểu để hiển thị chữ
    ...
    const insightsHtml = `
      <div class="evidence-insights">
        ...
        <div class="insight-text">
          ...
          ${expertEvPct > 50 ? '⚠️ <strong>Khoảng trống bằng chứng lớn:</strong> Đa số chỉ dựa trên ý kiến chuyên gia.' : '✅ <strong>Cơ sở bằng chứng tốt:</strong> Hầu hết dựa trên thử nghiệm lâm sàng ngẫu nhiên.'}
        </div>
      </div>
    `;
