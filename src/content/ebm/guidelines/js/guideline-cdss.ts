/**
 * CliniPortal 2.0 — Guidelines CDSS & Clinical Case Suggester (TypeScript)
...
interface CaseAnalysisItem {
  ...
  matchPct: number;
  reasons: string[];
  cdssAlerts: Array<{ type: string; text: string }>;
  dosingGuide: string[];
}
...
export function openCaseModal(): void {
...
export function closeCaseModal(): void {
  const modal = document.getElementById('clinical-case-modal');
  if (modal) modal.classList.remove('active');
}

export function handleCaseAnalysis(event?: Event): void {
  if (event) event.preventDefault();

  const icdVal = (document.getElementById('case-icd-input') as HTMLInputElement | null)?.value.trim().toUpperCase() || '';
  const problemVal = (document.getElementById('case-problem-input') as HTMLInputElement | null)?.value.trim() || '';

  const egfrVal = parseFloat((document.getElementById('case-egfr') as HTMLInputElement | null)?.value || '') || null;
  const lvefVal = parseFloat((document.getElementById('case-lvef') as HTMLInputElement | null)?.value || '') || null;
  const sbpVal = parseFloat((document.getElementById('case-sbp') as HTMLInputElement | null)?.value || '') || null;
  const kVal = parseFloat((document.getElementById('case-k') as HTMLInputElement | null)?.value || '') || null;

  const prefAsia = (document.getElementById('case-pref-asia') as HTMLInputElement | null)?.checked || false;
  const prefMoh = (document.getElementById('case-pref-moh') as HTMLInputElement | null)?.checked || false;
  const prefPc = (document.getElementById('case-pref-pc') as HTMLInputElement | null)?.checked || false;
  ...
  const scoredList: CaseAnalysisItem[] = studies.map(study => {
    let score = 0;
    const reasons: string[] = [];
    const cdssAlerts: Array<{ type: string; text: string }> = [];
    ...
    if (study.year && Number(study.year) >= 2024) {
      score += 10;
...
function getScoreColor(pct: number): { bg: string; text: string; border: string } {
...
export function copyEbmClinicalNote(idx: number): void {
...
export function copyAllEbmClinicalNotes(): void {
