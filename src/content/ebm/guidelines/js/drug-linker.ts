/**
 * CliniPortal 2.0 — Drug Interaction Linker Engine (TypeScript)
...
export interface DrugInteractionItem {
  ...
  group1: string | null;
  group2: string | null;
  severity: 'contraindicated' | 'major' | 'synergistic' | 'minor';
...
export interface StudyInteractionResult {
  detectedDrugs: DrugAliasItem[];
...
export const DRUG_INTERACTIONS: DrugInteractionItem[] = [
  {
...
export function extractDrugsFromText(text?: string): DrugAliasItem[] {
  if (!text || typeof text !== 'string') return [];
  const lower = text.toLowerCase();
  const found: DrugAliasItem[] = [];
...
export function detectStudyInteractions(study: Study): StudyInteractionResult {
  ...
  const matchedInteractions: DrugInteractionItem[] = [];
  ...
  const priority: Record<string, number> = { contraindicated: 1, major: 2, synergistic: 3, minor: 4 };
...
export function renderDrugInteractionBadge(study: Study): string {
  const { interactions } = detectStudyInteractions(study);
...
export function renderModalContent(study: Study): string {
...
export function openModal(studyId: string): void {
