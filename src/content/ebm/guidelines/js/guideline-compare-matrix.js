/**
 * guideline-compare-matrix.js
 * Quản lý Ma trận Đối Sánh 3D Multi-Compare Matrix & Floating Compare Action Bar
...
(function () {
  ...
  let compareStudyIds = new Set();

  function addToCompare(studyId) {
    if (!studyId) return;
  ...
  function removeFromCompare(studyId) {
            ...
            <tr>
              ...
              ${selectedStudies.map(s => `
                <td style="border: 1px solid var(--border-light); padding: 10px;">
                  <span style="display:inline-block; padding: 2px 8px; border-radius: 12px; font-size: 0.72rem; font-weight: 700; ${s.impact === 'practice-changing' ? 'background:#fef2f2; color:#dc2626;' : 'background:#eff6ff; color:#2563eb;'}">
                    ${s.impact === 'practice-changing' ? '🔥 Practice-Changing' : 'ℹ️ Informative'}
                  </span>
                </td>
              `).join('')}
            </tr>
            <tr>
              <td style="font-weight: 700; background: var(--surface-2); border: 1px solid var(--border-light); padding: 10px;">💡 Tóm tắt khuyến cáo chính</td>
              ${selectedStudies.map(s => `<td style="border: 1px solid var(--border-light); padding: 10px; font-size:0.78rem; line-height:1.5;">${s.summary ? s.summary.replace(/<[^>]*>/g, '') : 'N/A'}</td>`).join('')}
            </tr>
            <tr>
