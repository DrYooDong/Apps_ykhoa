/**
 * EBM Outcome Matrix Card Renderer (ebm-outcome-card.ts)
 * Phân hệ Y học chứng cứ (EBM) | CliniPortal
 * Hiển thị bảng đối sánh kết cục lâm sàng (Primary, Composite, Surrogate, Safety)
 */

export interface OutcomeItem {
  name: string;
  category: 'primary' | 'composite' | 'surrogate' | 'safety' | string;
  val: number | string;
  metric?: string; // 'HR' | 'RR' | 'OR'
  low?: number;
  high?: number;
}
...
export class EBMOutcomeCard {
  public static render(containerId: string, data: OutcomeCardData): void {
    const container = document.getElementById(containerId);
    if (!container || !data || !data.outcomes) return;

    const catBadgeMap: Record<string, { label: string; bg: string; color: string; border: string }> = {
    ...
    data.outcomes.forEach((item) => {
      ...
      const isSignificant = (item.low !== undefined && item.high !== undefined) 
        ? !(item.low <= 1.0 && item.high >= 1.0) 
        : true;
...
if (typeof window !== 'undefined') {
  (window as any).EBMOutcomeCard = EBMOutcomeCard;
}
