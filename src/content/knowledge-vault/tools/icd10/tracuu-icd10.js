...
(function () {
    ...
    function sendCaseToDocSpaceSOAP() {
        if (caseItems.length === 0) {
            showToast('Chưa có mã bệnh nào trong Hồ sơ ca bệnh!');
            return;
        }

        const primary = caseItems.find(c => c.role === 'primary') || caseItems[0];
        const secondaries = caseItems.filter(c => c.role === 'secondary' && c !== primary);

        const caseData = {
            primaryCode: primary.code,
            primaryName: primary.nameVi,
            secondaries: secondaries.map(s => ({ code: s.code, name: s.nameVi })),
            allCodes: caseItems.map(c => c.code),
            timestamp: new Date().toISOString()
        };

        try {
            localStorage.setItem('dsp_pending_icd_case', JSON.stringify(caseData));
            showToast('Đang chuyển ca bệnh sang DocSpace SOAP...');
            setTimeout(() => {
                window.location.href = '../../../docspace/index.html#/soap?from_icd_case=1';
            }, 300);
        } catch (e) {
            showToast('Không thể lưu ca bệnh vào bộ nhớ tạm: ' + e.message);
        }
    }
