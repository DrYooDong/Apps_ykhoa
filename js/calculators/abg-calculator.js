      ...
      document.addEventListener('DOMContentLoaded', () => {
        ...
        const inDuration = document.getElementById('abg-duration');
        ...
        const inUrineCl = document.getElementById('abg-urine-cl');
        ...
        const boxWarnings = document.getElementById('step7-warnings');
        ...
        function analyzeABG() {
          ...
          const duration = inDuration?.value || 'acute';
          ...
          const urineCl = inUrineCl?.value ? parseFloat(inUrineCl.value) : null;
          ...
          if (errorMargin > 0.15) {
            ...
            domPri.textContent = "-"; domComp.textContent = "-"; domAg.textContent = "-"; 
            boxDelta.style.display = "none"; boxCauses.style.display = "none"; boxWarnings.style.display = "none";
            return;
          } else {
          ...
          if (primary === "Toan chuyển hóa" || disorders.includes("metabolicAcidosis")) {
            ...
            if (pco2 > expectedPco2 + 2) { 
              compText += "➔ <strong>Kèm Toan hô hấp phối hợp.</strong>"; 
              ...
              compText += "➔ <strong>Kèm Kiềm hô hấp phối hợp.</strong>"; 
          ...
          else if (disorders.includes("respiratoryAcidosis")) {
            const deltaPco2 = pco2 - 40;
            if (deltaPco2 > 0) {
              if (duration === 'acute') {
                expectedHco3 = 24 + 1 * (deltaPco2 / 10);
                compText = `HCO₃⁻ kỳ vọng (Toan hô hấp cấp, tăng 1 mEq/L cho mỗi 10 mmHg pCO₂): <strong>${expectedHco3.toFixed(1)}</strong> ± 3 mEq/L.<br>`;
                if (hco3 > expectedHco3 + 3) {
                  compText += "➔ <strong>Kèm Kiềm chuyển hóa phối hợp.</strong>";
                  if (!disorders.includes("metabolicAlkalosis")) disorders.push("metabolicAlkalosis");
                  conclusions.push("Kiềm chuyển hóa phối hợp");
                } else if (hco3 < expectedHco3 - 3) {
                  compText += "➔ <strong>Kèm Toan chuyển hóa phối hợp.</strong>";
                  if (!disorders.includes("metabolicAcidosis")) disorders.push("metabolicAcidosis");
                  conclusions.push("Toan chuyển hóa phối hợp");
                } else {
                  compText += "➔ Đáp ứng bù trừ thận phù hợp với toan hô hấp cấp.";
                }
              } else {
                expectedHco3 = 24 + 3.5 * (deltaPco2 / 10);
                compText = `HCO₃⁻ kỳ vọng (Toan hô hấp mạn, tăng 3.5 mEq/L cho mỗi 10 mmHg pCO₂): <strong>${expectedHco3.toFixed(1)}</strong> ± 3 mEq/L.<br>`;
                if (hco3 > expectedHco3 + 3) {
                  compText += "➔ <strong>Kèm Kiềm chuyển hóa phối hợp.</strong>";
                  if (!disorders.includes("metabolicAlkalosis")) disorders.push("metabolicAlkalosis");
                  conclusions.push("Kiềm chuyển hóa phối hợp");
                  ...
                  compText += "➔ Đáp ứng bù trừ thận phù hợp với toan hô hấp mạn.";
                }
              }
            } else {
              compText = "pCO₂ không tăng tương xứng với chẩn đoán toan hô hấp.";
            }
          }
          else if (disorders.includes("respiratoryAlkalosis")) {
            const deltaPco2 = 40 - pco2;
            if (deltaPco2 > 0) {
              if (duration === 'acute') {
                expectedHco3 = 24 - 2 * (deltaPco2 / 10);
                compText = `HCO₃⁻ kỳ vọng (Kiềm hô hấp cấp, giảm 2 mEq/L cho mỗi 10 mmHg pCO₂): <strong>${expectedHco3.toFixed(1)}</strong> ± 2 mEq/L.<br>`;
                if (hco3 > expectedHco3 + 2) {
                  ...
                  compText += "➔ Đáp ứng bù trừ thận phù hợp với kiềm hô hấp cấp.";
                }
              } else {
                expectedHco3 = 24 - 5 * (deltaPco2 / 10);
                if (expectedHco3 < 12) expectedHco3 = 12;
                compText = `HCO₃⁻ kỳ vọng (Kiềm hô hấp mạn, giảm 5 mEq/L cho mỗi 10 mmHg pCO₂): <strong>${expectedHco3.toFixed(1)}</strong> ± 2 mEq/L.<br>`;
                if (hco3 > expectedHco3 + 2) {
                  compText += "➔ <strong>Kèm Kiềm chuyển hóa phối hợp.</strong>";
                  if (!disorders.includes("metabolicAlkalosis")) disorders.push("metabolicAlkalosis");
                  conclusions.push("Kiềm chuyển hóa phối hợp");
                  ...
                  compText += "➔ Đáp ứng bù trừ thận phù hợp với kiềm hô hấp mạn.";
                }
              }
            } else {
              compText = "pCO₂ không giảm tương xứng với chẩn đoán kiềm hô hấp.";
            }
          }
          ...
          // BƯỚC 7: CẢNH BÁO LÂM SÀNG & NGUY CƠ ĐỘNG
          let warningMessages = [];
          if (disorders.includes("respiratoryAcidosis")) {
            warningMessages.push("⚠️ <strong>Toan hô hấp:</strong> Đây là vấn đề THÔNG KHÍ, không phải thiếu bicarbonate. Điều trị chính là cải thiện thông khí (NIV, đặt nội khí quản nếu có chỉ định). <strong>KHÔNG nên bù bicarbonate thường quy</strong> vì không giải quyết được gốc vấn đề.");
            warningMessages.push("🚨 <strong>Dấu hiệu nguy hiểm cần theo dõi sát:</strong> Lơ mơ, mệt cơ hô hấp, giảm SpO₂, pH thấp, PaCO₂ tăng nhanh.");
          }
          if (disorders.includes("respiratoryAlkalosis")) {
            warningMessages.push("⚠️ <strong>Bẫy kiềm hô hấp:</strong> Đừng quy kết lo âu/panic quá sớm. Kiềm hô hấp có thể là dấu hiệu sớm của <strong>Nhiễm trùng huyết (Sepsis)</strong>, <strong>Thuyên tắc phổi (PE)</strong>, thiếu oxy hoặc ngộ độc Salicylate.");
            warningMessages.push("🚨 <strong>Cảnh báo:</strong> Tuyệt đối không cho thở lại vào túi giấy khi chưa loại trừ các nguyên nhân nguy hiểm trên (như thiếu oxy, PE, sepsis).");
            warningMessages.push("🚨 <strong>Dấu hiệu nguy hiểm:</strong> Khó thở thực sự, SpO₂ giảm, đau ngực, ho ra máu, ngất, sốt, rét run, tụt huyết áp, lơ mơ, tiền sử phẫu thuật/bất động/huyết khối.");
          }
          if (disorders.includes("metabolicAlkalosis")) {
            warningMessages.push("💡 <strong>Kiềm chuyển hóa:</strong> Hạ kali vừa là hậu quả vừa duy trì kiềm chuyển hóa. Trong rất nhiều ca, <strong>KCl chính là thuốc điều trị kiềm</strong>.");
            if (urineCl === null) {
              warningMessages.push("⚠️ <strong>Chỉ định xét nghiệm Chloride niệu (UCl⁻):</strong> Đây là xét nghiệm nhỏ nhưng là chìa khóa định hướng xử trí (phân loại đáp ứng chloride hay không).");
            } else if (urineCl < 20) {
              warningMessages.push("✅ <strong>Kiềm chuyển hóa ĐÁP ỨNG chloride (UCl⁻ < 20 mEq/L):</strong> Gợi ý mất dịch do nôn ói, hút dịch dạ dày, giảm thể tích hoặc lợi tiểu đã qua tác dụng. ➔ <strong>Xử trí:</strong> Bù dịch NaCl 0.9% kèm KCl, khôi phục thể tích tuần hoàn.");
            } else {
              warningMessages.push("⚠️ <strong>Kiềm chuyển hóa KHÔNG ĐÁP ỨNG chloride (UCl⁻ ≥ 20 mEq/L):</strong> Gợi ý lợi tiểu còn tác dụng, cường aldosterone, hội chứng Bartter/Gitelman hoặc hạ kali kéo dài. ➔ <strong>Xử trí:</strong> Ngưng thuốc gây ra, bù tích cực KCl. <em>Lưu ý: Nếu bệnh nhân đang quá tải dịch, không truyền saline máy móc (cân nhắc Acetazolamide)</em>.");
            }
          }

          if (warningMessages.length > 0) {
            boxWarnings.style.display = "block";
            domWarnings.innerHTML = "<ul style='margin: 0; padding-left: 1.2rem;'>" + warningMessages.map(m => `<li style='margin-bottom: 0.4rem;'>${m}</li>`).join("") + "</ul>";
          } else {
            boxWarnings.style.display = "none";
          }
        ...
        document.querySelectorAll('input, select').forEach(el => {
          el.addEventListener('input', analyzeABG);
          el.addEventListener('change', analyzeABG);
        });

        document.getElementById('btn-reset').addEventListener('click', () => {
          inPh.value = "7.40"; inPco2.value = "40"; inHco3.value = "24";
          inNa.value = "140"; inCl.value = "104"; inAlb.value = "4.0";
          if (inDuration) inDuration.value = "acute";
