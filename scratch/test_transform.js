...
data.forEach(item => {
  ...
  if (nonexistent.includes(item.file)) {
    console.log(`Clearing 404 nonexistent file for [${item.id}]: "${item.file}"`);
    item.file = '';
  }

  // Clear PDF names in file column
  if (item.file.toLowerCase().endsWith('.pdf')) {
    console.log(`Clearing raw PDF name in file column for [${item.id}]: "${item.file}"`);
    item.file = '';
  }
});
...
// Row 10 (guideline_2023_byt_benh_phoi_mo_ke) & Row 67 (study_2023_byt_benh_phoi_mo_ke)
...
if (gIld && sIld) {
  ...
  gIld.drug = sIld.drug || gIld.drug;
  gIld.specialty = sIld.specialty || gIld.specialty;
  data = data.filter(d => d.id !== 'study_2023_byt_benh_phoi_mo_ke');
  console.log('Merged Bệnh phổi mô kẽ into guideline_2023_byt_benh_phoi_mo_ke and removed duplicate study_2023_byt_benh_phoi_mo_ke');
}

// B. AHA/ACC 2025 High Blood Pressure:
// Row 28 (guideline_2025_aha_acc_hypertension) & Row 60 (study_1784201407856_17hl4qiwi)
...
if (gHtn && sHtn) {
  ...
  console.log('Merged AHA/ACC Hypertension into guideline_2025_aha_acc_hypertension and removed duplicate study_1784201407856_17hl4qiwi');
}

// C. FOB in Sputum-Negative TB (Cureus 2026):
// Row 57 (study_1783915156968_8wsq0fp4m) & Row 75 (study_2026_fob_sputum_negative_tb_chauda)
...
if (sFob1 && sFob2) {
  ...
  console.log('Merged FOB TB into study_2026_fob_sputum_negative_tb_chauda and removed duplicate study_1783915156968_8wsq0fp4m');
}

// 3. Add missing records for rich kho-guideline files:
// A. 2026-aha-acc-ckm-syndrome.html
if (!data.some(d => d.file && d.file.includes('2026-aha-acc-ckm-syndrome.html'))) {
  ...
  console.log('Added guideline_2026_aha_acc_ada_asn_ckm_syndrome linked to 2026-aha-acc-ckm-syndrome.html');
}

// B. 2021-ssc-soc-nhiem-khuan-sepsis3.html
if (!data.some(d => d.file && d.file.includes('2021-ssc-soc-nhiem-khuan-sepsis3.html'))) {
  ...
  console.log('Added guideline_2021_ssc_soc_nhiem_khuan_sepsis3 linked to 2021-ssc-soc-nhiem-khuan-sepsis3.html');
}

console.log(`Final rows count: ${data.length}`);

// Check all file links in data vs actual khoFiles
console.log('\n=== FINAL VERIFICATION OF KHO FILES LINKING ===');
const allReferencedKho = new Set();
data.forEach(d => {
  if (d.file && d.file.trim()) {
    const fn = path.basename(d.file.trim());
    if (khoFiles.includes(fn)) {
      allReferencedKho.add(fn);
    } else {
      console.log(`ERROR: Still references missing file "${d.file}" at [${d.id}]`);
    }
  }
});

console.log(`Total valid kho files referenced in CSV: ${allReferencedKho.size} / ${khoFiles.length}`);
khoFiles.forEach(kf => {
  if (!allReferencedKho.has(kf)) {
    console.log(`Note: Kho file not directly in 'file' column: ${kf}`);
  }
});
