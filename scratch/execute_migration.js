...
console.log('=== BẮT ĐẦU QUY TRÌNH TÁI CẤU TRÚC KNOWLEDGE VAULT ===');
...
console.log(`Đã phát hiện tổng cộng: ${allFiles.length} files nguồn.`);
...
function classifyFile(f) {
  ...
  // 1. Kho Dịch tễ học
  if (
    fnLower.includes('dịch tễ') || 
    fnLower.includes('định nghĩa & dịch tễ') || 
    fnLower.startsWith('1. tổng quan & dịch tễ') || 
    fnLower.startsWith('1. định nghĩa') || 
    fnLower.startsWith('1. tổng quan')
  ) {
    kho = 'Kho dịch tễ học';
    prefix = 'DTH';
    part = 'P1';
  ...
  let cleanSubject = diseaseName.trim();
  if (kho === 'Kho chưa lọc' || (prefix === 'DTH' && f.relToTarget.includes('1. Tác nhân'))) {
    cleanSubject = fileNameNoExt.replace(/^MOC\s*-\s*/i, '').replace(/^Bệnh lý\s*-\s*/i, '').trim();
  }

  let newFileName = `${prefix}_${cleanSubject}_${part}.md`.replace(/\s+/g, ' ');
  ...
  return {
    ...
    part,
    newFileName,
    targetRelDir: path.join(kho, specialty),
    originalRel: f.relToVault,
    fullPath: f.fullPath
  };
}

const list = allFiles.map(classifyFile);

// Resolve collisions deterministically
const destMap = {};
list.forEach(item => {
  let fullTargetRel = path.join(item.targetRelDir, item.newFileName);
  if (destMap[fullTargetRel]) {
    let count = 2;
    let candidate = path.join(item.targetRelDir, `${item.prefix}_${item.subject}_P${count}.md`);
    while (destMap[candidate]) {
      count++;
      candidate = path.join(item.targetRelDir, `${item.prefix}_${item.subject}_P${count}.md`);
    }
    item.part = `P${count}`;
    item.newFileName = `${item.prefix}_${item.subject}_P${count}.md`;
    fullTargetRel = candidate;
  }
  destMap[fullTargetRel] = item.originalRel;
  item.finalRelPath = fullTargetRel;
  item.finalFullPath = path.join(base, fullTargetRel);
});
...
// Execute file moves
console.log('\nTiến hành di chuyển và đổi tên các file...');
let successCount = 0;
let errorCount = 0;
...
list.forEach(item => {
  try {
    ...
    console.error(`Lỗi khi di chuyển ${item.fullPath} -> ${item.finalFullPath}:`, err.message);
    errorCount++;
  }
});
...
console.log('\n=== TỔNG KẾT TÁI CẤU TRÚC 7 KHO THÀNH CÔNG ===');
