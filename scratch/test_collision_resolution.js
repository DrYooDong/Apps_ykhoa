...
// Check collisions within same destination directory
const destMap = {};
const collisions = [];

list.forEach(item => {
  let fullTarget = path.join(item.targetRelDir, item.newFileName);
  if (destMap[fullTarget]) {
    // We have a collision, need disambiguation!
    let count = 2;
...
const summary = {};
list.forEach(item => {
  summary[item.kho] = (summary[item.kho] || 0) + 1;
});
console.log('\n--- Thống kê 7 Kho ---');
