/**
 * Arrhythmia Classification Engine & Pro Studio CDSS Core (TypeScript Module)
...
export interface ArrhythmiaEntity {
  id: string;
  label: string;
  rarity: string;
  ecgCriteria: string;
...
export interface ArrhythmiaDB {
  schemaVersion: string;
  entities: ArrhythmiaEntity[];
}
...
export class ArrhythmiaEngineService {
  private dbData: ArrhythmiaDB | null = null;
  private flowchartData: any = null;
  private isInitialized: boolean = false;

  public async init(
    dbPath: string = './arrhythmia-classification-db.v2.json',
    flowchartPath: string = './roi-loan-nhip-flowchart.json'
  ): Promise<boolean> {
    if (this.isInitialized && this.dbData) return true;

    try {
      const win = window as any;
      if (win.ARRHYTHMIA_DB_DATA) {
        this.dbData = win.ARRHYTHMIA_DB_DATA;
      } else {
        const resDb = await fetch(dbPath);
        if (!resDb.ok) throw new Error(`HTTP Error ${resDb.status} khi tải DB.`);
        this.dbData = await resDb.json();
      }

      const validationErrors = this.validateDB(this.dbData);
      if (validationErrors.length > 0) {
        console.warn('[ArrhythmiaEngine] Cảnh báo kiểm định schema:', validationErrors);
      }

      if (win.ARRHYTHMIA_FLOWCHART_DATA) {
        this.flowchartData = win.ARRHYTHMIA_FLOWCHART_DATA;
      } else {
        try {
          const resFc = await fetch(flowchartPath);
          if (resFc.ok) this.flowchartData = await resFc.json();
        } catch {
          this.flowchartData = { nodes: [], edges: [], redFlagsPanel: [] };
        }
      }
      ...
      this.renderFatalError(error.message);
      return false;
    }
  }

  public validateDB(data: any): string[] {
    const errors: string[] = [];
  ...
  public filterByAxes(criteria: Record<string, string> = {}): ArrhythmiaEntity[] {
    if (!this.dbData || !Array.isArray(this.dbData.entities)) return [];
    return this.dbData.entities.filter(ent => {
  ...
  public searchEntities(query: string = ''): ArrhythmiaEntity[] {
    if (!this.dbData || !Array.isArray(this.dbData.entities)) return [];
    if (!query || !query.trim()) return this.dbData.entities;

    const qNorm = this.normalizeVietnamese(query);
    return this.dbData.entities.filter(ent => {
      const labelNorm = this.normalizeVietnamese(ent.label);
      const critNorm = this.normalizeVietnamese(ent.ecgCriteria);
      const aliasNorm = Array.isArray(ent.aliases) ? ent.aliases.map(a => this.normalizeVietnamese(a)).join(' ') : '';
      const featNorm = Array.isArray(ent.ecgFeatures)
        ? ent.ecgFeatures.map(f => this.normalizeVietnamese(f)).join(' ')
        : '';

      return (
        labelNorm.includes(qNorm) ||
        critNorm.includes(qNorm) ||
        aliasNorm.includes(qNorm) ||
        featNorm.includes(qNorm)
      );
    });
  }

  public getEntityById(id: string): ArrhythmiaEntity | null {
    if (!this.dbData || !Array.isArray(this.dbData.entities)) return null;
    return this.dbData.entities.find(e => e.id === id) || null;
  }

  public getAllEntities(): ArrhythmiaEntity[] {
    return this.dbData && Array.isArray(this.dbData.entities) ? this.dbData.entities : [];
  }

  public getRarityInfo(rarityCode: string): { label: string; badgeClass: string } {
  ...
  public calculateQTc(
    qt: number | string,
    rrOrHr: number | string,
    isHr: boolean = false,
    formula: string = 'bazett'
  ): number {
    const qtNum = typeof qt === 'number' ? qt : parseFloat(qt);
    ...
    if (isHr) {
      hr = typeof rrOrHr === 'number' ? rrOrHr : parseFloat(rrOrHr);
      if (isNaN(hr) || hr <= 0) return 0;
      rrMs = 60000 / hr;
    } else {
      rrMs = typeof rrOrHr === 'number' ? rrOrHr : parseFloat(rrOrHr);
  ...
  public evaluateQTcRisk(qtc: number, gender: string = 'male'): QTcRiskResult {
    if (!qtc || qtc <= 0) {
      return {
        category: 'unknown',
    ...
    if (qtc >= 500) {
      return {
        ...
        recommendation:
          'CẢNH BÁO KHẨN CẤP: Nguy cơ xuất hiện nhịp nhanh thất đa hình xoắn đỉnh (Torsades de Pointes) cực cao! Tiêm tĩnh mạch Magie sulfat 2g IV, bù K+ >= 4.0 mmol/L, ngưng thuốc kéo dài QT.'
      };
      ...
      return {
        ...
        recommendation:
          'QTc vượt ngưỡng an toàn lâm sàng. Rà soát danh sách thuốc đang dùng (kháng sinh Macrolide/Fluoroquinolone, chống loạn nhịp, hướng thần).'
      };
      ...
      return {
        ...
        recommendation:
          'Khoảng QTc ở mức ranh giới. Theo dõi điện giải và đánh giá nguy cơ tim mạch tổng thể.'
      };
    } else if (qtc < 340) {
      return {
        category: 'short_qt',
        ...
        colorClass: 'risk-warning',
        recommendation:
          'Cảnh báo Hội chứng QT ngắn (SQTS). Nguy cơ xuất hiện rung nhĩ hoặc rung thất ở người trẻ.'
      };
    } else {
      return {
        category: 'normal',
        ...
        recommendation:
          'Khoảng QTc bình thường theo giới tính, nguy cơ loạn nhịp thất do tái cực thấp.'
      };
    }
  }

  public evaluateActiveParams(p: any): any {
    if (!this.dbData || !Array.isArray(this.dbData.entities)) return null;
    ...
    const primaryEnt = this.getEntityById(matchedId) || this.dbData.entities[0];
    const protocol = this.generateEmergencyProtocol(matchedId);
  ...
  public generateEmergencyProtocol(diagId: string): EmergencyProtocol {
  ...
  public renderMatrixCards(containerId: string, entities: ArrhythmiaEntity[]): void {
    ...
    const cardsHtml = entities
      .map(ent => {
        const rarity = this.getRarityInfo(ent.rarity);
        const tagsList = Object.entries(ent.tags || {})
          .map(([, v]) => `<span class="ent-tag">${v}</span>`)
          .join('');
        ...
        <div class="arrhythmia-card" data-id="${ent.id}">
          <div class="card-top">
            ...
            ${
              ent.redFlags && ent.redFlags.length > 0
                ? '<span class="flag-alert-icon"><i class="fa-solid fa-triangle-exclamation"></i> Cờ đỏ</span>'
                : ''
            }
          </div>
  ...
  public showEntityModal(entityId: string): void {
    ...
    const rarity = this.getRarityInfo(entity.rarity);
    const aliasesHtml =
    ...
    const featuresHtml =
      entity.ecgFeatures && entity.ecgFeatures.length > 0
        ? entity.ecgFeatures
            .map(f => `<li><i class="fa-solid fa-check-circle text-success"></i> ${f}</li>`)
            .join('')
        : '<li>Chưa cập nhật</li>';
    ...
    const redFlagsHtml =
      entity.redFlags && entity.redFlags.length > 0
        ? entity.redFlags
            .map(
              rf =>
                `<div class="rf-alert-item"><i class="fa-solid fa-triangle-exclamation"></i> <span>${rf}</span></div>`
            )
            .join('')
        : '<p class="text-muted">Không có dấu hiệu cảnh báo cờ đỏ đặc thù.</p>';

    const protocol = this.generateEmergencyProtocol(entity.id);
  ...
  private renderFatalError(msg: string): void {
...
export const ArrhythmiaEngine = new ArrhythmiaEngineService();

// Global binding
if (typeof window !== 'undefined') {
  (window as any).ArrhythmiaEngine = ArrhythmiaEngine;
}
