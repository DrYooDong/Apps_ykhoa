...
export const DIAGNOSTIC_CHAIN_DATABASE: Record<string, DiseaseReactionChainDefinition> = {
  ...
  'viem_phoi': {
    icdCode: 'J18.9',
    icdPrefixes: ['J13', 'J14', 'J15', 'J16', 'J18'],
    diseaseName: 'Viêm Phổi Mắc Phải Cộng Đồng (CAP)',
    specialty: 'Hô Hấp / Nhiễm Khuẩn',
    severity: 'urgent',
    summary: 'Nhiễm trùng cấp tính nhu mô phổi xuất hiện ngoài bệnh viện. Đánh giá độ nặng bằng thang điểm CURB-65 hoặc PSI để quyết định ngoại trú / nội trú / ICU.',
    goldStandard: 'X-quang / CT ngực có thâm nhiễm mới + Triệu chứng nhiễm trùng hô hấp dưới cấp tính',
    criteriaRule: {
      mandatoryIds: ['cap_img'],
      minMajorRequired: 1,
      minMinorRequired: 1,
      ruleDescription: 'Bắt buộc có hình ảnh tổn thương thâm nhiễm mới trên X-quang/CT ngực + Ít nhất 1 triệu chứng cơ năng + 1 triệu chứng thực thể/CLS'
    },
    criteria: [
      { id: 'cap_img', type: 'mandatory', label: 'Hình ảnh thâm nhiễm / đông đặc mới trên X-quang ngực thẳng hoặc CT', description: 'Đám mờ phế nang, hình ảnh khí phế quản đồ (Air bronchogram)', sourceGuideline: 'ATS/IDSA 2019' },
      { id: 'cap_fever', type: 'major', label: 'Sốt ≥ 38.0°C hoặc hạ thân nhiệt < 36.0°C ở người cao tuổi', sourceGuideline: 'Bộ Y Tế' },
      { id: 'cap_cough', type: 'major', label: 'Ho khạc đờm mủ / đờm rỉ sắt / đờm đục', sourceGuideline: 'ATS/IDSA' },
      { id: 'cap_rales', type: 'major', label: 'Khám phổi: Ran nổ, ran ẩm khu trú, hội chứng đông đặc (rung thanh tăng, gõ đục, RRPN giảm)', sourceGuideline: 'Bộ Y Tế' },
      { id: 'cap_leuko', type: 'lab', label: 'Bạch cầu máu > 10.000/μL hoặc < 4.000/μL hoặc Neutrophil > 80%', labThreshold: 'WBC > 10 G/L' },
      { id: 'cap_crp', type: 'lab', label: 'CRP > 20 mg/L hoặc Procalcitonin > 0.25 ng/mL', labThreshold: 'CRP > 20 mg/L' },
      { id: 'cap_hypox', type: 'minor', label: 'Khó thở, thở nhanh > 20 l/p, SpO2 < 95% khí phòng', sourceGuideline: 'CURB-65' }
    ],
    protocol: {
      title: 'Phác đồ Điều trị Viêm phổi Mắc phải Cộng đồng (Bộ Y Tế / ATS/IDSA 2024)',
      guideline: 'Hướng dẫn chẩn đoán và điều trị CAP - Bộ Y Tế & ATS/IDSA',
      targetGoals: ['Kiểm soát nhiễm trùng trong 48-72h', 'Cắt sốt, giảm khó thở, SpO2 ≥ 95%', 'Ngăn ngừa suy hô hấp tiến triển & Sốc nhiễm trùng'],
      initialManagement: [
        'Phân tầng CURB-65: 0-1 (Ngoại trú), 2 (Nội trú khoa thường), ≥ 3 (Cân nhắc ICU)',
        'Cấy máu và cấy đờm trước khi dùng liều kháng sinh đầu tiên (nếu nội trú)',
        'Dùng kháng sinh sớm trong ≤ 4 giờ đầu từ khi nhập viện'
      ],
      firstLineDrugs: [
        { drugName: 'Amoxicillin/Clavulanate', class: 'Aminopenicillin + Beta-lactamase inhibitor', route: 'Uống / Tiêm TMC', dosage: '1g (875/125mg) x 2-3 lần/ngày hoặc 1.2g IV x 3 lần/ngày', frequency: 'Mỗi 8-12 giờ', instructions: 'Dùng đầu bữa ăn (uống) hoặc tiêm TM chậm', isFirstLine: true },
        { drugName: 'Azithromycin', class: 'Macrolide', route: 'Uống', dosage: '500mg ngày 1, sau đó 250mg/ngày (ngày 2-5)', frequency: '1 lần/ngày', instructions: 'Uống trước ăn 1h hoặc sau ăn 2h. Phối hợp Beta-lactam để phủ vi khuẩn không điển hình', isFirstLine: true },
        { drugName: 'Ceftriaxone', class: 'Cephalosporin thế hệ 3', route: 'Tiêm TM', dosage: '1g - 2g x 1 lần/ngày', frequency: 'Mỗi 24 giờ', instructions: 'Dùng cho bệnh nhân CAP nội trú khoa phòng', isFirstLine: true }
      ],
      secondLineDrugs: [
        { drugName: 'Levofloxacin', class: 'Respiratory Fluoroquinolone', route: 'Uống / Tiêm truyền IV', dosage: '750mg x 1 lần/ngày (hoặc 500mg x 1 lần/ngày)', frequency: 'Mỗi 24 giờ', instructions: 'Dành cho BN dị ứng Beta-lactam hoặc thất bại điều trị bậc 1. Chỉnh liều theo eGFR', isFirstLine: false }
      ],
      supportiveCare: ['Thở Oxy duy trì SpO2 94-98% (88-92% nếu có COPD)', 'Bù dịch điện giải, vỗ rung dẫn lưu tư thế', 'Hạ sốt Paracetamol 500mg khi T ≥ 38.5°C']
    },
    complications: [
      { name: 'Tràn dịch màng phổi cận viêm / Mủ màng phổi', timeframe: 'subacute_7d', warningSigns: 'Sốt dai dẳng sau 72h dùng KS, đau ngực màng phổi tăng, X-quang có góc sườn hoành tù', preventiveAction: 'Chọc dò dịch màng phổi chẩn đoán + Siêu âm ngực', onCallAlertText: 'BN sốt dai dẳng sau KS 72h: Kiểm tra siêu âm màng phổi loại trừ tràn dịch mủ' },
      { name: 'Suy hô hấp cấp / ARDS', timeframe: 'acute_24h', warningSigns: 'Khó thở tăng nhanh, SpO2 tụt < 90%, PaO2/FiO2 < 300', preventiveAction: 'Thở HFNC / CPAP / Đặt NKQ sớm', onCallAlertText: 'Báo động ARDS: SpO2 giảm, thở co kéo ➔ Hội chẩn ICU ngay' },
      { name: 'Sốc nhiễm trùng (Septic Shock)', timeframe: 'acute_24h', warningSigns: 'Huyết áp tụt (HA tâm thu < 90 mmHg hoặc MAP < 65), Lactate máu > 2 mmol/L', preventiveAction: 'Bù dịch 30ml/kg Ringer Lactate trong 3h đầu + Vận mạch Noradrenaline', onCallAlertText: 'HA tụt + Sốt: Kích hoạt Bundle Sepsis 1h' }
    ],
    monitoringLabs: ['Công thức máu & CRP/Procalcitonin sau 48-72h', 'Khí máu động mạch (ABG) nếu SpO2 < 92%', 'X-quang ngực kiểm tra sau 5-7 ngày'],
    vaultPathways: [
      { khoCode: 'TC', khoName: 'Kho Lâm Sàng', articleTitle: 'Tiếp cận Ho & Khó thở cấp', searchKeyword: 'khó thở' },
      { khoCode: 'CD', khoName: 'Kho Tiêu Chuẩn CĐ', articleTitle: 'Tiêu chuẩn chẩn đoán Viêm phổi CAP/HAP', searchKeyword: 'viêm phổi' },
      { khoCode: 'CLS', khoName: 'Kho Cận Lâm Sàng', articleTitle: 'Đọc X-quang phổi & Khí máu động mạch', searchKeyword: 'xquang phổi' },
      { khoCode: 'PDDT', khoName: 'Kho Phác Đồ', articleTitle: 'Phác đồ kháng sinh CAP người lớn Bộ Y Tế', searchKeyword: 'kháng sinh viêm phổi' },
      { khoCode: 'DUOC', khoName: 'Kho Dược', articleTitle: 'Tra cứu Macrolide & Beta-lactamase', searchKeyword: 'amoxicillin clavulanate' },
      { khoCode: 'BC', khoName: 'Kho Biến Chứng', articleTitle: 'Biến chứng mủ màng phổi và ARDS', searchKeyword: 'tràn mủ màng phổi' }
    ]
  },

  // ─────────────────────────────────────────────────────────────────────────────
  // 2. THUYÊN TẮC ĐỘNG MẠCH PHỔI CẤP (PE) - I26
  // ─────────────────────────────────────────────────────────────────────────────
  'thuyen_tac_phoi': {
    icdCode: 'I26.9',
    icdPrefixes: ['I26', 'I26.0', 'I26.9'],
    diseaseName: 'Thuyên Tắc Động Mạch Phổi Cấp (PE)',
    specialty: 'Tim Mạch / Cấp Cứu / Hồi Sức',
    severity: 'emergency',
    summary: 'Tắc nghẽn cấp tính thân hoặc nhánh động mạch phổi do huyết khối từ hệ tĩnh mạch sâu. Cần tính thang điểm Wells/Geneva để phân tầng trước khi chụp CTPA.',
    goldStandard: 'Chụp cắt lớp vi tính động mạch phổi có cản quang (CTPA) thấy khuyết thuốc lòng mạch',
    criteriaRule: {
      mandatoryIds: ['pe_ctpa_or_ddimer'],
      minMajorRequired: 1,
      minMinorRequired: 1,
      ruleDescription: 'Bắt buộc có CTPA (+) hoặc D-dimer tăng cao kết hợp nguy cơ cao trên thang điểm Wells/Geneva + Triệu chứng tim phổi cấp'
    },
    criteria: [
      { id: 'pe_ctpa_or_ddimer', type: 'mandatory', label: 'CTPA có hình ảnh huyết khối làm tắc hoặc khuyết thuốc lòng mạch phổi (hoặc D-dimer > 500 ng/mL nếu Wells thấp)', sourceGuideline: 'ESC 2019' },
      { id: 'pe_dyspnea', type: 'major', label: 'Khó thở khởi phát đột ngột không giải thích được bằng nguyên nhân khác', sourceGuideline: 'ESC 2019' },
      { id: 'pe_pleuritic_pain', type: 'major', label: 'Đau ngực màng phổi kiểu nhói, tăng khi hít sâu hoặc ho', sourceGuideline: 'ESC 2019' },
      { id: 'pe_dvt_signs', type: 'major', label: 'Dấu hiệu huyết khối tĩnh mạch sâu chi dưới (DVT): Sưng đau, nóng đỏ, phù không đối xứng 1 bên chân (Wells +3 điểm)', sourceGuideline: 'Wells Score' },
      { id: 'pe_tachycardia', type: 'minor', label: 'Nhịp tim nhanh > 100 l/p (Wells +1.5 điểm)', sourceGuideline: 'ESC' },
      { id: 'pe_hemoptysis', type: 'minor', label: 'Ho ra máu tươi hoặc đờm vướng máu (Wells +1 điểm)', sourceGuideline: 'Wells' },
      { id: 'pe_ecg_s1q3t3', type: 'imaging', label: 'ECG có hình ảnh tâm phế cấp: Dạng S1Q3T3, T âm V1-V4, Block nhánh phải mới xuất hiện', sourceGuideline: 'ESC' }
    ],
    protocol: {
      title: 'Phác đồ Xử trí Thuyên tắc Động mạch Phổi cấp (ESC 2019 / AHA)',
      guideline: 'ESC Guidelines for the diagnosis and management of acute pulmonary embolism',
      targetGoals: ['Tái thông dòng máu phổi cấp cứu nếu có sốc/tụt HA (High risk)', 'Chống đông ngay lập tức ngăn tạo huyết khối mới', 'Phòng ngừa tái phát và tăng áp phổi mạn tính (CTEPH)'],
      initialManagement: [
        'Đánh giá huyết động: Có sốc hoặc HA tâm thu < 90 mmHg (PE Nguy cơ cao - High Risk)?',
        'Nếu High Risk: Tiêu sợi huyết toàn thân (Alteplase) + Heparin không phân đoạn (UFH)',
        'Nếu Intermediate/Low Risk: Khởi động thuốc chống đông đường uống thế hệ mới (DOAC) hoặc LMWH'
      ],
      firstLineDrugs: [
        { drugName: 'Enoxaparin (Lovenox)', class: 'Heparin trọng lượng phân tử thấp (LMWH)', route: 'Tiêm dưới da (SC)', dosage: '1 mg/kg x 2 lần/ngày (hoặc 1.5 mg/kg x 1 lần/ngày)', frequency: 'Mỗi 12 giờ', instructions: 'Tiêm vùng mỡ bụng. Chỉnh liều còn 1 mg/kg/ngày nếu eGFR < 30', isFirstLine: true },
        { drugName: 'Rivaroxaban (Xarelto)', class: 'DOAC - Ức chế yếu tố Xa', route: 'Uống', dosage: '15mg x 2 lần/ngày trong 21 ngày đầu, sau đó 20mg x 1 lần/ngày', frequency: 'Mỗi 12 giờ (3 tuần đầu)', instructions: 'Uống cùng với thức ăn để tăng hấp thu. Không cần chỉnh liều theo INR', isFirstLine: true },
        { drugName: 'Apixaban (Eliquis)', class: 'DOAC - Ức chế yếu tố Xa', route: 'Uống', dosage: '10mg x 2 lần/ngày trong 7 ngày đầu, sau đó 5mg x 2 lần/ngày', frequency: 'Mỗi 12 giờ', instructions: 'Uống cùng hoặc không cùng thức ăn', isFirstLine: true }
      ],
      secondLineDrugs: [
        { drugName: 'Alteplase (rt-PA)', class: 'Thuốc tiêu sợi huyết', route: 'Tiêm truyền IV', dosage: '100mg truyền tĩnh mạch trong 2 giờ (hoặc 0.6 mg/kg trong 15 phút)', frequency: 'Liều duy nhất cấp cứu', instructions: 'Chỉ định cho PE nguy cơ cao có tụt huyết áp / Sốc tim. Rà soát tuyệt đối bảng kiểm xuất huyết', isFirstLine: false }
      ],
      supportiveCare: ['Thở Oxy duy trì SpO2 > 90%', 'Hạn chế bù dịch quá mức (tránh làm suy thất phải nặng hơn, tối đa 500ml NaCl 0.9%)', 'Bất động tại giường trong giai đoạn cấp']
    },
    complications: [
      { name: 'Suy tim thất phải cấp (Acute Cor Pulmonale)', timeframe: 'acute_24h', warningSigns: 'Tĩnh mạch cổ nổi, phản hồi gan TMC (+), tụt HA, Troponin/BNP tăng vọt', preventiveAction: 'Hạn chế dịch truyền + Dùng vận mạch Noradrenaline / Dobutamine', onCallAlertText: 'PE có dấu hiệu suy thất phải: Siêu âm tim khẩn + Đo BNP/Troponin' },
      { name: 'Xuất huyết lớn do dùng chống đông / tiêu sợi huyết', timeframe: 'acute_24h', warningSigns: 'Nôn ra máu, đi ngoài phân đen, tụt Hb > 2 g/dL, đau đầu dữ dội', preventiveAction: 'Ngừng ngay thuốc chống đông + Chuẩn bị sẵn chất giải độc đặc hiệu (Protamine, Andexanet alfa)', onCallAlertText: 'Báo động xuất huyết trên BN dùng chống đông: Kiểm tra Hct/Hb và tri giác' }
    ],
    monitoringLabs: ['Công thức máu, Hct/Hb mỗi 24-48h', 'Siêu âm tim đánh giá chức năng thất phải (TAPSE, tỷ lệ RV/LV)', 'D-dimer và INR (nếu dùng Kháng Vitamin K)'],
    vaultPathways: [
      { khoCode: 'TC', khoName: 'Kho Lâm Sàng', articleTitle: 'Tiếp cận Đau ngực cấp & Khó thở đột ngột', searchKeyword: 'thuyên tắc phổi' },
      { khoCode: 'CD', khoName: 'Kho Tiêu Chuẩn CĐ', articleTitle: 'Tiêu chuẩn chẩn đoán & Phân tầng Wells PE', searchKeyword: 'tiêu chuẩn pe' },
      { khoCode: 'CLS', khoName: 'Kho Cận Lâm Sàng', articleTitle: 'Chỉ định CTPA và diễn giải D-dimer', searchKeyword: 'd-dimer ctpa' },
      { khoCode: 'PDDT', khoName: 'Kho Phác Đồ', articleTitle: 'Phác đồ Chống đông PE theo ESC 2019', searchKeyword: 'phác đồ thuyên tắc phổi' },
      { khoCode: 'DUOC', khoName: 'Kho Dược', articleTitle: 'Tra cứu liều DOAC và Heparin chuẩn', searchKeyword: 'rivaroxaban enoxaparin' },
      { khoCode: 'BC', khoName: 'Kho Biến Chứng', articleTitle: 'Theo dõi tăng áp động mạch phổi mạn tính (CTEPH)', searchKeyword: 'tăng áp phổi cteph' }
    ]
  },
  ...
  'suy_than': {
    icdCode: 'N18.9',
    icdPrefixes: ['N17', 'N17.0', 'N17.9', 'N18', 'N18.1', 'N18.2', 'N18.3', 'N18.4', 'N18.5', 'N18.9'],
    diseaseName: 'Tổn Thương Thận Cấp (AKI) / Bệnh Thận Mạn (CKD)',
    specialty: 'Thận - Lọc Máu',
    severity: 'urgent',
    summary: 'Suy giảm chức năng lọc của thận cấp tính theo tiêu chuẩn KDIGO (tăng Creatinine ≥ 0.3 mg/dL trong 48h) hoặc mạn tính (eGFR < 60 mL/min/1.73m² kéo dài > 3 tháng).',
    goldStandard: 'Định lượng Creatinine huyết thanh serial + Tính mức lọc cầu thận eGFR (CKD-EPI) & Tỷ lệ UACR nước tiểu',
    criteriaRule: {
      mandatoryIds: ['kdigo_creat_or_egfr'],
      minMajorRequired: 1,
      minMinorRequired: 1,
      ruleDescription: 'Thỏa tiêu chuẩn KDIGO AKI (Creatinine tăng ≥ 26.5 μmol/L trong 48h hoặc nước tiểu < 0.5 mL/kg/h > 6h) HOẶC CKD (eGFR < 60 kéo dài > 3 tháng)'
    },
    criteria: [
      { id: 'kdigo_creat_or_egfr', type: 'mandatory', label: 'Tăng Creatinine huyết thanh ≥ 0.3 mg/dL (≥ 26.5 μmol/L) trong 48h HOẶC Creatinine tăng ≥ 1.5 lần mức nền HOẶC eGFR < 60 mL/min/1.73m² > 3 tháng', sourceGuideline: 'KDIGO 2012/2023' },
      { id: 'aki_oliguria', type: 'major', label: 'Thiểu niệu: Thể tích nước tiểu < 0.5 mL/kg/giờ kéo dài trên 6 giờ', sourceGuideline: 'KDIGO' },
      { id: 'ckd_uacr', type: 'lab', label: 'Tỷ lệ Albumin/Creatinine nước tiểu (UACR) ≥ 30 mg/g (A2 hoặc A3) hoặc Protein niệu 24h > 150 mg', labThreshold: 'UACR ≥ 30 mg/g' },
      { id: 'ckd_us_atrophy', type: 'imaging', label: 'Siêu âm thận: Thận teo nhỏ, phân biệt tủy vỏ kém, nang thận hoặc tăng âm nhu mô thận mạn tính', sourceGuideline: 'KDIGO' },
      { id: 'aki_electrolyte', type: 'lab', label: 'Rối loạn điện giải/toan kiềm: Tăng Kali máu (K+ > 5.5 mmol/L) hoặc Toan chuyển hóa (HCO3- < 18 mmol/L)', labThreshold: 'K+ > 5.5 mmol/L' },
      { id: 'ckd_anemia', type: 'minor', label: 'Thiếu máu mạn tính đẳng sắc đẳng bào do thiếu hụt Erythropoietin (EPO)', sourceGuideline: 'KDIGO Anemia' }
    ],
    protocol: {
      title: 'Phác đồ Quản lý AKI và Làm chậm tiến triển CKD (KDIGO 2023 / Bộ Y Tế)',
      guideline: 'KDIGO Clinical Practice Guideline for the Evaluation and Management of CKD',
      targetGoals: ['Bảo tồn chức năng thận còn lại, ngăn ngừa chạy thận nhân tạo', 'Kiểm soát huyết áp (< 120 mmHg SBP theo KDIGO 2021)', 'Giảm đạm niệu bằng SGLT2i và ức chế hệ RAAS'],
      initialManagement: [
        'Phân loại nguyên nhân AKI: Trước thận (thiếu dịch), Tại thận (hoại tử ống thận cấp, viêm cầu thận), Sau thận (sỏi/tắc nghẽn)',
        'Ngừng ngay các thuốc độc thận: NSAIDs, Aminoglycoside, thuốc cản quang tĩnh mạch',
        'Điều chỉnh liều tất cả các thuốc theo mức lọc cầu thận eGFR'
      ],
      firstLineDrugs: [
        { drugName: 'Dapagliflozin / Empagliflozin', class: 'Ức chế SGLT2 (Bảo vệ thận)', route: 'Uống', dosage: '10mg x 1 lần/ngày (sáng)', frequency: '1 lần/ngày', instructions: 'Chỉ định cho CKD có eGFR ≥ 20 mL/min để làm chậm suy thận và giảm tử vong tim mạch', isFirstLine: true },
        { drugName: 'Losartan / Telmisartan', class: 'Ức chế thụ thể ARB', route: 'Uống', dosage: '50mg - 100mg (Losartan) hoặc 40mg - 80mg (Telmisartan) x 1 lần/ngày', frequency: '1 lần/ngày', instructions: 'Ưu tiên hàng đầu nếu có UACR ≥ 30 mg/g. Theo dõi K+ và Creatinine sau 1-2 tuần', isFirstLine: true },
        { drugName: 'Natri Bicarbonate', class: 'Kiềm hóa máu', route: 'Uống', dosage: '500mg - 1000mg x 2-3 lần/ngày', frequency: 'Mỗi 8-12 giờ', instructions: 'Chỉ định khi HCO3- máu < 22 mmol/L để làm chậm thoái hóa thận', isFirstLine: true }
      ],
      secondLineDrugs: [
        { drugName: 'Finerenone (Kerendia)', class: 'Kháng thụ thể Mineralocorticoid không steroid (nsMRA)', route: 'Uống', dosage: '10mg - 20mg x 1 lần/ngày', frequency: '1 lần/ngày', instructions: 'Chỉ định cho CKD đi kèm ĐTĐ típ 2 để giảm đạm niệu và biến cố tim mạch', isFirstLine: false }
      ],
      supportiveCare: ['Hạn chế đạm 0.6 - 0.8 g/kg/ngày ở CKD G3-G5 chưa lọc máu', 'Hạn chế muối < 2g Natri/ngày', 'Bù đủ dịch đẳng trương nếu AKI trước thận']
    },
    complications: [
      { name: 'Tăng Kali máu ác tính (Hyperkalemia > 6.5 mmol/L)', timeframe: 'acute_24h', warningSigns: 'ECG có sóng T nhọn cao đối xứng, QRS giãn rộng, mất sóng P, rung thất', preventiveAction: 'Tiêm Canxi Clorid/Gluconat 10% bảo vệ màng tim + Insulin 10UI + Glucose 20% + Lọc máu cấp cứu', onCallAlertText: 'Báo động Tăng Kali máu > 6.0: Đo ECG ngay và dùng thuốc hạ Kali khẩn' },
      { name: 'Phù phổi cấp do ứ trệ tuần hoàn (Fluid Overload)', timeframe: 'acute_24h', warningSigns: 'Khó thở khi nằm, ran ẩm dâng lên 2 phế trường, SpO2 giảm nhanh', preventiveAction: 'Dùng Furosemide liều cao IV + Thở CPAP + Chỉ định lọc máu cấp cứu', onCallAlertText: 'Suy thận vô niệu + Khó thở: Báo động phù phổi cấp quá tải thể tích' }
    ],
    monitoringLabs: ['Creatinine, Ure, eGFR và Điện giải đồ (Na, K, Cl, Ca, P) mỗi 24h trong AKI', 'UACR nước tiểu và eGFR mỗi 3-6 tháng trong CKD', 'Khí máu động mạch nếu toan hóa máu nặng'],
    vaultPathways: [
      { khoCode: 'TC', khoName: 'Kho Lâm Sàng', articleTitle: 'Tiếp cận Thiểu niệu & Phù toàn thân', searchKeyword: 'thiểu niệu suy thận' },
      { khoCode: 'CD', khoName: 'Kho Tiêu Chuẩn CĐ', articleTitle: 'Phân loại KDIGO AKI và Phân giai đoạn CKD', searchKeyword: 'kdigo ckd aki' },
      { khoCode: 'CLS', khoName: 'Kho Cận Lâm Sàng', articleTitle: 'Tính eGFR CKD-EPI & Diễn giải UACR', searchKeyword: 'egfr ckd-epi' },
      { khoCode: 'PDDT', khoName: 'Kho Phác Đồ', articleTitle: 'Phác đồ Làm chậm tiến triển CKD KDIGO 2023', searchKeyword: 'phác đồ suy thận kdigo' },
      { khoCode: 'DUOC', khoName: 'Kho Dược', articleTitle: 'Chỉnh liều thuốc theo eGFR & Tra cứu SGLT2i', searchKeyword: 'chỉnh liều suy thận' },
      { khoCode: 'BC', khoName: 'Kho Biến Chứng', articleTitle: 'Cấp cứu Tăng Kali máu & Chỉ định lọc máu cấp', searchKeyword: 'tăng kali máu cấp cứu' }
    ]
  },

  // ─────────────────────────────────────────────────────────────────────────────
  // 5. VIÊM TỤY CẤP (ACUTE PANCREATITIS) - K85
  // ─────────────────────────────────────────────────────────────────────────────
  'viem_tuy_cap': {
    icdCode: 'K85.9',
    icdPrefixes: ['K85', 'K85.0', 'K85.1', 'K85.2', 'K85.8', 'K85.9'],
    diseaseName: 'Viêm Tụy Cấp (Acute Pancreatitis)',
    specialty: 'Tiêu Hóa - Cấp Cứu',
    severity: 'urgent',
    summary: 'Tình trạng viêm vô khuẩn cấp tính của tuyến tụy. Chẩn đoán xác định khi thỏa ít nhất 2 trong 3 tiêu chuẩn Atlanta (Lâm sàng, Men tụy, Hình ảnh học).',
    goldStandard: 'Thỏa ≥ 2/3 tiêu chuẩn Atlanta 2012: Đau bụng tụy cấp + Amylase/Lipase tăng ≥ 3 lần ULN + CT/MRI có viêm tụy',
    criteriaRule: {
      minMajorRequired: 2,
      ruleDescription: 'Chẩn đoán xác định khi có ít nhất 2 trong 3 tiêu chuẩn Atlanta: (1) Đau bụng kiểu tụy; (2) Lipase/Amylase ≥ 3x giới hạn trên; (3) Hình ảnh CT/MRI/Siêu âm điển hình'
    },
    criteria: [
      { id: 'ap_pain', type: 'major', label: 'Đau bụng cấp tính, dữ dội liên tục vùng thượng vị, lan ra sau lưng, tăng sau ăn no/uống rượu, giảm khi ngồi gập người', sourceGuideline: 'Atlanta 2012' },
      { id: 'ap_enzymes', type: 'lab', label: 'Lipase hoặc Amylase huyết thanh tăng cao gấp ≥ 3 lần giới hạn trên bình thường (Lipase có độ nhạy và độ đặc hiệu cao hơn)', labThreshold: 'Lipase/Amylase ≥ 3x ULN' },
      { id: 'ap_imaging', type: 'imaging', label: 'Hình ảnh viêm tụy cấp trên CT bụng có cản quang, MRI hoặc Siêu âm (Tụy phù nề to, thâm nhiễm mỡ quanh tụy, tụ dịch)', sourceGuideline: 'Atlanta 2012' },
      { id: 'ap_etiology_stone', type: 'minor', label: 'Bằng chứng sỏi mật / sỏi đoạn cuối ống mật chủ trên siêu âm/CT hoặc Bilirubin/ALP tăng', sourceGuideline: 'Bộ Y Tế' },
      { id: 'ap_etiology_triglyceride', type: 'lab', label: 'Triglyceride huyết thanh tăng rất cao > 1000 mg/dL (11.3 mmol/L)', labThreshold: 'Triglyceride > 11.3 mmol/L' }
    ],
    protocol: {
      title: 'Phác đồ Điều trị Viêm Tụy Cấp (IAP/APA 2013 / AGA 2018 / Bộ Y Tế)',
      guideline: 'International Association of Pancreatology (IAP/APA) Guidelines',
      targetGoals: ['Hồi sức dịch tích cực trong 24h đầu ngăn hoại tử tụy', 'Giảm đau hiệu quả', 'Phát hiện sớm và ngăn ngừa suy đa tạng'],
      initialManagement: [
        'Bù dịch Ringer Lactate tích cực: 200 - 500 mL/h (hoặc 5-10 mL/kg/h) trong 12-24h đầu, điều chỉnh theo Hct, Ure, lượng nước tiểu (> 0.5 mL/kg/h)',
        'Đánh giá độ nặng theo thang điểm BISAP hoặc Hct/BUN tại thời điểm 0h và 24h',
        'Cho ăn lại đường miệng sớm (trong 24-48h) khi giảm đau và không còn nôn, thức ăn lỏng ít béo'
      ],
      firstLineDrugs: [
        { drugName: 'Paracetamol IV / Tramadol', class: 'Thuốc giảm đau bậc 1-2', route: 'Tiêm truyền tĩnh mạch', dosage: 'Paracetamol 1g x 3-4 lần/ngày hoặc Tramadol 50-100mg IV', frequency: 'Mỗi 6-8 giờ', instructions: 'Giảm đau sớm để tránh stress chuyển hóa', isFirstLine: true },
        { drugName: 'Morphine / Pethidine', class: 'Opioid giảm đau mạnh', route: 'Tiêm TM chậm', dosage: 'Morphine 2mg - 5mg IV mỗi 4-6h khi đau nhiều không đáp ứng giảm đau thông thường', frequency: 'Khi đau nhiều', instructions: 'An toàn trong viêm tụy cấp, không làm tăng co thắt cơ vòng Oddi trên lâm sàng', isFirstLine: true },
        { drugName: 'Ringer Lactate', class: 'Dịch truyền tinh thể đẳng trương', route: 'Truyền tĩnh mạch', dosage: '2500ml - 4000ml/24h (tùy tình trạng tim mạch và đáp ứng)', frequency: 'Truyền liên tục', instructions: 'Ưu tiên Ringer Lactate hơn NaCl 0.9% vì giảm nguy cơ toan chuyển hóa tăng Clo', isFirstLine: true }
      ],
      secondLineDrugs: [
        { drugName: 'Meropenem / Imipenem', class: 'Carbapenem kháng sinh', route: 'Tiêm truyền IV', dosage: '1g x 3 lần/ngày', frequency: 'Mỗi 8 giờ', instructions: 'CHỈ DÙNG khi có nghi ngờ hoặc bằng chứng Hoại tử tụy nhiễm trùng (sau 7-14 ngày). KHÔNG dùng KS dự phòng đại trà', isFirstLine: false }
      ],
      supportiveCare: ['Đặt sonde dạ dày CHỈ KHI nôn ói nhiều hoặc chướng bụng liệt ruột nặng', 'Nội soi mật tụy ngược dòng (ERCP) trong 24-48h nếu có sỏi kẹt OMC kèm viêm đường mật']
    },
    complications: [
      { name: 'Suy tạng kéo dài (Persistent Organ Failure > 48h)', timeframe: 'acute_24h', warningSigns: 'Suy hô hấp (PaO2/FiO2 < 300), Suy thận (Creatinine > 2 mg/dL), Tụt HA cần vận mạch', preventiveAction: 'Theo dõi điểm Marshall biến đổi + Chuyển ICU hồi sức', onCallAlertText: 'Báo động Viêm tụy cấp nặng: Theo dõi SpO2, Huyết áp và Nước tiểu mỗi 2 giờ' },
      { name: 'Hoại tử tụy nhiễm trùng (Infected Pancreatic Necrosis)', timeframe: 'subacute_7d', warningSigns: 'Sốt cao trở lại sau tuần thứ 2, bạch cầu tăng vọt, CT có bọt khí trong ổ hoại tử', preventiveAction: 'Chụp CT có cản quang sau 72h + Kháng sinh Carbapenem + Dẫn lưu/Can thiệp xâm lấn tối thiểu (Step-up approach)', onCallAlertText: 'Viêm tụy sốt lại sau ngày thứ 7: Chỉ định CT bụng cản quang kiểm tra ổ hoại tử' }
    ],
    monitoringLabs: ['Hct, BUN/Creatinine và Điện giải đồ mỗi 12-24h trong 48h đầu để đánh giá đáp ứng bù dịch', 'Đường huyết và Canxi máu (phát hiện hạ Calci máu)', 'CT scan bụng có cản quang sau 72-96h nếu không cải thiện'],
    vaultPathways: [
      { khoCode: 'TC', khoName: 'Kho Lâm Sàng', articleTitle: 'Tiếp cận Đau bụng cấp vùng thượng vị', searchKeyword: 'đau thượng vị cấp' },
      { khoCode: 'CD', khoName: 'Kho Tiêu Chuẩn CĐ', articleTitle: 'Tiêu chuẩn Atlanta 2012 & Thang điểm BISAP', searchKeyword: 'tiêu chuẩn atlanta bisap' },
      { khoCode: 'CLS', khoName: 'Kho Cận Lâm Sàng', articleTitle: 'Định lượng Lipase/Amylase & CT Balthazar', searchKeyword: 'lipase amylase balthazar' },
      { khoCode: 'PDDT', khoName: 'Kho Phác Đồ', articleTitle: 'Phác đồ Bù dịch và Dinh dưỡng Viêm tụy cấp', searchKeyword: 'phác đồ viêm tụy cấp' },
      { khoCode: 'DUOC', khoName: 'Kho Dược', articleTitle: 'Hướng dẫn dùng Kháng sinh và Opioid trong VTC', searchKeyword: 'meropenem ringer lactate' },
      { khoCode: 'BC', khoName: 'Kho Biến Chứng', articleTitle: 'Chiến lược Step-up approach xử trí hoại tử tụy', searchKeyword: 'hoại tử tụy step up' }
    ]
  },

  // ─────────────────────────────────────────────────────────────────────────────
  // 6. ĐÁI THÁO ĐƯỜNG (DIABETES MELLITUS) & CẤP CỨU DKA/HHS - E10-E14
  // ─────────────────────────────────────────────────────────────────────────────
  'dai_thao_duong': {
    icdCode: 'E11.9',
    icdPrefixes: ['E10', 'E11', 'E11.0', 'E11.2', 'E11.5', 'E11.9', 'E13', 'E14'],
    diseaseName: 'Đái Tháo Đường Típ 2 / Típ 1 & Biến Chứng Cấp',
    specialty: 'Nội Tiết - Chuyển Hóa',
    severity: 'routine',
    summary: 'Rối loạn chuyển hóa đặc trưng bởi tăng đường huyết mạn tính. Chẩn đoán theo tiêu chuẩn ADA với 4 xét nghiệm (Đường đói, HbA1c, Nghiệm pháp OGTT, Đường ngẫu nhiên + Triệu chứng).',
    goldStandard: 'Thỏa ít nhất 1 trong 4 tiêu chuẩn ADA 2024 (Đường đói ≥ 126 mg/dL hoặc HbA1c ≥ 6.5% làm lại 2 lần độc lập)',
    criteriaRule: {
      minMajorRequired: 1,
      ruleDescription: 'Thỏa ít nhất 1 trong 4 tiêu chuẩn của ADA (cần lặp lại 2 lần độc lập nếu không có triệu chứng kinh điển 4 nhiều)'
    },
    criteria: [
      { id: 'dm_fpg', type: 'lab', label: 'Glucose huyết tương lúc đói (FPG) ≥ 126 mg/dL (7.0 mmol/L) sau nhịn ăn ít nhất 8 giờ', sourceGuideline: 'ADA 2024', labThreshold: 'FPG ≥ 7.0 mmol/L' },
      { id: 'dm_hba1c', type: 'lab', label: 'HbA1c ≥ 6.5% (48 mmol/mol) thực hiện tại phòng xét nghiệm chuẩn hóa NGSP', sourceGuideline: 'ADA 2024', labThreshold: 'HbA1c ≥ 6.5%' },
      { id: 'dm_ogtt', type: 'lab', label: 'Glucose huyết tương 2 giờ sau nghiệm pháp dung nạp 75g Glucose (OGTT) ≥ 200 mg/dL (11.1 mmol/L)', sourceGuideline: 'ADA 2024', labThreshold: 'OGTT ≥ 11.1 mmol/L' },
      { id: 'dm_random_symptoms', type: 'major', label: 'Triệu chứng kinh điển của tăng đường huyết (Ăn nhiều, Uống nhiều, Tiểu nhiều, Sút cân) + Đường huyết ngẫu nhiên ≥ 200 mg/dL (11.1 mmol/L)', sourceGuideline: 'ADA 2024' },
      { id: 'dm_dka_signs', type: 'major', label: 'Cấp cứu DKA: Hơi thở mùi táo chín (Aceton), thở nhanh sâu Kussmaul, Toan máu pH < 7.3, Ceton máu/niệu (+)', sourceGuideline: 'ADA DKA Protocol' }
    ],
    protocol: {
      title: 'Phác đồ Điều trị Đái Tháo Đường Típ 2 Cá Thể Hóa (ADA/EASD 2024 / Bộ Y Tế)',
      guideline: 'Standards of Care in Diabetes - ADA 2024',
      targetGoals: ['Mục tiêu HbA1c < 7.0% cho đa số người lớn (cá thể hóa 6.5 - 8.0%)', 'Đường huyết đói 80 - 130 mg/dL (4.4 - 7.2 mmol/L)', 'Đường huyết sau ăn 2h < 180 mg/dL (< 10.0 mmol/L)'],
      initialManagement: [
        'Thay đổi lối sống: Giảm tinh bột tinh chế, tập thể dục ít nhất 150 phút/tuần',
        'Đánh giá kèm theo: Bệnh tim mạch xơ vữa (ASCVD), Suy tim (HF), hoặc Bệnh thận mạn (CKD)?',
        'Nếu có ASCVD/HF/CKD: Bắt buộc chọn SGLT2i hoặc GLP-1RA độc lập với mức HbA1c ban đầu'
      ],
      firstLineDrugs: [
        { drugName: 'Metformin', class: 'Biguanide', route: 'Uống', dosage: '500mg - 1000mg x 2 lần/ngày (sau ăn)', frequency: 'Mỗi 12 giờ', instructions: 'Thuốc nền tảng hàng đầu. Dừng nếu eGFR < 30 mL/min để phòng toan Lactic', isFirstLine: true },
        { drugName: 'Empagliflozin / Dapagliflozin', class: 'SGLT2 inhibitor (Bảo vệ tim thận)', route: 'Uống', dosage: '10mg - 25mg x 1 lần/ngày (sáng)', frequency: '1 lần/ngày', instructions: 'Ưu tiên nếu có suy tim, bệnh thận mạn hoặc ASCVD. Uống đủ nước phòng ngừa nhiễm nấm niệu dục', isFirstLine: true },
        { drugName: 'Gliclazide MR', class: 'Sulfonylurea thế hệ mới', route: 'Uống', dosage: '30mg - 120mg x 1 lần/ngày (uống sáng trước ăn)', frequency: '1 lần/ngày', instructions: 'Tăng tiết Insulin hiệu quả cao chi phí thấp. Thận trọng nguy cơ hạ đường huyết ở người già', isFirstLine: true },
        { drugName: 'Sitagliptin / Vildagliptin', class: 'DPP-4 inhibitor', route: 'Uống', dosage: 'Sitagliptin 100mg x 1 lần/ngày hoặc Vildagliptin 50mg x 2 lần/ngày', frequency: 'Mỗi 12-24 giờ', instructions: 'Không gây tăng cân, nguy cơ hạ đường huyết rất thấp', isFirstLine: true }
      ],
      secondLineDrugs: [
        { drugName: 'Insulin Glargine (Lantus / Toujeo)', class: 'Insulin nền tác dụng kéo dài (Basal Insulin)', route: 'Tiêm dưới da (SC)', dosage: '10 UI hoặc 0.1 - 0.2 UI/kg tiêm 1 lần cố định buổi tối', frequency: '1 lần/ngày', instructions: 'Chỉ định khi HbA1c > 9-10% hoặc đường huyết > 300 mg/dL hoặc thất bại thuốc uống', isFirstLine: false }
      ],
      supportiveCare: ['Kiểm soát huyết áp đồng thời < 130/80 mmHg', 'Statin phòng ngừa tiên phát/thứ phát biến cố tim mạch', 'Khám đáy mắt và kiểm tra bàn chân định kỳ hàng năm']
    },
    complications: [
      { name: 'Hạ đường huyết cấp tính (Hypoglycemia < 3.9 mmol/L)', timeframe: 'acute_24h', warningSigns: 'Vã mồ hôi lạnh, run tay, tim đập nhanh, đói cồn cào, lơ mơ', preventiveAction: 'Quy tắc 15: Uống 15g đường nhanh (1/2 lon nước ngọt/3 viên đường) ➔ Đo lại sau 15p', onCallAlertText: 'Báo động Hạ đường huyết: Cho uống nước đường ngay hoặc truyền Glucose 20-30% IV' },
      { name: 'Nhiễm toan Ceton ĐTĐ (DKA) / Tăng áp lực thẩm thấu (HHS)', timeframe: 'acute_24h', warningSigns: 'Khô kiệt, thở Kussmaul, lú lẫn, Glucose > 300 mg/dL, Ceton máu > 3.0 mmol/L', preventiveAction: 'Bù dịch NaCl 0.9% 1000ml/h đầu + Bơm tiêm điện Insulin 0.1 UI/kg/h + Bù Kali', onCallAlertText: 'Cấp cứu DKA/HHS: Kích hoạt phác đồ truyền Insulin tĩnh mạch liên tục' }
    ],
    monitoringLabs: ['HbA1c định kỳ mỗi 3 tháng (mỗi 6 tháng nếu ổn định)', 'UACR nước tiểu và Creatinine/eGFR hàng năm', 'Bilan Lipid máu toàn phần hàng năm'],
    vaultPathways: [
      { khoCode: 'TC', khoName: 'Kho Lâm Sàng', articleTitle: 'Tiếp cận Tăng đường huyết & Hạ đường huyết', searchKeyword: 'đái tháo đường' },
      { khoCode: 'CD', khoName: 'Kho Tiêu Chuẩn CĐ', articleTitle: 'Tiêu chuẩn chẩn đoán ĐTĐ theo ADA 2024', searchKeyword: 'tiêu chuẩn ada hba1c' },
      { khoCode: 'CLS', khoName: 'Kho Cận Lâm Sàng', articleTitle: 'Diễn giải HbA1c và Bilan biến chứng đái tháo đường', searchKeyword: 'hba1c uacr' },
      { khoCode: 'PDDT', khoName: 'Kho Phác Đồ', articleTitle: 'Phác đồ Điều trị ĐTĐ Típ 2 ADA/EASD 2024', searchKeyword: 'phác đồ đái tháo đường' },
      { khoCode: 'DUOC', khoName: 'Kho Dược', articleTitle: 'Tra cứu SGLT2i, GLP-1RA và Bảng chỉnh liều Insulin', searchKeyword: 'metformin empagliflozin insulin' },
      { khoCode: 'BC', khoName: 'Kho Biến Chứng', articleTitle: 'Xử trí Cấp cứu DKA, HHS và Bàn chân đái tháo đường', searchKeyword: 'nhiễm toan ceton dka' }
    ]
  },

  // ─────────────────────────────────────────────────────────────────────────────
  // 7. TĂNG HUYẾT ÁP & CƠN THA CẤP CỨU (HYPERTENSION) - I10-I15
  // ─────────────────────────────────────────────────────────────────────────────
  'tang_huyet_ap': {
    icdCode: 'I10',
    icdPrefixes: ['I10', 'I11', 'I11.0', 'I12', 'I13', 'I15', 'I15.0'],
    diseaseName: 'Tăng Huyết Áp Vô Căn / Cơn THA Cấp Cứu',
    specialty: 'Tim Mạch',
    severity: 'routine',
    summary: 'Tình trạng áp lực máu động mạch tăng dai dẳng. Chẩn đoán khi HA phòng khám ≥ 140/90 mmHg hoặc HA lưu động 24h (ABPM) ≥ 130/80 mmHg. Phân biệt cơn THA khẩn cấp (Urgency) và THA cấp cứu (Emergency có tổn thương cơ quan đích).',
    goldStandard: 'Đo huyết áp đúng quy trình ≥ 2 lần ở ít nhất 2 buổi khám khác nhau (hoặc đo ABPM / HBPM tại nhà)',
    criteriaRule: {
      minMajorRequired: 1,
      ruleDescription: 'Huyết áp tâm thu ≥ 140 mmHg và/hoặc Huyết áp tâm trương ≥ 90 mmHg đo tại phòng khám'
    },
    criteria: [
      { id: 'htn_clinic_bp', type: 'major', label: 'Huyết áp phòng khám ≥ 140/90 mmHg qua ít nhất 2 lần đo đúng quy trình', sourceGuideline: 'VNHA 2022 / ESC 2024' },
      { id: 'htn_abpm', type: 'lab', label: 'Đo HA lưu động 24h (ABPM) trung bình ≥ 130/80 mmHg (hoặc ban ngày ≥ 135/85 mmHg)', sourceGuideline: 'ESC 2024' },
      { id: 'htn_emergency_tod', type: 'major', label: 'Cơn THA Cấp cứu (Emergency): HA > 180/120 mmHg kèm Tổn thương cơ quan đích mới xuất hiện (Đau ngực ACS, Phù phổi cấp, Đột quỵ não, Phình bóc tách ĐMC, Suy thận cấp)', sourceGuideline: 'ESC/AHA' },
      { id: 'htn_lvh', type: 'imaging', label: 'Phì đại thất trái trên ECG (Sokolow-Lyon > 35mm) hoặc Siêu âm tim (Chỉ số khối cơ thất trái LVMI tăng)', sourceGuideline: 'VNHA' }
    ],
    protocol: {
      title: 'Phác đồ Điều trị Tăng Huyết Áp (VNHA 2022 / ESC 2024)',
      guideline: 'Khuyến cáo chẩn đoán và điều trị THA - Phân hội Tăng huyết áp Việt Nam (VNHA)',
      targetGoals: ['Hạ HA về < 130/80 mmHg cho đa số bệnh nhân (nếu dung nạp tốt)', 'Người cao tuổi (≥ 65-70t): Mục tiêu HA tâm thu 130 - 139 mmHg', 'Ưu tiên phối hợp đôi cố định (Single-Pill Combination) ngay từ đầu'],
      initialManagement: [
        'Thay đổi lối sống: Ăn nhạt (< 5g muối/ngày), giảm cân, bỏ thuốc lá, hạn chế rượu bia',
        'Phân tầng nguy cơ tim mạch tổng thể (SCORE2 / ASCVD)',
        'Khởi đầu bằng viên phối hợp đôi: ACEi/ARB + CCB hoặc ACEi/ARB + Thiazide'
      ],
      firstLineDrugs: [
        { drugName: 'Amlodipine + Perindopril (Coveram)', class: 'CCB DHP + ACE inhibitor (Phối hợp đôi cố định)', route: 'Uống', dosage: '5mg/5mg hoặc 5mg/10mg x 1 viên/ngày (sáng)', frequency: '1 lần/ngày', instructions: 'Viên kết hợp chuẩn kiểm soát huyết áp 24h và bảo vệ tim thận', isFirstLine: true },
        { drugName: 'Telmisartan + Amlodipine (Twynsta)', class: 'ARB + CCB DHP', route: 'Uống', dosage: '40mg/5mg hoặc 80mg/5mg x 1 viên/ngày', frequency: '1 lần/ngày', instructions: 'Lựa chọn thay thế hàng đầu nếu ho khan do ACEi', isFirstLine: true },
        { drugName: 'Indapamide (Natrilix SR)', class: 'Lợi tiểu giống Thiazide (Thiazide-like)', route: 'Uống', dosage: '1.5mg x 1 lần/ngày (uống sáng)', frequency: '1 lần/ngày', instructions: 'Lợi tiểu ưu tiên hạ HA vượt trội so với Hydrochlorothiazide', isFirstLine: true },
        { drugName: 'Bisoprolol / Nebivolol', class: 'Chẹn Beta chọn lọc tim', route: 'Uống', dosage: 'Bisoprolol 2.5mg - 5mg x 1 lần/ngày', frequency: '1 lần/ngày', instructions: 'Ưu tiên đặc biệt nếu có kèm Bệnh mạch vành, Sau NMCT, Suy tim hoặc Nhịp tim nhanh', isFirstLine: true }
      ],
      secondLineDrugs: [
        { drugName: 'Nicardipine IV', class: 'Thuốc hạ áp truyền tĩnh mạch', route: 'Bơm tiêm điện IV', dosage: 'Khởi đầu 5 mg/h, tăng dần mỗi 15p tối đa 15 mg/h đến khi đạt HA mục tiêu', frequency: 'Truyền liên tục', instructions: 'Thuốc lựa chọn hàng đầu trong Cơn Tăng huyết áp Cấp cứu (Emergency)', isFirstLine: false }
      ],
      supportiveCare: ['Đo huyết áp tại nhà đúng quy trình và ghi sổ tay theo dõi', 'Tránh hạ huyết áp quá nhanh trong đột quỵ thiếu máu não cấp']
    },
    complications: [
      { name: 'Đột quỵ xuất huyết não do THA', timeframe: 'acute_24h', warningSigns: 'Đau đầu dữ dội đột ngột, nôn vọt, yếu liệt nửa người, hôn mê', preventiveAction: 'Hạ HA tâm thu từ từ xuống < 140 mmHg bằng Nicardipine IV + CT sọ não khẩn', onCallAlertText: 'HA > 200 kèm đau đầu nôn ói: Báo động xuất huyết não cấp' },
      { name: 'Bóc tách động mạch chủ ngực (Aortic Dissection)', timeframe: 'acute_24h', warningSigns: 'Đau ngực xé rách lan sau lưng, HA 2 tay chênh lệch > 20 mmHg', preventiveAction: 'Hạ nhịp tim < 60 l/p bằng Esmolol/Labetalol trước ➔ Hạ HA tâm thu < 120 mmHg', onCallAlertText: 'Đau ngực xé rách + THA ác tính: Đo HA 2 tay và chụp CTA động mạch chủ' }
    ],
    monitoringLabs: ['Điện giải đồ (Na, K) và Creatinine/eGFR trước và sau khi dùng ACEi/ARB 1-2 tuần', 'ECG 12 chuyển đạo tìm phì đại thất trái', 'Soi đáy mắt nếu có cơn THA cấp cứu'],
    vaultPathways: [
      { khoCode: 'TC', khoName: 'Kho Lâm Sàng', articleTitle: 'Tiếp cận Cơn Tăng huyết áp cấp cứu & Nhức đầu', searchKeyword: 'tăng huyết áp cấp cứu' },
      { khoCode: 'CD', khoName: 'Kho Tiêu Chuẩn CĐ', articleTitle: 'Phân độ Tăng huyết áp VNHA/ESC 2024', searchKeyword: 'tiêu chuẩn tăng huyết áp' },
      { khoCode: 'CLS', khoName: 'Kho Cận Lâm Sàng', articleTitle: 'Đo HA lưu động ABPM & Đánh giá tổn thương cơ quan đích', searchKeyword: 'abpm ecg tăng huyết áp' },
      { khoCode: 'PDDT', khoName: 'Kho Phác Đồ', articleTitle: 'Phác đồ Phối hợp thuốc Tăng huyết áp VNHA', searchKeyword: 'phác đồ tăng huyết áp' },
      { khoCode: 'DUOC', khoName: 'Kho Dược', articleTitle: 'Tra cứu Amlodipine, Perindopril và Nicardipine IV', searchKeyword: 'amlodipine perindopril nicardipine' },
      { khoCode: 'BC', khoName: 'Kho Biến Chứng', articleTitle: 'Xử trí Cơn THA Cấp cứu và Biến chứng tim mạch', searchKeyword: 'cơn tha cấp cứu' }
    ]
  },

  // ─────────────────────────────────────────────────────────────────────────────
  // 8. SUY TIM CẤP & MẠN (HEART FAILURE) - I50
  // ─────────────────────────────────────────────────────────────────────────────
  'suy_tim': {
    icdCode: 'I50.9',
    icdPrefixes: ['I50', 'I50.0', 'I50.1', 'I50.2', 'I50.9'],
    diseaseName: 'Suy Tim Cấp / Mạn Tính (HFrEF / HFpEF)',
    specialty: 'Tim Mạch',
    severity: 'urgent',
    summary: 'Hội chứng lâm sàng do bất thường cấu trúc hoặc chức năng tim dẫn đến giảm cung lượng tim hoặc tăng áp lực trong buồng tim. Phân loại theo phân suất tống máu thất trái (LVEF): HFrEF (≤ 40%), HFmrEF (41-49%), HFpEF (≥ 50%).',
    goldStandard: 'Siêu âm tim Doppler (đánh giá EF, kích thước buồng tim, áp lực đổ đầy) + BNP / NT-proBNP tăng cao',
    criteriaRule: {
      mandatoryIds: ['hf_echo_or_bnp'],
      minMajorRequired: 2,
      ruleDescription: 'Thỏa tiêu chuẩn Framingham (2 tiêu chuẩn chính HOẶC 1 chính + 2 phụ) HOẶC Siêu âm tim bất thường kết hợp NT-proBNP tăng'
    },
    criteria: [
      { id: 'hf_echo_or_bnp', type: 'mandatory', label: 'NT-proBNP > 125 pg/mL (ngoại trú) hoặc > 300 pg/mL (cấp cứu) HOẶC BNP > 35 pg/mL / > 100 pg/mL HOẶC Siêu âm tim có giảm LVEF', sourceGuideline: 'ESC 2023 / AHA 2022', labThreshold: 'NT-proBNP > 300 pg/mL' },
      { id: 'hf_dyspnea_orthopnea', type: 'major', label: 'Khó thở khi nằm đầu thấp (Orthopnea) hoặc Cơn khó thở kịch phát về đêm (PND)', sourceGuideline: 'Framingham Major' },
      { id: 'hf_jvd', type: 'major', label: 'Tĩnh mạch cổ nổi tự nhiên hoặc Phản hồi gan - tĩnh mạch cổ (+)', sourceGuideline: 'Framingham Major' },
      { id: 'hf_rales', type: 'major', label: 'Ran ẩm ứ trệ ở đáy 2 phổi (không do viêm phổi)', sourceGuideline: 'Framingham Major' },
      { id: 'hf_s3', type: 'major', label: 'Tiếng tim T3 (Gallop thất)', sourceGuideline: 'Framingham Major' },
      { id: 'hf_edema', type: 'minor', label: 'Phù mềm hai chi dưới đối xứng, tăng về chiều tối', sourceGuideline: 'Framingham Minor' },
      { id: 'hf_cardiomegaly', type: 'imaging', label: 'Bóng tim to trên X-quang ngực (Chỉ số tim/lồng ngực > 0.5)', sourceGuideline: 'Framingham' }
    ],
    protocol: {
      title: 'Phác đồ 4 Trụ Cột Nền Tảng Điều trị Suy tim HFrEF (ESC 2023 / AHA 2022 / VNHA)',
      guideline: 'ESC Guidelines for the diagnosis and treatment of acute and chronic heart failure',
      targetGoals: ['Giảm tử vong tim mạch và tái nhập viện vì suy tim', 'Cải thiện triệu chứng gắng sức và chất lượng cuộc sống (NYHA)', 'Tối ưu hóa liều 4 trụ cột (Fantastic Four)'],
      initialManagement: [
        'Phân tầng lâm sàng: Đợt cấp suy tim sung huyết (Ấm - Ướt) ➔ Dùng lợi tiểu quai IV sớm',
        'Khởi động sớm và phối hợp đồng thời 4 nhóm thuốc trụ cột ngay khi huyết động ổn định',
        'Hạn chế muối (< 2g/ngày) và dịch (< 1.5 - 2L/ngày nếu có phù/hạ Natri máu)'
      ],
      firstLineDrugs: [
        { drugName: 'Sacubitril/Valsartan (ARNI - Entresto)', class: 'ARNI (Ức chế thụ thể Angiotensin-Neprilysin)', route: 'Uống', dosage: 'Khởi đầu 50mg (24/26mg) x 2 lần/ngày, tăng dần đến đích 200mg (97/103mg) x 2 lần/ngày', frequency: 'Mỗi 12 giờ', instructions: 'Trụ cột 1. Nếu chuyển từ ACEi sang phải ngừng ACEi đủ 36 giờ', isFirstLine: true },
        { drugName: 'Bisoprolol / Metoprolol Succinate / Carvedilol', class: 'Chẹn Beta giao cảm thế hệ mới', route: 'Uống', dosage: 'Bisoprolol khởi đầu 1.25mg/ngày, tăng dần mỗi 2-4 tuần đến đích 10mg/ngày', frequency: '1 lần/ngày', instructions: 'Trụ cột 2. Chỉ khởi đầu khi bệnh nhân không còn ứ dịch nặng và huyết động ổn', isFirstLine: true },
        { drugName: 'Dapagliflozin / Empagliflozin', class: 'SGLT2 inhibitor', route: 'Uống', dosage: '10mg x 1 lần/ngày (sáng)', frequency: '1 lần/ngày', instructions: 'Trụ cột 3. Hiệu quả trên cả HFrEF, HFmrEF và HFpEF không phụ thuộc có ĐTĐ hay không', isFirstLine: true },
        { drugName: 'Spironolactone', class: 'Kháng thụ thể Mineralocorticoid (MRA)', route: 'Uống', dosage: '25mg - 50mg x 1 lần/ngày', frequency: '1 lần/ngày', instructions: 'Trụ cột 4. Theo dõi K+ và Creatinine máu sau 1 tuần khởi đầu', isFirstLine: true },
        { drugName: 'Furosemide', class: 'Lợi tiểu quai (Điều trị triệu chứng sung huyết)', route: 'Uống / Tiêm TM', dosage: '20mg - 80mg/ngày (hoặc tiêm TM liều gấp đôi liều uống tại nhà nếu đợt cấp)', frequency: 'Mỗi 12-24 giờ', instructions: 'Giảm liều khi bệnh nhân đạt tình trạng khô (Euvolemia)', isFirstLine: true }
      ],
      secondLineDrugs: [
        { drugName: 'Ivabradine', class: 'Ức chế kênh If nút xoang', route: 'Uống', dosage: '5mg - 7.5mg x 2 lần/ngày', frequency: 'Mỗi 12 giờ', instructions: 'Chỉ định khi nhịp xoang ≥ 70 l/p dù đã dùng chẹn Beta tối đa dung nạp', isFirstLine: false }
      ],
      supportiveCare: ['Theo dõi cân nặng hàng ngày vào buổi sáng (báo bác sĩ nếu tăng > 2kg trong 3 ngày)', 'Tiêm phòng cúm và phế cầu hàng năm']
    },
    complications: [
      { name: 'Phù phổi cấp huyết động (Flash Pulmonary Edema)', timeframe: 'acute_24h', warningSigns: 'Khó thở dữ dội, vã mồ hôi, khạc bọt hồng, SpO2 tụt < 80%, ran ẩm dâng nhanh', preventiveAction: 'Ngồi thõng 2 chân + Thở Oxy/CPAP + Furosemide 40-80mg IV + Nitroglycerin truyền TM', onCallAlertText: 'BÁO ĐỘNG PHÙ PHỔI CẤP: Cho BN ngồi dậy, thở CPAP và tiêm Furosemide IV' },
      { name: 'Hạ Natri máu do pha loãng / Suy tim tiến triển', timeframe: 'subacute_7d', warningSigns: 'Natri máu < 125 mmol/L, mệt mỏi, lơ mơ, phù kháng trị', preventiveAction: 'Hạn chế nước uống nghiêm ngặt < 1000 mL/ngày + Tối ưu hóa điều trị suy tim', onCallAlertText: 'Natri < 125 trên nền suy tim: Hạn chế nước, không truyền muối ưu trương trừ khi có co giật' }
    ],
    monitoringLabs: ['Điện giải đồ (Na, K) và Creatinine định kỳ mỗi 1-2 tuần khi chỉnh liều thuốc 4 trụ cột', 'NT-proBNP trước khi xuất viện để tiên lượng', 'Siêu âm tim kiểm tra sau 3-6 tháng điều trị nội khoa tối ưu'],
    vaultPathways: [
      { khoCode: 'TC', khoName: 'Kho Lâm Sàng', articleTitle: 'Tiếp cận Khó thở và Phù trong Suy tim', searchKeyword: 'khó thở suy tim' },
      { khoCode: 'CD', khoName: 'Kho Tiêu Chuẩn CĐ', articleTitle: 'Tiêu chuẩn Framingham & Phân loại Suy tim HFrEF/HFpEF', searchKeyword: 'framingham hfref hfpef' },
      { khoCode: 'CLS', khoName: 'Kho Cận Lâm Sàng', articleTitle: 'Diễn giải BNP / NT-proBNP và Siêu âm tim Doppler', searchKeyword: 'nt probnp siêu âm tim' },
      { khoCode: 'PDDT', khoName: 'Kho Phác Đồ', articleTitle: 'Phác đồ 4 Trụ Cột Suy Tim ESC 2023', searchKeyword: 'phác đồ suy tim esc' },
      { khoCode: 'DUOC', khoName: 'Kho Dược', articleTitle: 'Tra cứu Entresto, SGLT2i, Spironolactone và Lợi tiểu', searchKeyword: 'sacubitril valsartan entresto' },
      { khoCode: 'BC', khoName: 'Kho Biến Chứng', articleTitle: 'Cấp cứu Phù phổi cấp và Choáng tim (Cardiogenic Shock)', searchKeyword: 'phù phổi cấp choáng tim' }
    ]
  },
  ...
  'dot_quy_nao': {
    icdCode: 'I63.9',
    icdPrefixes: ['I61', 'I61.0', 'I63', 'I63.0', 'I63.9', 'I64'],
    diseaseName: 'Đột Quỵ Não Cấp (Nhồi Máu Não / Xuất Huyết Não)',
    specialty: 'Thần Kinh - Cấp Cứu',
    severity: 'emergency',
    summary: 'Thiếu hụt chức năng thần kinh khu trú khởi phát đột ngột do nguyên nhân mạch máu não. Cần thực hiện thang điểm BE-FAST / NIHSS và chụp CT/MRI sọ não khẩn cấp để phân biệt Nhồi máu não (85%) và Xuất huyết não (15%).',
    goldStandard: 'Chụp Cắt lớp vi tính (CT scan) hoặc Cộng hưởng từ (MRI) sọ não khẩn cấp',
    criteriaRule: {
      mandatoryIds: ['stroke_imaging'],
      minMajorRequired: 1,
      ruleDescription: 'Bắt buộc chụp CT/MRI sọ não để phân biệt xuất huyết hay nhồi máu + Có triệu chứng thần kinh khu trú khởi phát đột ngột (BE-FAST / NIHSS)'
    },
    criteria: [
      { id: 'stroke_imaging', type: 'mandatory', label: 'CT sọ não không cản quang loại trừ xuất huyết não (hoặc MRI Diffusion DWI thấy vùng hạn chế khuếch tán)', sourceGuideline: 'AHA/ASA 2019' },
      { id: 'stroke_fast_face', type: 'major', label: 'Mặt (Face): Méo miệng, lệch nhân trung, nếp nhăn mũi má một bên mờ', sourceGuideline: 'BE-FAST' },
      { id: 'stroke_fast_arm', type: 'major', label: 'Tay chân (Arm/Leg): Yếu liệt nửa người, rơi tay khi giơ ngang', sourceGuideline: 'BE-FAST' },
      { id: 'stroke_fast_speech', type: 'major', label: 'Ngôn ngữ (Speech): Nói đớ, nói ngọng, mất ngôn ngữ (Broca/Wernicke) hoặc không hiểu lời', sourceGuideline: 'BE-FAST' },
      { id: 'stroke_time', type: 'major', label: 'Thời gian khởi phát (Time): Xác định chính xác giờ bình thường cuối cùng (Last Known Well)', sourceGuideline: 'AHA/ASA' },
      { id: 'stroke_nihss', type: 'lab', label: 'Điểm đột quỵ NIHSS ≥ 1 điểm', labThreshold: 'NIHSS Score' }
    ],
    protocol: {
      title: 'Phác đồ Cấp cứu Đột quỵ Thiếu máu Não Cấp (AHA/ASA 2019 / Bộ Y Tế)',
      guideline: 'Guidelines for the Early Management of Patients With Acute Ischemic Stroke',
      targetGoals: ['Tái thông mạch máu não càng sớm càng tốt ("Thời gian là Não - Time is Brain")', 'Dùng thuốc tiêu sợi huyết Alteplase trong "Cửa sổ vàng" < 4.5 giờ', 'Lấy huyết khối bằng dụng cụ cơ học (EVT) trong < 6-24 giờ'],
      initialManagement: [
        'Kích hoạt Code Stroke Cấp cứu bệnh viện',
        'Đo đường huyết mao mạch ngay tại giường (loại trừ hạ đường huyết mô phỏng đột quỵ)',
        'Kiểm soát huyết áp: Nếu chỉ định tiêu sợi huyết, cần hạ HA < 185/110 mmHg bằng Nicardipine IV'
      ],
      firstLineDrugs: [
        { drugName: 'Alteplase (rt-PA)', class: 'Thuốc tiêu sợi huyết đường tĩnh mạch', route: 'Tiêm truyền tĩnh mạch', dosage: '0.9 mg/kg (tối đa 90mg): 10% tiêm tĩnh mạch trực tiếp trong 1 phút, 90% còn lại truyền tĩnh mạch trong 60 phút', frequency: 'Liều duy nhất cấp cứu', instructions: 'Chỉ định trong cửa sổ < 4.5h từ khi khởi phát. Kiểm tra nghiêm ngặt bảng kiểm chống chỉ định xuất huyết', isFirstLine: true },
        { drugName: 'Aspirin', class: 'Chống kết tập tiểu cầu', route: 'Uống / Bơm sonde', dosage: '160mg - 325mg uống liều đầu trong 24-48h sau khởi phát (hoãn 24h nếu có dùng rt-PA)', frequency: '1 lần/ngày', instructions: 'Sau đó duy trì 81-100mg/ngày để dự phòng thứ phát', isFirstLine: true },
        { drugName: 'Atorvastatin', class: 'Statin liều cao ổn định mảng xơ vữa', route: 'Uống', dosage: '40mg - 80mg x 1 lần/ngày (buổi tối)', frequency: '1 lần/ngày', instructions: 'Khởi đầu sớm cho đột quỵ nhồi máu não do xơ vữa động mạch lớn', isFirstLine: true }
      ],
      secondLineDrugs: [
        { drugName: 'Clopidogrel (Plavix)', class: 'Chống kết tập tiểu cầu P2Y12', route: 'Uống', dosage: 'Nạp 300mg, sau đó 75mg/ngày (phối hợp Aspirin DAPT trong 21 ngày đầu nếu đột quỵ nhẹ NIHSS ≤ 3 hoặc TIA nguy cơ cao)', frequency: '1 lần/ngày', instructions: 'Liệu pháp kháng kết tập tiểu cầu kép DAPT ngắn hạn theo thử nghiệm CHANCE/POINT', isFirstLine: false }
      ],
      supportiveCare: ['Nằm đầu bằng 0-15 độ nếu thiếu máu não (nâng đầu 30 độ nếu có tăng áp lực nội sọ)', 'Thử nuốt trước khi cho ăn đường miệng để phòng ngừa viêm phổi hít', 'Kiểm soát thân nhiệt < 37.5°C và đường huyết 140-180 mg/dL']
    },
    complications: [
      { name: 'Chuyển dạng xuất huyết sau nhồi máu não (Hemorrhagic Transformation)', timeframe: 'acute_24h', warningSigns: 'Tri giác xấu đi đột ngột, điểm NIHSS tăng ≥ 4 điểm, đau đầu dữ dội sau dùng rt-PA', preventiveAction: 'Ngừng ngay truyền rt-PA + Chụp CT sọ não khẩn + Chuẩn bị Cryoprecipitate/Fibrinogen', onCallAlertText: 'BÁO ĐỘNG BIẾN CHỨNG SAU rt-PA: Tri giác giảm, NIHSS tăng ➔ Ngừng tiêu sợi huyết và chụp CT sọ não' },
      { name: 'Phù não tiến triển & Tăng áp lực nội sọ', timeframe: 'subacute_7d', warningSigns: 'Lơ mơ, đồng tử giãn một bên, tam chứng Cushing (HA tăng, mạch chậm, thở không đều)', preventiveAction: 'Truyền Mannitol 20% hoặc Natri Clorid ưu trương 3% + Hội chẩn phẫu thuật mở sọ giải áp', onCallAlertText: 'Đột quỵ diện rộng có dấu hiệu phù não: Truyền Mannitol và hội chẩn ngoại thần kinh' }
    ],
    monitoringLabs: ['Theo dõi dấu hiệu sinh tồn và thang điểm NIHSS mỗi 15p trong 2h đầu, mỗi 30p trong 6h tiếp theo', 'Chụp CT sọ não kiểm tra sau 24h dùng thuốc tiêu sợi huyết trước khi bắt đầu dùng Aspirin', 'Siêu âm Doppler động mạch cảnh và Holter điện tâm đồ tìm nguồn gốc thuyên tắc từ tim'],
    vaultPathways: [
      { khoCode: 'TC', khoName: 'Kho Lâm Sàng', articleTitle: 'Tiếp cận Liệt nửa người & Thang điểm FAST/NIHSS', searchKeyword: 'đột quỵ não liệt nửa người' },
      { khoCode: 'CD', khoName: 'Kho Tiêu Chuẩn CĐ', articleTitle: 'Tiêu chuẩn Chỉ định & Chống chỉ định Tiêu sợi huyết rt-PA', searchKeyword: 'tiêu chuẩn rt-pa tiêu sợi huyết' },
      { khoCode: 'CLS', khoName: 'Kho Cận Lâm Sàng', articleTitle: 'Đọc CT sọ não sớm trong đột quỵ (Điểm ASPECTS)', searchKeyword: 'ct sọ não aspects' },
      { khoCode: 'PDDT', khoName: 'Kho Phác Đồ', articleTitle: 'Phác đồ Cấp cứu Đột quỵ AHA/ASA & Code Stroke', searchKeyword: 'phác đồ code stroke' },
      { khoCode: 'DUOC', khoName: 'Kho Dược', articleTitle: 'Tính liều Alteplase rt-PA và Liệu pháp DAPT Aspirin + Clopidogrel', searchKeyword: 'alteplase aspirin clopidogrel' },
      { khoCode: 'BC', khoName: 'Kho Biến Chứng', articleTitle: 'Xử trí Xuất huyết não sau dùng rt-PA và Phù não ác tính', searchKeyword: 'chuyển dạng xuất huyết phù não' }
    ]
  }
,
