/**
 * ════════════════════════════════════════════════════════════════════════════
 *  MARKDOWN EVIDENCE READER ENGINE — PATHOLOGY MODULE (CLINIPORTAL) [TypeScript Module]
 *  Bộ đọc & hiển thị tài liệu Markdown YHC chuẩn y khoa
 * ════════════════════════════════════════════════════════════════════════════
 */

export class PathologyMarkdownViewer {
  public static parseMarkdown(mdText: string): string {
    if (!mdText) return '';
    ...
    html = html.replace(/(<li class="md-li">.*<\/li>)/gis, '<ul class="md-ul">$1</ul>');
  ...
  public static async fetchAndShowMarkdown(
    filePath: string,
    modalTitle: string = 'Tài liệu Y học Chứng cứ'
  ): Promise<void> {
    ...
    bodyEl.innerHTML =
      '<div class="md-loading"><i class="fa-solid fa-spinner fa-spin"></i> Đang nạp tài liệu YHC...</div>';
...
if (typeof window !== 'undefined') {
  (window as any).PathologyMarkdownViewer = PathologyMarkdownViewer;
}
